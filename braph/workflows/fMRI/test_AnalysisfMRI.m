...
group1 = Group('SubjectfMRI', {sub11, sub12, sub13, sub14}, 'GroupName', 'GroupTestfMRI1');
...
group2 = Group('SubjectfMRI', {sub21, sub22, sub23}, 'GroupName', 'GroupTestfMRI2');

cohort = Cohort('Cohort fMRI', 'SubjectfMRI', atlas, {sub11, sub12, sub13, sub14, sub21, sub22, sub23});
...
for i = 1:1:length(measures)
    ...
    for j = 1:1:numel(measurement_keys)
        ...
        calculated_average = calculated_measurement.getGroupAverageValue();
        ...
        if Measure.is_global(measure)
            ...
            assert(isequal(size(calculated_average), [1 1]), ...
                ['BRAPH:AnalysisfMRI:Instantiation'], ...
                ['AnalysisfMRI does not initialize correctly with global measures.']) %#ok<*NBRAK>
        ...
        elseif Measure.is_nodal(measure)
            ...
            assert(isequal(size(calculated_average), [atlas.getBrainRegions().length(), 1]), ...
                ['BRAPH:AnalysisfMRI:Instantiation'], ...
                ['AnalysisfMRI does not initialize correctly with nodal measures.']) %#ok<*NBRAK>
        ...
        elseif Measure.is_binodal(measure)
            ...
            assert(isequal(size(calculated_average), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]), ...
                ['BRAPH:MeasurementDTI:Instantiation'], ...
                ['MeasurementDTI does not initialize correctly with binodal measures.']) %#ok<*NBRAK>
        end
    end
...
for i = 1:1:numel(measures)
    ...
    assert(analysis.getComparisons().length() == 1, ...
        ['BRAPH:AnalysisfMRI:calculateComparison'], ...
        ['AnalysisfMRI.calculateComparison() not working'])
...
for i = 1:1:numel(measures)
    ...
    assert(analysis.getRandomComparisons().length() == 1, ...
        ['BRAPH:AnalysisfMRI:calculateRandomComparison'], ...
        ['AnalysisfMRI.calculateRandomComparison() not working'])
    ...
    randomcomparison_average_1 = randomcomparison.getGroupAverageValue();
