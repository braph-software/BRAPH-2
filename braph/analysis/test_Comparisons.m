...
for i = 1:1:length(comparisons_class_list)
    ...
    subject_class = Comparison.getSubjectClass(comparisons_class);
    ...
    %act
    comparison = Comparison.getComparison(comparisons_class, 'c1', atlas, {group group});
    %assert
    assert(~isempty(comparison), ...
        ['BRAPH:Comparison:Instantiation'], ...
        ['Comparison instantiation fail.'])
end
...
for i = 1:1:length(comparisons_class_list)
    ...
    assert(ischar(comparison.getName()), ...
        ['BRAPH:Comparison:StaticFunctions'], ...
        ['Comparison getName() fail.'])
    assert(ischar(comparison.getDescription()), ...
        ['BRAPH:Comparison:StaticFunctions'], ...
        ['Comparison getDescription() fail.'])
    assert(ischar(comparison.getDescription()), ...
        ['BRAPH:Comparison:StaticFunctions'], ...
        ['Comparison getDescription() fail.'])
    assert(ischar(comparison.getAnalysisClass()), ...
        ['BRAPH:Comparison:StaticFunctions'], ...
        ['Comparison getAtlasesNumber() fail.'])
