...
for i = 1:1:length(analysis_class_list)
    ...
    %act
    analysis = Analysis.getAnalysis(analysis_class, cohort, measurement);
    measurement3 = Measurement.getMeasurement('ComparisionMRI', atlas, group);
    analysis.replaceMeasurement(2, measurement3);
    %assert
    assert(analysis.measurementnumber()==2, ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.replaceMeasurement() does not work')  
    assert( isequal(analysis.getMeasurement(2).getClass(), 'ComparisionMRI'), ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.replaceMeasurement() does not work') 
end
...
for i = 1:1:length(analysis_class_list)
    ...
    group = Group(sub_class, {sub1, sub2, sub3});
    measurement1 = Measurement.getMeasurement('MeasurementMRI', atlas, group);
    measurement2 = Measurement.getMeasurement('ComparisionMRI', atlas, group);
    measurement = {measurement1, measurement2};    
    %act
    analysis = Analysis.getAnalysis(analysis_class, cohort, measurement);  
...
for i = 1:1:length(analysis_class_list)
    ...
    measurement1 = Measurement.getMeasurement('MeasurementMRI', atlas, group);
    measurement2 = Measurement.getMeasurement('ComparisionMRI', atlas, group);
    measurement3 = Measurement.getMeasurement('RandomComparisionMRI', atlas, group);
    measurement = {measurement1, measurement2, measurement3};    
    %act
    analysis = Analysis.getAnalysis(analysis_class, cohort, measurement);  
    analysis.movetoMeasurement(3, 1);
    %assert 
    assert( isequal(analysis.getMeasurement(1).getClass(), 'RandomComparisionMRI'), ...
        'BRAPH:Analysis:Bug', ...
...
for i = 1:1:length(analysis_class_list)
    ...
    assert( isequal(analysis.getMeasurement(1).getClass(), 'ComparisionMRI'), ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.removeMeasurements() does not work') 
...
for i = 1:1:length(analysis_class_list)
    ...
    %act
    analysis = Analysis.getAnalysis(analysis_class, cohort, measurement);
    [selected, added] = analysis.addaboveMeasurements('RandomComparisionMRI', [2]); %#ok<NBRAK>
    %assert 
    assert(analysis.measurementnumber()==4, ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.addaboveMeasurements() does not work')  
    assert( isequal(analysis.getMeasurement(2).getClass(), 'RandomComparisionMRI'), ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.addaboveMeasurements() does not work') 
end
...
for i = 1:1:length(analysis_class_list)
    ...
    assert( isequal(analysis.getMeasurement(3).getClass(), 'ComparisionMRI'), ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.movedownMeasurements() does not work') 
end
...
for i = 1:1:length(analysis_class_list)
    ...
    %act
    analysis = Analysis.getAnalysis(analysis_class, cohort, measurement);
    selected = analysis.move2topMeasurements([3]); 
    %assert 
    assert(analysis.measurementnumber()==3, ...
        'BRAPH:Analysis:Bug', ...
...
for i = 1:1:length(analysis_class_list)
    ...
    %act
    analysis = Analysis.getAnalysis(analysis_class, cohort, measurement);
    selected = analysis.move2bottomMeasurements([1]); 
    %assert 
    assert(analysis.measurementnumber()==3, ...
        'BRAPH:Analysis:Bug', ...
        'Analysis.move2topMeasurements() does not work')  
