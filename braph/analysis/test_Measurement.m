...
for i = 1:1:length(measurement_class_list)
    ...
    %act
    measurement = Measurement.getMeasurement(measurement_class, atlas, group);
    %assert
...
for i = 1:1:length(measurement_class_list)
    ...
    assert(~isempty(measurement.getBrainAtlasesNumber()), ...
        ['BRAPH:Measurement:StaticFunctions'], ...
        ['Measurement getAtlasesNumber() fail.']) %#ok<NBRAK>
    assert(~isempty(measurement.getGroupsNumber()), ...
        ['BRAPH:Measurement:StaticFunctions'], ...
