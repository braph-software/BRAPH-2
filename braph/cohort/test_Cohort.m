...
for i = 1:1:length(sub_class_list)
    ...
    assert(cohort.getGroup().length() == 0, ...
        ['BRAPH:Cohort:Groups'], ...
    ...
    cohort.getGroup().add(group1.getName(), group1);
    assert(cohort.getGroup().length() == 1, ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>
    
    group2 = Group(cohort.getSubjectClass(), {}, 'GroupName', 'Group 2', 'GroupDescription', 'Group 2 Description');
    assert(~cohort.getGroup().contains(group2), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    assert(~cohort.getGroup().contains(2), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    

    cohort.getGroup().add(group2.getName(), group2)
    assert(cohort.getGroup().contains(group2), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    assert(cohort.getGroup().contains(2), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    assert(cohort.getGroup().length() == 2, ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>
    assert(cohort.getGroup().getValue(2) == group2, ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>
    
    cohort.getGroup().remove(1)
    assert(cohort.getGroup().length() == 1, ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>
    
    group1 = Group(cohort.getSubjectClass(), {}, 'GroupName', 'Group 1', 'GroupDescription', 'Group 1 Description');
    cohort.getGroup().add(group1.getName(), group1, 1)
    assert(cohort.getGroup().length() == 2, ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>
    assert(cohort.getGroup().contains(group1), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    assert(cohort.getGroup().contains(1), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    assert(isequal(cohort.getGroup().getValue(1).getName(), 'Group 1'), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>
    ...
    assert(isequal(cohort.getGroup().getValue(1).getSubjects(), {sub1}), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    assert(isequal(cohort.getGroup().getValue(2).getSubjects(), {sub2 sub3}), ...
        ['BRAPH:Cohort:Groups'], ...
        ['The management of groups does not work']) %#ok<NBRAK>    
    ...
    copied_subjects = copy_cohort.getGroup().getValue(2).getSubjects();
