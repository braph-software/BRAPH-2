...
switch lower(standardize_rule)
    case {'range', 'compress'}  % scaled interval
        if min(A(:)) ~= max(A(:))
            B = (A - min(A(:))) / (max(A(:)) - min(A(:)));
        else
            B = binarize(A);
        end
    otherwise  % {'threshold', 'one', '1', 1}  % one-rule
        B = A;
        B(B < 0) = 0;
        B(B > 1) = 1;
end
