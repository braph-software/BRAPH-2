...
for i = 1:1:length(graph_class_list)
    ...
    d = Diameter(g).getValue();
    
    assert(~isempty(d), ...
        ['BRAPH:' graph_class ':Diameter'], ...
        ['Diameter is not calculated for ' graph_class])   
end
...
for i = 1:1:length(graph_class_list)
    ...
    g = Graph.getGraph(graph_class, A);
    d = Diameter(g, 'DiameterRule', 'default').getValue();
    ...
    assert(isequal(d, known_solution), ...
        ['BRAPH:' graph_class ':Diameter'], ...
        ['Diameter is not calculated for ' graph_class])   
end
...
d = Diameter(g, 'RadiusRule', 'default').getValue();
distance = Distance(g).getValue();
[~, ~, ~, ~, bct_value]= charpath(distance);

assert(isequal(d, bct_value), ...
   ('BRAPH:Diameter'), ...
   ('Diameter is not calculated for BCT.'))
