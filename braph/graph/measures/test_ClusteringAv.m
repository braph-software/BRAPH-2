...
assert(isequal(clustering_av_1, clustering_av_2), ...
    'BRAPH:Clustering:Bug', ...
    'ClusteringAv(''DirectedTrianglesRule'', ''cycle'') is not being calculated correctly for GraphBD')

%% Test 4: Comparison with known BD graph - in rule
g = GraphBD(A_BD);
clustering_av_1 = ClusteringAv(g,'DirectedTrianglesRule', 'in').getValue();
...
g = GraphBD(A_BD);
clustering_av_1 = ClusteringAv(g,'DirectedTrianglesRule', 'out').getValue();
...
g = GraphBD(A_BD);
clustering_av_1 = ClusteringAv(g,'DirectedTrianglesRule', 'middleman').getValue();
...
g = GraphBD(A_BD);
clustering_av_1 = ClusteringAv(g,'DirectedTrianglesRule', 'all').getValue();
...
g = GraphWD(A_test);
A_WD = g.getA();
clustering_av_1 = round(ClusteringAv(g,'DirectedTrianglesRule', 'all').getValue(),10);
