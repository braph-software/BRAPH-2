...
for i = 1:1:length(graph_class_list)
    ...
    ecc = LocalEfficiencyAv(g).getValue();
    
    assert(~isempty(ecc), ...
        ['BRAPH:' graph_class ':LocalEfficiencyAv'], ...
        ['LocalEfficiencyAv is not calculated for ' graph_class])
end
...
for i = 1:1:length(graph_class_list)
    ...
    g = Graph.getGraph(graph_class, A);
    le = LocalEfficiencyAv(g).getValue();
...
assert(isequal(le, mean(bct_value)), ...
    ['BRAPH:LocalEfficiencyAv'], ...
    ['LocalEfficiencyAv is not calculated for BCT.'])
