...
for i = 1:1:length(graph_class_list)
    ...
    ecc = LocalEfficencyAv(g).getValue();
    
    assert(~isempty(ecc), ...
        ['BRAPH:' graph_class ':LocalEfficencyAv'], ...
        ['LocalEfficencyAv is not calculated for ' graph_class])
end
...
for i = 1:1:length(graph_class_list)
    ...
    g = Graph.getGraph(graph_class, A);
    le = LocalEfficencyAv(g).getValue();
...
assert(isequal(le, mean(bct_value)), ...
    ['BRAPH:LocalEfficencyAv'], ...
    ['LocalEfficencyAv is not calculated for BCT.'])
