classdef PlotBrainAtlas < PlotBrainSurf
    ...
    methods (Access = protected)  % Set Brain Regions
        function setRegions(ba)
            ...
            X = cellfun(@(br) br.get('X'), ba.getBrainAtlas().get('BR_DICT').getItems(), 'UniformOutput', false);
            Y = cellfun(@(br) br.get('Y'), ba.getBrainAtlas().get('BR_DICT').getItems(), 'UniformOutput', false);
            Z = cellfun(@(br) br.get('Z'), ba.getBrainAtlas().get('BR_DICT').getItems(), 'UniformOutput', false);
            ids = cellfun(@(br) br.get('ID'), ba.getBrainAtlas().get('BR_DICT').getItems(), 'UniformOutput', false); %#ok<PROP>
            ...
            if ba.getBrainAtlas().get('BR_DICT').length() > 0
                ...
                ba.syms.h = NaN(1, ba.getBrainAtlas().get('BR_DICT').length());
                ba.syms.X = X;
                ba.syms.Y = Y;
                ba.syms.Z = Z;
                
                ba.sphs.h = NaN(1, ba.getBrainAtlas().get('BR_DICT').length());
                ba.sphs.X = X;
                ba.sphs.Y = Y;
                ba.sphs.Z = Z;
                ba.sphs.R = PlotBrainAtlas.INIT_SPH_R * ones(1, ba.getBrainAtlas().get('BR_DICT').length());
                
                ba.ids.h = NaN(1, ba.getBrainAtlas().get('BR_DICT').length());
                ba.ids.X = X;
                ba.ids.Y = Y;
                ba.ids.Z = Z;
                ba.ids.ID = ids;  %#ok<PROP>

                ba.labs.h = NaN(1,  ba.getBrainAtlas().get('BR_DICT').length());
                ba.labs.X = X;
                ba.labs.Y = Y;
                ba.labs.Z = Z;
                ba.labs.LAB = labels;
    ...
    methods  % Functions symbols
        function h = br_sym(ba, i, varargin)
            ...
            % center coordinates
            X = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('X');
            Y = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Y');
            Z = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Z');
        ...
        function br_syms(ba, i_vec, varargin)
            ...
            if nargin < 2 || isempty(i_vec)
                i_vec = 1:1:ba.getBrainAtlas().get('BR_DICT').length();
            end
        ...
        function br = get_sym_br(ba, h)
            ...
            i = ba.get_sym_i(h);
            br = ba.getBrainAtlas().get('BR_DICT').getItem(i);
        end
        function br_syms_settings(ba, i_vec, varargin)
            ...
            FigColor = get_from_varargin([.9 .4 .1], 'figcolor', varargin);
    ...
    methods  % Functions spheres
        ...
        function br_sphs_settings(ba, i_vec, varargin)
            ...
            set(ui_text, 'BackgroundColor', [.9 .4 .1])
            ...
            set(ui_slider_alpha, 'BackgroundColor', [.9 .4 .1])
            ...
            set(ui_slider_facealpha, 'BackgroundColor', [.9 .4 .1])
            ...
            set(ui_slider_edgealpha, 'BackgroundColor', [.9 .4 .1])
            ...
            set(ui_text_radius, 'BackgroundColor', [.9 .4 .1])
            ...
            set(ui_edit_radius, 'BackgroundColor', [.9 .4 .1])
    ...
    methods  % Functions ids
        function h = br_id(ba, i, varargin)
            ...
            Y = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Y');
            Z = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Z');
            ID = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('ID');
        ...
        function br_ids_settings(ba, i_vec, varargin)
            ...
            set(ui_text_interpreter, 'BackgroundColor', [.9 .4 .1])
            ...
            set(ui_popup_interpreter, 'BackgroundColor', [.9 .4 .1])
    ...
    methods  % Functions labels
        function h = br_lab(ba, i, varargin)
            ...
            Y = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Y');
            Z = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Z');
            LAB = ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Label');
    ...
    methods  % distance
        function distance_map_on(ba, i_vec)
            ...
            if ~isempty(i_vec)
                brain_surf_coords = ba.get('coordinates');
                inverse_distance = zeros(length(i_vec), size(brain_surf_coords, 2));
                for m = 1:1:length(i_vec)
                    i = i_vec(m);
                    position = [ba.getBrainAtlas().get('BR_DICT').getItem(i).get('X'), ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Y'), ba.getBrainAtlas().get('BR_DICT').getItem(i).get('Z')];
                    inverse_distance(m, :) = sum((brain_surf_coords - position').^2).^-.5;
                end
