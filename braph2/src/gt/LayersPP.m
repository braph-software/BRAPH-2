classdef LayersPP < PanelProp
	...
	%  <strong>1</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the panel to select the layers of an adjacency matrix.
	%  <strong>2</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the panel to select the layers of an adjacency matrix.
	%  <strong>3</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the panel to select the layers of an adjacency matrix.
	%  <strong>4</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the panel to select the layers of an adjacency matrix.
	%  <strong>5</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the panel to select the layers of an adjacency matrix.
	%  <strong>6</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the panel to select the layers of an adjacency matrix.
	...
	%  <strong>34</strong> <strong>G_PROP</strong> 	G_PROP (data, scalar) is the graph property number.
	%  <strong>35</strong> <strong>ENABLE</strong> 	ENABLE (gui, option) switches table between on and off.
	%  <strong>36</strong> <strong>ROWNAME</strong> 	ROWNAME (gui, stringlist) determines the table row names.
	%  <strong>37</strong> <strong>COLUMNNAME</strong> 	COLUMNNAME (gui, stringlist) determines the table column names.
	%  <strong>38</strong> <strong>TABLE_HEIGHT</strong> 	TABLE_HEIGHT (gui, size) is the pixel height of the property panel when the table is shown.
	%  <strong>39</strong> <strong>TABLE</strong> 	TABLE (evanescent, handle) is the table.
	%
	...
	properties (Constant) % properties
		G_PROP = 34; %CET: Computational Efficiency Trick
		G_PROP_TAG = 'G_PROP';
		G_PROP_CATEGORY = 4;
		G_PROP_FORMAT = 11;
		...
		ROWNAME = 36; %CET: Computational Efficiency Trick
		ROWNAME_TAG = 'ROWNAME';
		ROWNAME_CATEGORY = 9;
		ROWNAME_FORMAT = 3;
		
		COLUMNNAME = 37; %CET: Computational Efficiency Trick
		COLUMNNAME_TAG = 'COLUMNNAME';
		COLUMNNAME_CATEGORY = 9;
		COLUMNNAME_FORMAT = 3;
		
		TABLE_HEIGHT = 38; %CET: Computational Efficiency Trick
		TABLE_HEIGHT_TAG = 'TABLE_HEIGHT';
		TABLE_HEIGHT_CATEGORY = 9;
		TABLE_HEIGHT_FORMAT = 22;
		
		TABLE = 39; %CET: Computational Efficiency Trick
		TABLE_TAG = 'TABLE';
		TABLE_CATEGORY = 7;
		TABLE_FORMAT = 18;
	end
	...
	methods (Static) % inspection
		...
		function prop_list = getProps(category)
			...
			switch category
				...
				case 4 % Category.DATA
					prop_list = [4 21 22 27 34];
				case 6 % Category.QUERY
					prop_list = [9 10 14 15 16 17 18 19 20];
				case 7 % Category.EVANESCENT
					prop_list = [8 13 25 26 28 29 30 31 32 33 39];
				case 8 % Category.FIGURE
					prop_list = 12;
				case 9 % Category.GUI
					prop_list = [7 11 23 24 35 36 37 38];
				otherwise
					prop_list = [];
			end
		end
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				layerspp_tag_list = { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'X_DRAW'  'UPDATE'  'REDRAW'  'EL'  'PROP'  'HEIGHT'  'TITLE'  'LABEL_TITLE'  'BUTTON_CB'  'GUI_CB'  'LISTENER_CB'  'BUTTON_CALC'  'BUTTON_DEL'  'LISTENER_SET'  'LISTENER_MEMORIZED'  'LISTENER_LOCKED'  'G_PROP'  'ENABLE'  'ROWNAME'  'COLUMNNAME'  'TABLE_HEIGHT'  'TABLE' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			layerspp_category_list = { 1  1  3  4  2  2  9  7  6  6  9  8  7  6  6  6  6  6  6  6  4  4  9  9  7  7  4  7  7  7  7  7  7  4  9  9  9  9  7 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			layerspp_format_list = { 2  2  8  2  2  2  4  18  4  4  8  20  18  4  4  4  4  4  4  4  8  11  22  2  18  18  8  18  18  18  19  19  19  11  5  3  3  22  18 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 34 % LayersPP.G_PROP
					prop_settings = Format.getFormatSettings(11);
				case 35 % LayersPP.ENABLE
					prop_settings = {'on', 'off'};
				case 36 % LayersPP.ROWNAME
					prop_settings = Format.getFormatSettings(3);
				case 37 % LayersPP.COLUMNNAME
					prop_settings = Format.getFormatSettings(3);
				case 38 % LayersPP.TABLE_HEIGHT
					prop_settings = Format.getFormatSettings(22);
				case 39 % LayersPP.TABLE
					prop_settings = Format.getFormatSettings(18);
				case 3 % LayersPP.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 1 % LayersPP.NAME
					prop_default = 'LayersPP';
				case 2 % LayersPP.DESCRIPTION
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pr, prop, varargin)
			...
			switch prop
				...
				case 18 % LayersPP.X_DRAW
					value = calculateValue@PanelProp(pr, 18, varargin{:}); % also warning
					if value
					    pr.memorize('TABLE')
					end
					
				case 19 % LayersPP.UPDATE
