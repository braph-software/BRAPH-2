classdef NNClassifierMLP_Evaluator < NNEvaluator
	...
	%  <strong>8</strong> <strong>NN</strong> 	NN (data, item) contains a trained neural network classifier.
	%  <strong>9</strong> <strong>D</strong> 	D (data, item) is the dataset to evaluate the neural network model.
	%  <strong>10</strong> <strong>GROUND_TRUTH</strong> 	GROUND_TRUTH (query, stringlist) returns the matrix of ground truth derived from the targets.
	%  <strong>11</strong> <strong>AUC</strong> 	AUC (result, rvector) provides the value of the area under the receiver operating characteristic curve.
	%  <strong>12</strong> <strong>MACRO_AUC</strong> 	MACRO_AUC (result, scalar) provides the metric of the average AUC value.
	%  <strong>13</strong> <strong>C_MATRIX</strong> 	C_MATRIX (result, matrix) provides the confusion matrix.
	%  <strong>14</strong> <strong>P</strong> 	P (parameter, scalar) is the permutation number.
	%  <strong>15</strong> <strong>PERM_SEEDS</strong> 	PERM_SEEDS (result, rvector) is the list of seeds for the random permutations.
	%  <strong>16</strong> <strong>FEATURE_IMPORTANCE</strong> 	FEATURE_IMPORTANCE (result, cell) quantifies the average significance and impact of individual input features within neural network models. Various techniques, such as permutation feature importance for MLPs and gradient-based analysis for CNNs, can be applied to quantify this aspect.
	%
	...
	properties (Constant) % properties
		...
		AUC = 11; %CET: Computational Efficiency Trick
		AUC_TAG = 'AUC';
		AUC_CATEGORY = 5;
		AUC_FORMAT = 12;
		
		MACRO_AUC = 12; %CET: Computational Efficiency Trick
		MACRO_AUC_TAG = 'MACRO_AUC';
		MACRO_AUC_CATEGORY = 5;
		MACRO_AUC_FORMAT = 11;
		
		C_MATRIX = 13; %CET: Computational Efficiency Trick
		C_MATRIX_TAG = 'C_MATRIX';
		C_MATRIX_CATEGORY = 5;
		C_MATRIX_FORMAT = 14;
		
		P = 14; %CET: Computational Efficiency Trick
		P_TAG = 'P';
		P_CATEGORY = 3;
		P_FORMAT = 11;
		...
		FEATURE_IMPORTANCE = 16; %CET: Computational Efficiency Trick
	...
	methods (Static) % inspection
		...
		function check_out = existsTag(tag)
			...
			check = any(strcmp(tag, { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'NN'  'D'  'GROUND_TRUTH'  'AUC'  'MACRO_AUC'  'C_MATRIX'  'P'  'PERM_SEEDS'  'FEATURE_IMPORTANCE' })); %CET: Computational Efficiency Trick
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				prop = find(strcmp(pointer, { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'NN'  'D'  'GROUND_TRUTH'  'AUC'  'MACRO_AUC'  'C_MATRIX'  'P'  'PERM_SEEDS'  'FEATURE_IMPORTANCE' })); % tag = pointer %CET: Computational Efficiency Trick
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				nnclassifiermlp_evaluator_tag_list = { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'NN'  'D'  'GROUND_TRUTH'  'AUC'  'MACRO_AUC'  'C_MATRIX'  'P'  'PERM_SEEDS'  'FEATURE_IMPORTANCE' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			nnclassifiermlp_evaluator_category_list = { 1  1  3  4  2  2  6  4  4  6  5  5  5  3  5  5 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			nnclassifiermlp_evaluator_format_list = { 2  2  8  2  2  2  2  8  8  3  12  11  14  11  12  16 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 10 % NNClassifierMLP_Evaluator.GROUND_TRUTH
					prop_settings = Format.getFormatSettings(3);
				case 11 % NNClassifierMLP_Evaluator.AUC
					prop_settings = Format.getFormatSettings(12);
				case 12 % NNClassifierMLP_Evaluator.MACRO_AUC
					prop_settings = Format.getFormatSettings(11);
				case 13 % NNClassifierMLP_Evaluator.C_MATRIX
					prop_settings = Format.getFormatSettings(14);
				case 14 % NNClassifierMLP_Evaluator.P
					prop_settings = Format.getFormatSettings(11);
				case 15 % NNClassifierMLP_Evaluator.PERM_SEEDS
					prop_settings = Format.getFormatSettings(12);
				case 16 % NNClassifierMLP_Evaluator.FEATURE_IMPORTANCE
					prop_settings = Format.getFormatSettings(16);
				case 3 % NNClassifierMLP_Evaluator.TEMPLATE
					prop_settings = 'NNClassifierMLP_Evaluator';
				case 8 % NNClassifierMLP_Evaluator.NN
