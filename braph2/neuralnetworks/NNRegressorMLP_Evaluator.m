classdef NNRegressorMLP_Evaluator < NNEvaluator
	...
	%  <strong>12</strong> <strong>GROUND_TRUTH</strong> 	GROUND_TRUTH (query, matrix) returns the matrix of ground truth derived from the targets.
	%  <strong>13</strong> <strong>CORR</strong> 	CORR (result, rvector) provides the metric of the correlation of coefficients.
	%  <strong>14</strong> <strong>DET</strong> 	DET (result, rvector) provides the coefficient of determination, a measure showing how well the predictions are replicated by the model.
	%  <strong>15</strong> <strong>MAE</strong> 	MAE (result, rvector) provides the metric of the mean absolute error.
	%  <strong>16</strong> <strong>MSE</strong> 	MSE (result, rvector) provides the metric of the mean squared error.
	%  <strong>17</strong> <strong>RMSE</strong> 	RMSE (result, rvector) provides the metric of the root mean squared error.
	%  <strong>18</strong> <strong>P</strong> 	P (parameter, scalar) is the permutation number.
	%  <strong>19</strong> <strong>PERM_SEEDS</strong> 	PERM_SEEDS (result, rvector) is the list of seeds for the random permutations.
	%  <strong>20</strong> <strong>FEATURE_IMPORTANCE</strong> 	FEATURE_IMPORTANCE (result, cell) quantifies the average significance and impact of individual input features within neural network models. Various techniques, such as permutation feature importance for MLPs and gradient-based analysis for CNNs, can be applied to quantify this aspect.
	%  <strong>21</strong> <strong>PFSP</strong> 	PFSP (gui, item) contains the panel figure of the scatter plot for regression model.
	%
	...
	properties (Constant) % properties
		...
		GROUND_TRUTH = 12; %CET: Computational Efficiency Trick
		GROUND_TRUTH_TAG = 'GROUND_TRUTH';
		GROUND_TRUTH_CATEGORY = 6;
		GROUND_TRUTH_FORMAT = 14;
		
		CORR = 13; %CET: Computational Efficiency Trick
		CORR_TAG = 'CORR';
		CORR_CATEGORY = 5;
		CORR_FORMAT = 12;
		
		DET = 14; %CET: Computational Efficiency Trick
		DET_TAG = 'DET';
		DET_CATEGORY = 5;
		DET_FORMAT = 12;
		
		MAE = 15; %CET: Computational Efficiency Trick
		MAE_TAG = 'MAE';
		MAE_CATEGORY = 5;
		MAE_FORMAT = 12;
		
		MSE = 16; %CET: Computational Efficiency Trick
		MSE_TAG = 'MSE';
		MSE_CATEGORY = 5;
		MSE_FORMAT = 12;
		
		RMSE = 17; %CET: Computational Efficiency Trick
		RMSE_TAG = 'RMSE';
		RMSE_CATEGORY = 5;
		RMSE_FORMAT = 12;
		
		P = 18; %CET: Computational Efficiency Trick
		P_TAG = 'P';
		P_CATEGORY = 3;
		P_FORMAT = 11;
		
		PERM_SEEDS = 19; %CET: Computational Efficiency Trick
		PERM_SEEDS_TAG = 'PERM_SEEDS';
		PERM_SEEDS_CATEGORY = 5;
		PERM_SEEDS_FORMAT = 12;
		
		FEATURE_IMPORTANCE = 20; %CET: Computational Efficiency Trick
		FEATURE_IMPORTANCE_TAG = 'FEATURE_IMPORTANCE';
		FEATURE_IMPORTANCE_CATEGORY = 5;
		FEATURE_IMPORTANCE_FORMAT = 16;
		
		PFSP = 21; %CET: Computational Efficiency Trick
		PFSP_TAG = 'PFSP';
		PFSP_CATEGORY = 9;
		PFSP_FORMAT = 8;
	end
	...
	methods (Static) % inspection
		...
		function prop_list = getProps(category)
			...
			switch category
				...
				case 5 % Category.RESULT
					prop_list = [13 14 15 16 17 19 20];
		...
		function check_out = existsTag(tag)
			...
			check = any(strcmp(tag, { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'NN'  'D'  'BA'  'GROUND_TRUTH'  'CORR'  'DET'  'MAE'  'MSE'  'RMSE'  'P'  'PERM_SEEDS'  'FEATURE_IMPORTANCE'  'PFSP' })); %CET: Computational Efficiency Trick
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				prop = find(strcmp(pointer, { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'NN'  'D'  'BA'  'GROUND_TRUTH'  'CORR'  'DET'  'MAE'  'MSE'  'RMSE'  'P'  'PERM_SEEDS'  'FEATURE_IMPORTANCE'  'PFSP' })); % tag = pointer %CET: Computational Efficiency Trick
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				nnregressormlp_evaluator_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'NN'  'D'  'BA'  'GROUND_TRUTH'  'CORR'  'DET'  'MAE'  'MSE'  'RMSE'  'P'  'PERM_SEEDS'  'FEATURE_IMPORTANCE'  'PFSP' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			nnregressormlp_evaluator_category_list = { 1  1  1  3  4  2  2  6  4  4  4  6  5  5  5  5  5  3  5  5  9 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			nnregressormlp_evaluator_format_list = { 2  2  2  8  2  2  2  2  8  8  8  14  12  12  12  12  12  11  12  16  8 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 11 % NNRegressorMLP_Evaluator.BA
					prop_settings = 'BrainAtlas';
				case 12 % NNRegressorMLP_Evaluator.GROUND_TRUTH
					prop_settings = Format.getFormatSettings(14);
				case 13 % NNRegressorMLP_Evaluator.CORR
					prop_settings = Format.getFormatSettings(12);
				case 14 % NNRegressorMLP_Evaluator.DET
					prop_settings = Format.getFormatSettings(12);
				case 15 % NNRegressorMLP_Evaluator.MAE
					prop_settings = Format.getFormatSettings(12);
				case 16 % NNRegressorMLP_Evaluator.MSE
					prop_settings = Format.getFormatSettings(12);
				case 17 % NNRegressorMLP_Evaluator.RMSE
					prop_settings = Format.getFormatSettings(12);
				case 18 % NNRegressorMLP_Evaluator.P
					prop_settings = Format.getFormatSettings(11);
				case 19 % NNRegressorMLP_Evaluator.PERM_SEEDS
					prop_settings = Format.getFormatSettings(12);
				case 20 % NNRegressorMLP_Evaluator.FEATURE_IMPORTANCE
					prop_settings = Format.getFormatSettings(16);
				case 21 % NNRegressorMLP_Evaluator.PFSP
			...
			switch prop
				case 20 % NNRegressorMLP_Evaluator.FEATURE_IMPORTANCE
					...
					if any(strcmp(dp_class, graph_dp_classes)) % GRAPH input
					    pr = NNxMLP_xPP_FI_Graph('EL', nne, 'D', input_dataset, 'PROP', 20, varargin{:});
					elseif any(strcmp(dp_class, measure_dp_classes))% MEASURE input
					    pr = NNxMLP_xPP_FI_Measure('EL', nne, 'D', input_dataset, 'PROP', 20, varargin{:});
					else % DATA input
					    pr = NNxMLP_xPP_FI_Data('EL', nne, 'D', input_dataset, 'PROP', 20, varargin{:});
					end
					
				case 21 % NNRegressorMLP_Evaluator.PFSP
					pr = PanelPropItem('EL', nne, 'PROP', 21, ...
					    'GUICLASS', 'GUIFig', ...
