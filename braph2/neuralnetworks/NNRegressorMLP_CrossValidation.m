classdef NNRegressorMLP_CrossValidation < NNCrossValidation
	...
	%  <strong>30</strong> <strong>BA</strong> 	BA (data, item) is a brain atlas.
	%  <strong>31</strong> <strong>P</strong> 	P (parameter, scalar) is the permutation number.
	%  <strong>32</strong> <strong>AV_CORR</strong> 	AV_CORR (result, rvector) provides the metric of the correlation of coefficients.
	%  <strong>33</strong> <strong>AV_DET</strong> 	AV_DET (result, rvector) provides the coefficient of determination, a measure showing how well the predictions are replicated by the model.
	%  <strong>34</strong> <strong>AV_MAE</strong> 	AV_MAE (result, rvector) provides the metric of the mean absolute error.
	%  <strong>35</strong> <strong>AV_MSE</strong> 	AV_MSE (result, rvector) provides the metric of the mean squared error.
	%  <strong>36</strong> <strong>AV_RMSE</strong> 	AV_RMSE (result, rvector) provides the metric of the root mean squared error.
	%  <strong>37</strong> <strong>AV_FEATURE_IMPORTANCE</strong> 	AV_FEATURE_IMPORTANCE (result, cell) averages the feature importances across k folds.
	%  <strong>38</strong> <strong>PFSP</strong> 	PFSP (gui, item) contains the panel figure of the scatter plot for regression model.
	%
	...
	properties (Constant) % properties
		BA = 30; %CET: Computational Efficiency Trick
		BA_TAG = 'BA';
		BA_CATEGORY = 4;
		BA_FORMAT = 8;
		...
		AV_CORR = 32; %CET: Computational Efficiency Trick
		AV_CORR_TAG = 'AV_CORR';
		AV_CORR_CATEGORY = 5;
		AV_CORR_FORMAT = 12;
		
		AV_DET = 33; %CET: Computational Efficiency Trick
		AV_DET_TAG = 'AV_DET';
		AV_DET_CATEGORY = 5;
		AV_DET_FORMAT = 12;
		
		AV_MAE = 34; %CET: Computational Efficiency Trick
		AV_MAE_TAG = 'AV_MAE';
		AV_MAE_CATEGORY = 5;
		AV_MAE_FORMAT = 12;
		
		AV_MSE = 35; %CET: Computational Efficiency Trick
		AV_MSE_TAG = 'AV_MSE';
		AV_MSE_CATEGORY = 5;
		AV_MSE_FORMAT = 12;
		
		AV_RMSE = 36; %CET: Computational Efficiency Trick
		AV_RMSE_TAG = 'AV_RMSE';
		AV_RMSE_CATEGORY = 5;
		AV_RMSE_FORMAT = 12;
		
		AV_FEATURE_IMPORTANCE = 37; %CET: Computational Efficiency Trick
		AV_FEATURE_IMPORTANCE_TAG = 'AV_FEATURE_IMPORTANCE';
		AV_FEATURE_IMPORTANCE_CATEGORY = 5;
		AV_FEATURE_IMPORTANCE_FORMAT = 16;
		
		PFSP = 38; %CET: Computational Efficiency Trick
		PFSP_TAG = 'PFSP';
		PFSP_CATEGORY = 9;
		PFSP_FORMAT = 8;
	end
	...
	methods (Static) % inspection
		...
		function prop_list = getProps(category)
			...
			switch category
				...
				case 3 % Category.PARAMETER
					prop_list = [4 13 14 23 24 25 26 31];
				case 4 % Category.DATA
					prop_list = [5 10 11 12 30];
				case 5 % Category.RESULT
					prop_list = [15 16 17 19 21 32 33 34 35 36 37];
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			nnregressormlp_crossvalidation_category_list = { 1  1  1  3  4  2  2  6  9  4  4  4  3  3  5  5  5  6  5  6  5  6  3  3  3  3  2  2  6  4  3  5  5  5  5  5  5  9 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			nnregressormlp_crossvalidation_format_list = { 2  2  2  8  2  2  2  2  4  11  16  9  8  8  9  9  9  8  9  8  9  8  11  11  5  5  4  5  1  8  11  12  12  12  12  12  16  8 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 30 % NNRegressorMLP_CrossValidation.BA
					prop_settings = 'BrainAtlas';
				case 31 % NNRegressorMLP_CrossValidation.P
					prop_settings = Format.getFormatSettings(11);
				case 32 % NNRegressorMLP_CrossValidation.AV_CORR
					prop_settings = Format.getFormatSettings(12);
				case 33 % NNRegressorMLP_CrossValidation.AV_DET
					prop_settings = Format.getFormatSettings(12);
				case 34 % NNRegressorMLP_CrossValidation.AV_MAE
					prop_settings = Format.getFormatSettings(12);
				case 35 % NNRegressorMLP_CrossValidation.AV_MSE
					prop_settings = Format.getFormatSettings(12);
				case 36 % NNRegressorMLP_CrossValidation.AV_RMSE
					prop_settings = Format.getFormatSettings(12);
				case 37 % NNRegressorMLP_CrossValidation.AV_FEATURE_IMPORTANCE
					prop_settings = Format.getFormatSettings(16);
				case 38 % NNRegressorMLP_CrossValidation.PFSP
			...
			switch prop
				case 37 % NNRegressorMLP_CrossValidation.AV_FEATURE_IMPORTANCE
					...
					if any(strcmp(dp_class, graph_dp_classes)) % GRAPH input
					    pr = NNxMLP_xPP_FI_Graph('EL', nncv, 'D', input_dataset, 'PROP', 37, varargin{:});
					elseif any(strcmp(dp_class, measure_dp_classes))% MEASURE input
					    pr = NNxMLP_xPP_FI_Measure('EL', nncv, 'D', input_dataset, 'PROP', 37, varargin{:});
					else % DATA input
					    pr = NNxMLP_xPP_FI_Data('EL', nncv, 'D', input_dataset, 'PROP', 37, varargin{:});
					end
					
				case 38 % NNRegressorMLP_CrossValidation.PFSP
					pr = PanelPropItem('EL', nncv, 'PROP', 38, ...
					    'GUICLASS', 'GUIFig', ...
