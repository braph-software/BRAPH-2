function create_data_FUN_MP_XLS(data_dir, random_seed)
%CREATE_DATA_FUN_MP_XLS creates functional data
%
% CREATE_DATA_FUN_MP_XLS() creates functional data in default folder 'Example
%  data ST XLS'.
%
% CREATE_DATA_FUN_MP_XLS(DATA_DIR) creates functional data in DATA_DIR folder.
%
% CREATE_DATA_FUN_MP_XLS(DATA_DIR, RANDOM_SEED) cretes functional data in DATA_DIR
%  folder with a specified RANDOM_SEED.
%
% See also create_data_st_mp_txt.

if nargin < 1
    data_dir = [fileparts(which('SubjectFUN_MP')) filesep 'Example data FUN_MP XLS'];
end
...
if ~isfolder(data_dir)
    ...
    for i = 1:1:10 % subject number
        ...
        writetable(array2table(R11), [gr1_dir filesep() sub_id '.1.xlsx'], 'WriteVariableNames', false)
        writetable(array2table(R12), [gr1_dir filesep() sub_id '.2.xlsx'], 'WriteVariableNames', false)
        writetable(array2table(R13), [gr1_dir filesep() sub_id '.3.xlsx'], 'WriteVariableNames', false)
    ...
    for i = 51:1:60
        ...
        writetable(array2table(R21), [gr2_dir filesep() sub_id '.1.xlsx'], 'WriteVariableNames', false)
        writetable(array2table(R22), [gr2_dir filesep() sub_id '.2.xlsx'], 'WriteVariableNames', false)
        writetable(array2table(R23), [gr2_dir filesep() sub_id '.3.xlsx'], 'WriteVariableNames', false)
