function create_data_NN_CLA_CON_TXT(data_dir, random_seed)
%CREATE_DATA_NN_CLA_CON_TXT creates connectivity data for neural network classification analysis
%
% CREATE_DATA_NN_CLA_CON_TXT() creates connectivity data in default folder 'Example
%  data NN CLA CON TXT'.
%
% CREATE_DATA_NN_CLA_CON_TXT(DATA_DIR) creates connectivity data in DATA_DIR folder.
%
% CREATE_DATA_NN_CLA_CON_TXT(DATA_DIR, RANDOM_SEED) cretes connectivity data in
%  DATA_DIR folder with a specified RANDOM_SEED.
%
% See also create_data_nn_cla_con_txt.

if nargin < 1
    data_dir = [fileparts(which('NNDataPoint_CON_CLA')) filesep 'Example data NN CLA CON TXT'];
end
...
if ~isdir(data_dir)
    ...
    for i = 11:1:20
        ...
        writetable(array2table(A2), [gr2_dir filesep() sub_id '.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
    ...
    for i = 21:1:30
        ...
        writetable(array2table(A3), [gr3_dir filesep() sub_id '.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
