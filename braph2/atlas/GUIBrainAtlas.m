...
% TODO - check build if less than 2020 send error, this is not a correct atlas.
        % select file
...
ui_edit_table_atlasid = uicontrol(ui_panel_table, 'Style', 'edit');
...
init_table()
    function init_table()
        ...
        set(ui_edit_table_atlasid, 'Position', [.02 .95 .96 .04])
        set(ui_edit_table_atlasid, 'HorizontalAlignment', 'left')
        set(ui_edit_table_atlasid, 'FontWeight', 'bold')
        set(ui_edit_table_atlasid, 'Callback', {@cb_table_atlasname})
    ...
    function update_table_atlasid()
        atlas_id = atlas.getID();
        if isempty(atlas_id)
            set(f, 'Name', [APPNAME ' - ' BRAPH2.BUILD])
        else
            set(f, 'Name', [APPNAME ' - ' BRAPH2.BUILD ' - ' atlas_id])
        end
        set(ui_edit_table_atlasid, 'String', atlas_id)
    end
    ...
    function cb_table_atlasname(~, ~)  % (src, event)
        atlas.setID(get(ui_edit_table_atlasid,  'String'));
        update_table_atlasid()
    end
...
init_contextmenu()
    ...
    function create_figure(brain_surf_file)
        if nargin < 1
            brain_surf_file = 'BrainMesh_ICBM152';
        end
