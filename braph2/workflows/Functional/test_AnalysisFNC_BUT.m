...
graph_type = AnalysisFNC_BUT.getGraphType();
measures = Graph.getCompatibleMeasureList(graph_type);

%% Test 1: Instantiation
analysis = AnalysisFNC_BUT('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'threshold', 0.7); %#ok<NASGU>

%% Test 2: Create correct ID
analysis = AnalysisFNC_BUT('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'threshold', 0.7);
...
assert(ischar(measurement_id), ...
    [BRAPH2.STR ':AnalysisFNC_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisFNC_BUT.getMeasurementID() not creating an ID')
assert(isequal(measurement_id, expected_value), ...
    [BRAPH2.STR ':AnalysisFNC_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisFNC_BUT.getMeasurementID() not creating correct ID')

randomcomparison_id = analysis.getRandomComparisonID('PathLength', group1, 'threshold', 0.7);
expected_value = [ ...
...
assert(ischar(randomcomparison_id), ...
    [BRAPH2.STR ':AnalysisFNC_BUT:getRandomComparisonID'], ...
    ['AnalysisFNC_BUT.getRandomComparisonID() not creating an ID']) %#ok<*NBRAK>
assert(isequal(randomcomparison_id, expected_value), ...
    [BRAPH2.STR ':AnalysisFNC_BUT:getRandomComparisonID'], ...
    ['AnalysisFNC_BUT.getRandomComparisonID() not creating correct ID']) %#ok<*NBRAK>
...
assert(ischar(comparison_id), ...
    [BRAPH2.STR ':AnalysisFNC_BUT:getComparisonID'], ...
    ['AnalysisFNC_BUT.getComparisonID() not creating an ID']) %#ok<*NBRAK>
assert(isequal(comparison_id, expected_value), ...
    [BRAPH2.STR ':AnalysisFNC_BUT:getComparisonID'], ...
    ['AnalysisFNC_BUT.getComparisonID() not creating correct ID']) %#ok<*NBRAK>
...
for i = 1:1:length(measures)
    ...
    assert(~isempty(calculated_measurement), ...
        [BRAPH2.STR ':AnalysisFNC_BUT:' BRAPH2.BUG_FUNC], ...
        'AnalysisFNC_BUT.getMeasurement() not working')
    ...
    for j = 1:1:numel(measurement_keys)
        ...
        if Measure.is_global(measure)
            ...
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisFNC_BUT:calculateMeasurement'], ...
                ['AnalysisFNC_BUT.calculateMeasurement() not working for global']) %#ok<*NBRAK>
            assert(iscell(calculated_value) & ...
                ...
                [BRAPH2.STR ':AnalysisFNC_BUT:Instantiation'], ...
                ['AnalysisFNC_BUT does not initialize correctly with global measures.']) %#ok<*NBRAK>
            
        elseif Measure.is_nodal(measure)
            
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisFNC_BUT:calculateMeasurement'], ...
                ['AnalysisFNC_BUT.calculateMeasurement() not working for nodal']) %#ok<*NBRAK>
            assert(iscell(calculated_value) & ...
                isequal(numel(calculated_value), group1.subjectnumber) & ...
                all(cellfun(@(x) isequal(size(x), [atlas.getBrainRegions().length(), 1]), calculated_value)), ...
                [BRAPH2.STR ':AnalysisFNC_BUT:Instantiation'], ...
                ['AnalysisFNC_BUT does not initialize correctly with nodal measures.']) %#ok<*NBRAK>
        
        elseif Measure.is_binodal(measure)
            
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisFNC_BUT:calculateMeasurement'], ...
                ['AnalysisFNC_BUT.calculateMeasurement() not working for binodal']) %#ok<*NBRAK>
            assert(iscell(calculated_value) & ...
                isequal(numel(calculated_value), group1.subjectnumber) & ...
                all(cellfun(@(x) isequal(size(x), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]), calculated_value)), ...
                [BRAPH2.STR ':AnalysisFNC_BUT:Instantiation'], ...
                ['AnalysisFNC_BUT does not initialize correctly with binodal measures.']) %#ok<*NBRAK>
        end
    end
end
...
for i = 1:1:numel(measures)
    measure = measures{i};
    analysis = AnalysisFNC_BUT('id', 'label', 'notes', cohort, {}, {}, {}, 'threshold', 0.7);
    ...
    assert(~isempty(calculate_comparison), ...
        [BRAPH2.STR ':AnalysisFNC_BUT:calculateComparison'], ...
        ['AnalysisFNC_BUT.calculateComparison() not working']) %#ok<*NBRAK>
    
    assert(analysis.getRandomComparisons().length() == 1, ...
        [BRAPH2.STR ':AnalysisFNC_BUT:calculateComparison'], ...
        ['AnalysisFNC_BUT.calculateComparison() not working'])
    ...
    if Measure.is_global(measures{i})
        ...
        assert(iscell(random_comparison_difference) && ...
            ...
            [BRAPH2.STR ':AnalysisFNC_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisFNC_BUT.getComparison() not working with global measures') 
    ...
    elseif Measure.is_nodal(measures{i})
        ...
        assert(iscell(random_comparison_difference) && ...
            ...
            [BRAPH2.STR ':AnalysisFNC_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisFNC_BUT.getComparison() not working with nodal measures') 
    ...
    elseif Measure.is_binodal(measures{i})
       assert(iscell(random_comparison_values_1) & ...
        ...
        assert(iscell(random_comparison_difference) && ...
            ...
            [BRAPH2.STR ':AnalysisFNC_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisFNC_BUT.getComparison() not working with binodal measures') 
