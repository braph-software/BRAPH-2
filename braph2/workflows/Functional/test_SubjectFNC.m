...
sub = SubjectFNC('id', 'label', 'notes', atlas);
...
% act
SubjectFNC.save_to_xls(cohort, save_dir_rule, save_dir_path);

load_cohort = SubjectFNC.load_from_xls(atlas, sub_class, 'Directory', save_dir_path);
...
% act
SubjectFNC.save_to_txt(cohort, save_dir_rule, save_dir_path);

load_cohort = SubjectFNC.load_from_txt(atlas, sub_class, 'Directory', [save_dir_path filesep() group.getID()]);

% assert
assert(isequal(cohort.getSubjects().length(), load_cohort.getSubjects().length()), ...
    'BRAPH:SubjectFNC:SaveLoadTXT', ...
    'Problems saving or loading a cohort.')
assert(isequal(cohort.getGroups().length(), load_cohort.getGroups().length()), ...
    'BRAPH:SubjectFNC:SaveLoadTXT', ...
    'Problems saving or loading a cohort.')
...
% act
SubjectFNC.save_to_json(cohort, save_dir_rule, save_dir_path);

load_cohort = SubjectFNC.load_from_json(atlas, sub_class, 'Directory', [save_dir_path filesep() group.getID()]);
