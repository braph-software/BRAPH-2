...
analysis = AnalysisCON_BUD('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'density', 0.7); %#ok<NASGU>

%% Test 2: Create correct ID
analysis = AnalysisCON_BUD('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'density', 0.7);
...
assert(ischar(measurement_id), ...
    [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisCON_BUD.getMeasurementID() not creating an ID')
assert(isequal(measurement_id, expected_value), ...
    [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisCON_BUD.getMeasurementID() not creating correct ID')
...
assert(ischar(comparison_id), ...
    ['BRAPH:AnalysisCON_BUD:getComparisonID'], ...
    ['AnalysisCON_BUD.getComparisonID() not creating an ID']) %#ok<*NBRAK>
assert(isequal(comparison_id, expected_value), ...
    ['BRAPH:AnalysisCON_BUD:getComparisonID'], ...
    ['AnalysisCON_BUD.getComparisonID() not creating correct ID']) %#ok<*NBRAK>

randomcomparison_id = analysis.getRandomComparisonID('PathLength', group1, 'density', 0.7);
expected_value = [ ...
...
assert(ischar(randomcomparison_id), ...
    ['BRAPH:AnalysisCON_BUD:getRandomComparisonID'], ...
    ['AnalysisCON_BUD.getRandomComparisonID() not creating an ID']) %#ok<*NBRAK>
assert(isequal(randomcomparison_id, expected_value), ...
    ['BRAPH:AnalysisCON_BUD:getRandomComparisonID'], ...
    ['AnalysisCON_BUD.getRandomComparisonID() not creating correct ID']) %#ok<*NBRAK>
...
for i = 1:1:length(measures)
    ...
    assert(~isempty(calculated_measurement), ...
        ...
        ['AnalysisCON_BUD.getMeasurement() not working']) %#ok<*NBRAK>
    ...
    for j = 1:1:numel(measurement_keys)
        ...
        if Measure.is_global(measure)
            ...
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisCON_BUD:calculateMeasurement'], ...
                ['AnalysisCON_BUD.calculateMeasurement() not working for global']) %#ok<*NBRAK>
            assert(iscell(calculated_value) & ...
                ...
                [BRAPH2.STR ':AnalysisCON_BUD:Instantiation'], ...
                ['AnalysisCON_BUD does not initialize correctly with global measures.']) %#ok<*NBRAK>
        ...
        elseif Measure.is_nodal(measure)
            ...
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisCON_BUD:calculateMeasurement'], ...
                ['AnalysisCON_BUD.calculateMeasurement() not working for nodal']) %#ok<*NBRAK>
            assert(iscell(calculated_value) & ...
                isequal(numel(calculated_value), group1.subjectnumber) & ...
                all(cellfun(@(x) isequal(size(x), [atlas.getBrainRegions().length(), 1]), calculated_value)), ...
                [BRAPH2.STR ':AnalysisCON_BUD:Instantiation'], ...
                ['AnalysisCON_BUD does not initialize correctly with nodal measures.']) %#ok<*NBRAK>
        ...
        elseif Measure.is_binodal(measure)
            ...
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisCON_BUD:calculateMeasurement'], ...
                ['AnalysisCON_BUD.calculateMeasurement() not working for binodal']) %#ok<*NBRAK>
            assert(iscell(calculated_value) & ...
                isequal(numel(calculated_value), group1.subjectnumber) & ...
...
measure = measures{i};
    analysis = AnalysisCON_BUD('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {});
    group = group1;
    ...
    assert(~isempty(calculated_randomcomparison), ...
        [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.BUG_FUNC], ...
        'AnalysisCON_BUD.getComparison() not working')
    
    assert(analysis.getRandomComparisons().length() == 1, ...
        [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.BUG_FUNC], ...
        'AnalysisCON_BUD.getRandomComparisons() not working')
    ...
    if Measure.is_global(measures{i})
        ...
        assert(iscell(randomComparison_values_group) && ...
            ...
            [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisCON_BUD.getComparison() not working with global measures') 
    ...
    elseif Measure.is_nodal(measures{i})
        ...
        assert(iscell(randomComparison_values_group) && ...
            ...
            [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisCON_BUD.getComparison() not working with nodal measures') 
    ...
    elseif Measure.is_binodal(measures{i})
        ...
        assert(iscell(randomComparison_values_group) && ...
            ...
            [BRAPH2.STR ':AnalysisCON_BUD:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisCON_BUD.getComparison() not working with binodal measures') 
