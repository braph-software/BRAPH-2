...
for i = 1:1:numel(measures)
    ...
    if Measure.is_global(measures{i})
        ...
        assert(iscell(value_2) && ...
         ...
         assert(isequal(numel(difference{1}), 1) && ...
            isequal(numel(all_differences), number_of_permutations) , ...
            [BRAPH2.STR ':ComparisonMRI:' BRAPH2.BUG_FUNC], ...
            'ComparisonMRI does not initialize correctly with global measures')
        
        assert(isequal(numel(p1{1}), 1) && ...
            isequal(numel(p2{1}), 1) && ...
            isequal(numel(confidence_interval_min{1}), 1) && ...
            isequal(numel(confidence_interval_max{1}), 1), ...
            [BRAPH2.STR ':ComparisonMRI:' BRAPH2.BUG_FUNC], ...
            'ComparisonMRI does not initialize correctly with global measures')
    ...
    elseif Measure.is_nodal(measures{i})
        ...
        assert(isequal(size(difference{1}), [atlas.getBrainRegions().length(), 1]) && ...
            isequal(size(all_differences), [1, number_of_permutations]) , ...
            [BRAPH2.STR ':ComparisonMRI:' BRAPH2.BUG_FUNC], ...
            'ComparisonMRI does not initialize correctly with nodal measures')
        
        assert(isequal(size(p1{1}), [atlas.getBrainRegions().length(), 1]) && ...
            isequal(size(p2{1}), [atlas.getBrainRegions().length(), 1]) && ...
            isequal(size(confidence_interval_min{1}), [atlas.getBrainRegions().length(), 1]) && ...
            isequal(size(confidence_interval_max{1}), [atlas.getBrainRegions().length(), 1]), ...
            [BRAPH2.STR ':ComparisonMRI:' BRAPH2.BUG_FUNC], ...
            'ComparisonMRI does not initialize correctly with nodal measures')
    ...
    elseif Measure.is_binodal(measures{i})
        ...
        assert(isequal(size(difference{1}), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]) && ...
            isequal(size(all_differences), [1, number_of_permutations]) , ...
            [BRAPH2.STR ':ComparisonMRI:' BRAPH2.BUG_FUNC], ...
            'ComparisonMRI does not initialize correctly with binodal measures')
        
        assert(isequal(size(p1{1}), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]) && ...
            isequal(size(p2{1}), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]) && ...
            isequal(size(confidence_interval_min{1}), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]) && ...
            isequal(size(confidence_interval_max{1}), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()]), ...
            [BRAPH2.STR ':ComparisonMRI:' BRAPH2.BUG_FUNC], ...
            'ComparisonMRI does not initialize correctly with binodal measures')
    end
end
...
for i = 1:1:numel(measures)
    ...
    % get inner value of the cells
    difference = comparison_difference{1};
    p1 = comparison_p1{1};
    p2 = comparison_p2{1};
    confidence_interval_min = comparison_confidence_interval_min{1};
    confidence_interval_max = comparison_confidence_interval_max{1};
