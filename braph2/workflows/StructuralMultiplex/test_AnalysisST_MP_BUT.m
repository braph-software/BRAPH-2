...
cohort = Cohort('Cohort ST MP WU', 'cohort label', 'cohort notes', 'SubjectST_MP', atlas, {sub11, sub12, sub13, sub14, sub21, sub22, sub23});
...
analysis = AnalysisST_MP_BUT('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'threshold1', 0.7, 'threshold2', 0.5); %#ok<NASGU>

%% Test 2: Create correct ID
analysis = AnalysisST_MP_BUT('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'threshold1', 0.7, 'threshold2', 0.7);

measurement_id = analysis.getMeasurementID('Degree', group1, 'threshold1', 0.7, 'threshold2', 0.7);
expected_value = [ ...
    tostring(analysis.getMeasurementClass()) ' ' ...
    tostring('Degree') ' ' ...
    tostring(analysis.getCohort().getGroups().getIndex(group1)) ' ' ...
    ['threshold1=' num2str(0.7)], ...
    [' threshold2=' num2str(0.7)] ...
    ];
assert(ischar(measurement_id), ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisST_MP_BUT.getMeasurementID() not creating an ID')
assert(isequal(measurement_id, expected_value), ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisST_MP_BUT.getMeasurementID() not creating correct ID')

randomcomparison_id = analysis.getRandomComparisonID('Degree', group1, 'threshold1', 0.7, 'threshold2', 0.7);
expected_value = [ ...
...
assert(ischar(randomcomparison_id), ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisST_MP_BUT.getRandomComparisonID() not creating an ID')
assert(isequal(randomcomparison_id, expected_value), ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisST_MP_BUT.getRandomComparisonID() not creating correct ID')

comparison_id = analysis.getComparisonID('Distance', group1, group2, 'threshold1', 0.7, 'threshold2', 0.7);
expected_value = [ ...
...
assert(ischar(comparison_id), ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisST_MP_BUT.getComparisonID() not creating an ID')
assert(isequal(comparison_id, expected_value), ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
    'AnalysisST_MP_BUT.getComparisonID() not creating correct ID')

%% Test 3: Measurement
for i = 1:1:length(measures)
    measure = measures{i};
    % I can do it with settings or just passing to the measurement.
    analysis = AnalysisST_MP_BUT('analysis id', 'analysis label', 'analysis notes', cohort, {}, {}, {}, 'threshold1', .6, 'threshold2', .5);
    calculated_measurement = analysis.getMeasurement(measure, group1, 'threshold1', .6, 'threshold2', .5);
    
    assert(~isempty(calculated_measurement), ...
        [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
        'AnalysisST_MP_BUT.getMeasurement() not working')
    ...
    for j = 1:1:numel(measurement_keys)
        ...
        if Measure.is_global(measure)
            ...
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
                'AnalysisST_MP_BUT.calculateMeasurement() not working for global')
            assert(iscell(calculated_value) && ...
                ...
                [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
                'AnalysisST_MP_BUT does not initialize correctly with global measures')
            
        elseif Measure.is_nodal(measure)
            
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisST_MP_BUT:calculateMeasurement'], ...
                'AnalysisST_MP_BUT.calculateMeasurement() not working for nodal')
            assert(iscell(calculated_value) && ...
                isequal(numel(calculated_value), num_elements) && ...
                ...
                'AnalysisST_MP_BUT does not initialize correctly with nodal measures')
        
        elseif Measure.is_binodal(measure)
            
            assert(isequal(calculated_measurement.getMeasureCode(), measure), ...
                [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
                'AnalysisST_MP_BUT.calculateMeasurement() not working for binodal')
            assert(iscell(calculated_value) && ...
                isequal(numel(calculated_value), num_elements) && ...
                all(cellfun(@(x) isequal(size(x, 1), atlas.getBrainRegions().length()), calculated_value)) && ...
                ...
                all(cellfun(@(x) isequal(size(x, 3), parameter_values_length), calculated_value)), ...
                [BRAPH2.STR ':AnalysisST_MP_BUD:' BRAPH2.BUG_FUNC], ...
                'AnalysisST_MP_BUT does not initialize correctly with binodal measures')
        end
    end
end
...
for i = 1:1:numel(measures)
    ...
    measurement_group = analysis.getMeasurement(measure, group1, 'threshold1', .6, 'threshold2', .5);
    ...
    calculated_comparison = analysis.getRandomComparison(measure, group1, 'RandomizationNumber', number_of_randomizations, 'RandomComparisonST_MP.ParameterValues', parameter_value_group, 'threshold1', .6, 'threshold2', .5);

    assert(~isempty(calculated_comparison), ...
        [BRAPH2.STR ':AnalysisST_MP_BUT:calculateComparison'], ...
        ['AnalysisST_MP_BUT.calculateComparison() not working']) %#ok<*NBRAK>
    
    assert(analysis.getRandomComparisons().length() == 1, ...
        [BRAPH2.STR ':AnalysisST_MP_BUT:calculateComparison'], ...
        ['AnalysisST_MP_BUT.calculateComparison() not working'])
    ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
    'AnalysisST_MP_BUT.getRandomComparison() does not initialize correctly the parameter of the measures')
    ...
    if Measure.is_global(measure)
        assert(iscell(randomcomparison_value_group) && ...
            ...
            [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisST_MP_BUT.getRandomComparison() not working with global measures') 
    ...
    elseif Measure.is_nodal(measure)
        assert(iscell(randomcomparison_value_group) && ...
            ...
            all(cellfun(@(x) isequal(size(x, 3), parameter_values_length), randomcomparison_value_group)), ...
            [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisST_MP_BUT.getRandomComparison() not working with nodal measures') 
    ...
    elseif Measure.is_binodal(measure)
        assert(iscell(randomcomparison_value_group) && ...
            ...
            all(cellfun(@(x) isequal(size(x, 3), parameter_values_length), randomcomparison_value_group)), ...
            [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisST_MP_BUT.getRandomComparison() not working with binodal measures') 
...
for i = 1:1:numel(measures)
    ...
    calculated_comparison = analysis.getComparison(measure, group1, group2, 'PermutationNumber', number_of_permutations, 'ComparisonST_MP.ParameterValues', parameter_value_group, 'threshold1', .6, 'threshold2', .5);
    
    assert(~isempty(calculated_comparison), ...
        [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
        'AnalysisST_MP_BUT.getComparison() not working')
    ...
    [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.BUG_FUNC], ...
    'AnalysisST_MP_BUT.getComparison() does not initialize correctly the parameter of the measures')
    ...
    if Measure.is_global(measure)
        assert(iscell(comparison_values_1) && ...
            ...
            [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisST_MP_BUT.getComparison() not working with global measures') 
    ...
    elseif Measure.is_nodal(measure)
        assert(iscell(comparison_values_1) && ...
            ...
            all(cellfun(@(x) isequal(size(x, 3), parameter_values_length), comparison_values_1)), ...
            [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisST_MP_BUT.getComparison() not working with nodal measures') 
    ...
    elseif Measure.is_binodal(measure)
        assert(iscell(comparison_values_1) && ...
            ...
            all(cellfun(@(x) isequal(size(x, 3), parameter_values_length), comparison_values_1)), ...
            [BRAPH2.STR ':AnalysisST_MP_BUT:' BRAPH2.WRONG_OUTPUT], ...
            'AnalysisST_MP_BUT.getComparison() not working with binodal measures') 
