        ...
        function initialize_data(m, varargin)
            ...
            if Measure.is_global(measure_code)  % global measure
                m.values = get_from_varargin( ...
                    repmat({zeros(1, 1)}, 1,  m.getGroup().subjectnumber()), ...
                    'MeasurementFNC_MP.values', ...
                    varargin{:});
                assert(iscell(m.getMeasureValues()) & ...
                    isequal(size(m.getMeasureValues()), [rows, m.getGroup().subjectnumber]) & ...
                ...
                m.average_value = get_from_varargin( ...
                    ...
                    varargin{:});
                assert(isequal(size(m.getGroupAverageValue()), [1, rows]), ...
                    [BRAPH2.STR ':MeasurementFNC_MP:' BRAPH2.WRONG_INPUT], ...
                    'Data not compatible with MeasurementFNC_MP')
                
            elseif Measure.is_nodal(measure_code)  % nodal measure
                m.values = get_from_varargin( ...
                    repmat({zeros(atlas.getBrainRegions().length(), rows, parameter_values_length)}, 1,  m.getGroup().subjectnumber()), ...
                    'MeasurementFNC_MP.values', ...
                    varargin{:});
                assert(iscell(m.getMeasureValues()) & ...
                    isequal(size(m.getMeasureValues()), [rows, m.getGroup().subjectnumber]) & ...
                ...
                m.average_value = get_from_varargin( ...
                    ...
                    'MeasurementFNC_MP.average_value', ...
                    varargin{:});
                assert(isequal(size(m.getGroupAverageValue()), [atlas.getBrainRegions().length(), rows]), ...
                    [BRAPH2.STR ':MeasurementFNC_MP:' BRAPH2.WRONG_INPUT], ...
                    'Data not compatible with MeasurementFNC_MP')
                
            elseif Measure.is_binodal(measure_code)  % binodal measure
                m.values = get_from_varargin( ...
                    repmat({zeros(atlas.getBrainRegions().length(), atlas.getBrainRegions().length(), parameter_values_length)}, 1,  m.getGroup().subjectnumber()), ...
                    'MeasurementFNC_MP.values', ...
                    varargin{:});
                assert(iscell(m.getMeasureValues()) & ...
                    isequal(size(m.getMeasureValues()), [rows, m.getGroup().subjectnumber]) & ...
                ...
                m.average_value = get_from_varargin( ...
                    ...
                    'MeasurementFNC_MP.average_value', ...
                    varargin{:});
                assert(isequal(size(m.getGroupAverageValue()), [atlas.getBrainRegions().length(), atlas.getBrainRegions().length()*rows]), ...
                    [BRAPH2.STR ':MeasurementFNC_MP:' BRAPH2.WRONG_INPUT], ...
                    'Data not compatible with MeasurementFNC_MP')
            end
        end
