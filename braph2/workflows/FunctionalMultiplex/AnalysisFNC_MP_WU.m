classdef AnalysisFNC_MP_WU < Analysis
    ...
    methods  % Plot panel functions
        function graph_panel = getGraphPanel(analysis, varargin)
            ...
            set(ui_matrix_subjects_popup, 'Position', [.70 .72 .28 .05])
            ...
            function cb_groups_popup(~, ~)
                ...
                set(ui_matrix_subjects_popup, 'Value', 1)
            ...
            function update_subjects()
                ...
                subjects_labels_inner = cellfun(@(x) x.getID(), subjects, 'UniformOutput', false);
                subject_labels = ['Group Average' subjects_labels_inner];
                set(ui_matrix_subjects_popup, 'String', subject_labels)
            end
            ...
            function update_matrix()
                ...
                if ~isempty(groups)
                    ...
                    if selected_subject == 1
                        graphs = analysis.get_graphs_for_group(group, varargin{:});
                        g_As = cellfun(@(x) x.getA(selected_layer), graphs, 'UniformOutput', false);  % a(layer, layer)
