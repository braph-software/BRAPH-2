...
assert(isequal(radius.getValue(), known_radius_subgraphs), ...
...
assert(isequal(radius.getValue(), known_radius_default), ...
    [BRAPH2.STR ':Radius:' BRAPH2.BUG_ERR], ...
    'Radius is not being calculated correctly for GraphBU.')
...
g = GraphWU(A);
radius = Radius(g, 'EccentricityRule', 'subgraphs').getValue();

distance = Distance(g).getValue();
[~, ~, ~, radius_bct, ~]= charpath(distance{1});

assert(isequal(round(cell2mat(radius), 3), round(radius_bct, 3)), ...
        [BRAPH2.STR ':Radius:' BRAPH2.BUG_ERR], ...
        'Radius is not being calculated correctly for BCT.')
