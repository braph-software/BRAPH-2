classdef MultilayerWD < Graph
	...
	%  <strong>26</strong> <strong>A</strong> 	A (result, cell) is the cell containing the within-layer weighted adjacency matrices of the multilayer weighted directed graph and the connections between layers.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the multilayer weighted directed graph.
			%
			% BUILD = MultilayerWD.GETBUILD() returns the build of 'MultilayerWD'.
			%
			% Alternative forms to call this method are:
			%  BUILD = G.GETBUILD() returns the build of the multilayer weighted directed graph G.
			%  BUILD = Element.GETBUILD(G) returns the build of 'G'.
			%  BUILD = Element.GETBUILD('MultilayerWD') returns the build of 'MultilayerWD'.
			%
			% Note that the Element.GETBUILD(G) and Element.GETBUILD('MultilayerWD')
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerwd_category_list = { 1  1  1  3  4  2  2  6  1  6  6  6  6  2  6  2  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  3  6 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerwd_format_list = { 2  2  2  8  2  2  2  2  11  15  15  15  11  12  12  3  3  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  16  5  5  11  11  16 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 36 % MultilayerWD.B
					prop_settings = Format.getFormatSettings(16);
				case 37 % MultilayerWD.SEMIPOSITIVIZE_RULE
					prop_settings = {'zero', 'absolute'};
				case 38 % MultilayerWD.STANDARDIZE_RULE
					prop_settings = {'threshold' 'range'};
				case 39 % MultilayerWD.ATTEMPTSPEREDGE
					prop_settings = Format.getFormatSettings(11);
				case 40 % MultilayerWD.NUMBEROFWEIGHTS
					prop_settings = Format.getFormatSettings(11);
				case 41 % MultilayerWD.RANDOMIZATION
					prop_settings = Format.getFormatSettings(16);
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 9 % MultilayerWD.GRAPH_TYPE
					prop_default = 6;
				case 32 % MultilayerWD.COMPATIBLE_MEASURES
