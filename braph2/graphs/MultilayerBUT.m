classdef MultilayerBUT < MultilayerWU
	...
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the multilayer undirected with fixed thresholds graph.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the multilayer undirected with fixed thresholds graph.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the multilayer binarized at different thresholds graph.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the multilayer binary  undirected with fixed thresholds graph.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the multilayer binary  undirected with fixed thresholds graph.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the multilayer binary undirected with fixed thresholds graph.
	...
	%  <strong>22</strong> <strong>A</strong> 	A (result, cell) is the cell array containing the binary supra-adjacency matrix of the multilayer binary undirected with fixed thresholds (BUT) graph.
	...
	%  <strong>26</strong> <strong>PARTITIONS</strong> 	PARTITIONS (result, rvector) returns the number of layers for each partition (threshold) of the graph.
	...
	%  <strong>38</strong> <strong>RANDOMIZATION</strong> 	RANDOMIZATION (query, cell) is the attempts to rewire each edge.
	%  <strong>39</strong> <strong>THRESHOLDS</strong> 	THRESHOLDS (parameter, rvector) is the vector of thresholds.
	%
	...
	properties (Constant) % properties
		THRESHOLDS = 39; %CET: Computational Efficiency Trick
	...
	methods (Static) % inspection
		...
		function prop_list = getProps(category)
			...
			switch category
				...
				case 3 % Category.PARAMETER
					prop_list = [4 20 21 33 34 35 36 37 39];
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerbut_category_list = { 1  1  1  3  4  2  2  6  1  6  6  6  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  3  3  6  3 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerbut_format_list = { 2  2  2  8  2  2  2  2  11  15  15  15  11  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  16  5  5  5  11  11  16  12 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 39 % MultilayerBUT.THRESHOLDS
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(g, prop, varargin)
			...
			switch prop
				...
				case 26 % MultilayerBUT.PARTITIONS
				...
				case 38 % MultilayerBUT.RANDOMIZATION
	...
	methods % GUI
		function pr = getPanelProp(g, prop, varargin)
			...
			switch prop
				case 39 % MultilayerBUT.THRESHOLDS
					pr = PanelPropRVectorSmart('EL', g, 'PROP', 39, 'MAX', 1, 'MIN', -1, varargin{:});
