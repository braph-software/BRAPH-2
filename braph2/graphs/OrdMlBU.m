classdef OrdMlBU < Graph
	...
	%
	% In an ordinal multilayer binary undirected graph (OrdMlBU), layers could have different number 
	...
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ordinal multilayer binary undirected graph.
	...
	%  <strong>26</strong> <strong>A</strong> 	A (result, cell) is the cell containing the within-layer binary adjacency matrices of the multilayer binary undirected graph and the ordinal connections between layers.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the ordinal multilayer binary undirected graph.
			%
			% BUILD = OrdMlBU.GETBUILD() returns the build of 'OrdMlBU'.
			%
			% Alternative forms to call this method are:
			%  BUILD = G.GETBUILD() returns the build of the ordinal multilayer binary undirected graph G.
			%  BUILD = Element.GETBUILD(G) returns the build of 'G'.
			%  BUILD = Element.GETBUILD('OrdMlBU') returns the build of 'OrdMlBU'.
			%
			% Note that the Element.GETBUILD(G) and Element.GETBUILD('OrdMlBU')
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			ordmlbu_category_list = { 1  1  1  3  4  2  2  6  1  6  6  6  6  2  6  2  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  6 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			ordmlbu_format_list = { 2  2  2  8  2  2  2  2  11  15  15  15  11  12  12  3  3  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  16  5  5  11  16 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 37 % OrdMlBU.SYMMETRIZE_RULE
					prop_settings = {'max', 'sum', 'average', 'min'};
				case 38 % OrdMlBU.SEMIPOSITIVIZE_RULE
					prop_settings = {'zero', 'absolute'};
				case 39 % OrdMlBU.ATTEMPTSPEREDGE
					prop_settings = Format.getFormatSettings(11);
				case 40 % OrdMlBU.RANDOMIZATION
					prop_settings = Format.getFormatSettings(16);
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 9 % OrdMlBU.GRAPH_TYPE
					prop_default = 5;
				case 32 % OrdMlBU.COMPATIBLE_MEASURES
