%%%%%%%%%%%%%%%% CALCULATION OF CORE-PERIPHERY - CONNECTIVITY SL %%%%%%%%%%%%%%%%
...
%% Pipeline Connectivity BUD Analysis
% Core-Periphery calculation
densities = 5:0.5:9; % the results figure show results at density 9%

analysis_women = AnalyzeEnsemble_CON_BUD( ...
    'GR', gr1_women_CON, 'DENSITIES', densities);
analysis_men = AnalyzeEnsemble_CON_BUD( ...
    'GR', gr2_men_CON, 'DENSITIES', densities);

coreperiphery_CON_women_ensemble_bud = analysis_women.get('MEASUREENSEMBLE', 'CorePeriphery').memorize('M');
coreperiphery_CON_men_ensemble_bud = analysis_men.get('MEASUREENSEMBLE', 'CorePeriphery').memorize('M');

%% Pipeline Connectivity BUD Comparison 
c_CON_BUD = CompareEnsemble( ...
    'P', 10000, ...
    'A1', analysis_women, ...
    'A2', analysis_men, ...
    'VERBOSE', true, ...
...
% CORE-PERIPHERY
core_CON_BUD_p1 = c_CON_BUD.get('COMPARISON', 'CorePeriphery').memorize('P1');
core_CON_BUD_p2 = c_CON_BUD.get('COMPARISON', 'CorePeriphery').memorize('P2');
core_CON_BUD_cil = c_CON_BUD.get('COMPARISON', 'CorePeriphery').memorize('CIL');
core_CON_BUD_ciu = c_CON_BUD.get('COMPARISON', 'CorePeriphery').memorize('CIU');
core_CON_BUD_diff = c_CON_BUD.get('COMPARISON', 'CorePeriphery').memorize('DIFF');

save('CON_coreperiphery.mat')
...
% % % 
% % % %% Save the pipeline
% % % Element.save(pipeline, 'pipeline_coreperiphery_CON_BUD_beta.b2', [])
