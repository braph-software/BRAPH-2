...
%%
load('MP_communities.mat')

%% create a pipeline
pipeline = Pipeline();
pipeline.set('LABEL', 'Pipeline Connectivity-Functional Multiplex Group Average WU');
pipeline.set('NOTES', 'Pipeline Connectivity-Functional Multiplex Group Average WU with the calculation of Multiplex-Community for BRAPH 2 publication');
...
pc6 = PipelineCode( ...
    ...
    'CODE', char("AnalyzeGroup_CON_FUN_MP_GA_WU('GR', gr1_women)"), ...
    'EL', analysis_g1 ...
    );
...
pc7 = PipelineCode( ...
    ...
    'CODE', char("AnalyzeGroup_CON_FUN_MP_GA_WU('GR', gr2_men)"), ...
    'EL', analysis_g2 ...
    );
...
tic
Element.save(pipeline, 'pipeline_Communities_MP.b2')
toc
fprintf('pipeline saved\n')

%% Load the pipeline
pipeline = Element.load('pipeline_Communities_MP.b2')
gui = GUIElement('PE', pipeline);
gui.get('DRAW');
gui.get('SHOW')