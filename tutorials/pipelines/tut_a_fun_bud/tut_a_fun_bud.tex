...
\begin{lstlisting}[
	...
	caption={
		...
		Command to generate the example data for functional analyses. They will be placed in the folder \fn{./braph2/pipelines/functional/Example data FUN XLS}, and include the brain atlas \fn{atlas.xlsx}, two folders with the subject files \fn{FUN\_Group\_1\_XLS} and \fn{FUN\_Group\_2\_XLS}, and the associated covariates files \fn{FUN\_Group\_1\_XLS.vois} and \fn{FUN\_Group\_2\_XLS.vois}. The details about the format of these files can be found in the tutorials \href{https://github.com/braph-software/BRAPH-2/tree/develop/tutorials/general/tut_ba}{Brain Atlas} and \href{https://github.com/braph-software/BRAPH-2/tree/develop/tutorials/general/tut_gr_fun}{Group of Subjects with Functional Data}.
	}
]
create_data_FUN_XLS()
\end{lstlisting}
...
Once the pipeline is uploaded, you can see a GUI that contains different steps to: upload a brain atlas, upload the functional data of two groups, analyze them, and finally, compare the groups (\Figref{fig:03}). 
...
\Figref{fig:04} shows how to upload and plot the brain atlas that you used to extract the \emph{functional data} for your analysis. For more information on where to find different atlases or how to change plotting settings on the brain surface, check the tutorial \href{https://github.com/braph-software/BRAPH-2/tree/develop/tutorials/general/tut_ba}{Brain Atlas}.
...
\fig{figure*}
	...
	{\bf b} Navigate to the BRAPH~2.0 folder \fn{atlases} and select one of the atlas files, in this example the \fn{aal90\_atlas.xlsx}. 
...
\fig{figure*}
	...
	{
	{\bf a} From the pipeline GUI, click on \fn{Load Group FUN 1 from XLS} to load the data of group 1.
...
\fig{figure*}
	...
	{\bf c} Adjust visualization settings by clicking on the \code{Settings Panel Figure} button in the brain surface toolbar.
	{\bf d} Explore different views such as the \code{Axial dorsal} button in the same toolbar.
...
Finally, when you right-click in the \code{Group-averaged MEASURES} panel, you will find other options to explore, such as \code{Plot Selected Measures} (which generates a line plot for the selected measure at different densities and/or different nodes) and \code{Data Selected Measure} (providing the calculated values of the selected measures). These options can also be saved for further analyses outside BRAPH 2.0.
...
\fig{figure*}
	...
	{\bf b} In this new window, you can see that the measure parameters, such as the {\bf b} \fn{GRAPH TEMPLATE} and the graph property \fn{densities}, as well as {\bf c} rule for \fn{Community Structure}, are blocked since they should be the same as the ones set in the analysis of group 1. You can reset the analysis parameters for Group 1 by clicking on the checkbox marked with a 'C' next to the settings of Group 1 {\bf a}.
	}
...
\fig{figure}
	{fig:11}
	{
	\includegraphics{fig11abc.jpg}
	\includegraphics{fig11d.jpg}
	}
	{Compare the groups}
	{
