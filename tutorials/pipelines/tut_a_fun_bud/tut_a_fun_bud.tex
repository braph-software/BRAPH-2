...
The general GUI of BRAPH~2 can be opened by typing \code{braph2} in MatLab's terminal. This GUI allows you to select a pipeline, in this case, \emph{Pipeline Functional Comparison BUD}, as shown in \Figref{fig:02}.
...
To open the GUI and upload the functional comparison pipeline, you can use the command line by typing the commands in \Coderef{cd:launch}.
...
\fig{figure*}
	...
	{\bf b} Navigate to the BRAPH~2 folder \fn{atlases} and select one of the atlas files, in this example the \fn{aal90\_atlas.xlsx}. 
...
Finally, when you right-click in the \code{Group-averaged MEASURES} panel, you will find other options to explore, such as \code{Plot Selected Measures} (which generates a line plot for the selected measure at different densities and/or different nodes) and \code{Data Selected Measure} (providing the calculated values of the selected measures). These options can also be saved for further analyses outside BRAPH~2.
...
\fig{figure*}
	...
	{\bf b} In this new window, you can see that the measure parameters, such as the {\bf b} \fn{GRAPH TEMPLATE} and the graph property \fn{densities}, as well as {\bf c} rule for \fn{Community Structure}, are blocked since they should be the same as the ones set in the analysis of group 1. You can reset the analysis parameters for Group 1 by clicking on the checkbox marked with a \code{D} next to the settings of Group 1 {\bf a}.
	}
