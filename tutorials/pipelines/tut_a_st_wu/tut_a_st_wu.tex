...
Finally, we can visualize the comparison results on a brain surface by selecting the measure comparisons we want and right click and press \fn{Plot Selected Comparisons on Brain} ({\Figref{fig:10}a}). A new window with comparison results on a brain surface will appear, where blue color indicates group 1 > group 2 and red color group 2 > group 1, and the size of the spheres is proportional to the absolute difference value between groups. If you press the Settings button ({\Figref{fig:10}b}), then you will have available more options, for example to apply FDR correction to your plot ({\Figref{fig:10}c}), which by default is set to off.
  	\fig{figure*}
	{fig:10}
	{
