...
\fig{figure*}
	...
	{\bf b} Navigate to the BRAPH~2.0 folder \fn{pipelines\_structural} and select the atlas file \fn{atlas.xlsx} that would be used in this example. You can also plot the brain atlas by pressing \fn{Plot Brain Atlas}. 
	}
...
After you have loaded the data for both groups, you can analyze the data of your first group by clicking on \fn{Analyze Group 1} (\Figref{fig:05}a-b). A new interface will be shown that allows you to pre-calculate network measures for each group and explore them. First of all, you can specify the parameters for constructing the graph from the structural group data (\Figref{fig:05}c): you can select the statistical test to calculate correlations in structural values between pairs of brain regions (\Figref{fig:05}d), and you can decide how you want to analyze the negative weights from the correlation (\Figref{fig:05}e). 
...
Importantly, the parameters you select for this analysis (graph and measures parameters) will be fixed for the analysis of the second group and for the group comparisons.
 ...
 Next, the measure window will appear, allowing you to choose the rules or parameters (\Figref{fig:06}b). In this instance, the rule applies to the calculation of triangles in directed graphs. However, since we're working with an undirected graph, we will keep the default option (\Figref{fig:06}b).
