...
In the \code{DENSITIES} section, you can define the densities by entering values like \code{1:1:2} (you can also use any other valid mathematical expression, such as \code{0 .25 .5 .75 1}, or \code{1:1:2}).
...
After configuring the parameters, you can proceed to calculate specific graph adjacency matricees (\Figref{fig:09}). To do this, return to the \fn{Analyze Ensemble} interface (\Figref{fig:09}a) and scroll down to locate the \fn{Individual GRAPHS} panel. By clicking the 'C' button, you will see a table displaying all subjects.
...
Right-click on the top of the table and select \code{Plot Selected Adjacency Matrices} within the Analyze Ensemble interface (\Figref{fig:09}). This will open a figure panel with the \fn{Adjacency Matrix} plotted on it.

Within the toolbar of the adjacency matrix plot, you can explore various views.
For instance, by clicking on the \code{Show axis} button (\Figref{fig:09}c), you will get the same axis as shown in \Figref{fig:09}d. Additionally, clicking on the \code{Settings Panel Figure} button (\Figref{fig:09}c) in the same toolbar allows you to adjust different visualization settings.

For instance, within the settings menu (\Figref{fig:09}e), you can turn on \code{tight}, resulting in the same kind of figure as shown in \Figref{fig:09}d. Within the settings menu, you can customize the visualization of the plots and save them for reference.

Finally, when you right-click in the \code{Individual GRAPHS} panel, you will find other options to explore, such as \code{Plot Selected Histograms} (which generates a histogram plot for the adjacnecy matrices at different densities and/or different nodes) and \code{Data Selected Graph ...} (providing the calculated values of individual graphs). These options can also be saved for further analyses outside BRAPH 2.0.

\section{Step 4: Analyzing the Data of Group 2 and Group 3}

After completing the analysis of the first group, you can analyze the second and third group by simply clicking on \code{Analyze Group 2} and \code{Analyze Group 3} (\Figref{fig:10}a). You will notice that in the new GUI (\Figref{fig:10}b-d), the parameters you previously selected for the first group are already preselected and fixed for this analysis. 
...
Once you have analyzed the data for three groups, you can begin training the neural networks by clicking on \fn{Cross-Validation} (\Figref{fig:11}a). 
...
\fig{figure*}
	...
	{\bf c} By clicking on the section \code{Template for Neural Network Classifier}, you open {\bf b} a new interface that permits you to configure the neural network classifier parameters.
	{\bf d} The available parameters shall be set at this point.
	{\bf e} By clicking on the section \code{Template for Neural Network Evaluator}, you open {\bf b} a new interface that permits you to configure the neural network evaluator parameters.
	}

\subsection{Setting Training Parameters}

In the \fn{Cross-Validation} interface (\Figref{fig:06}), you can configure the analysis parameters.

In the new window, you have several options to configure the cross-validation analysis. First, you can choose whether to enable the verbose functions and training-progress plots (\Figref{fig:11}b) while the analysis is running, which can help you monitor the progress of the analysis. 
In the \code{K FOLDS} section, you can define the densities by entering values of folds.

To configure the training parameters, you click on the section \code{Template for Neural Network Classifier} (\Figref{fig:11}c). This will open a new interface for neural networks template settings (\Figref{fig:11}d). 
...
To configure the evaluator parameters, you click on the section \code{Template for Neural Network Evaluator} (\Figref{fig:11}e). This will open a new interface for neural networks evaluator template settings. 
...
\fig{figure*}
	...
	{\bf g} To see the feature importance, locate the \code{Average of Feature Importance} panel and click the \code{C} button. 
	}
...
Locate the \code{Dataset List}, \code{Neural Network Classifier List}, and \code{Neural Network Evaluator List} panel and click those panels' \code{C} button to see all divided datasets, classifiers, and evaluators at all folds (\Figref{fig:12}a).
Next, to train the classifiers at all folds (\Figref{fig:12}b), locate the \code{Train All Neural Network Classifiers} panel and click the \code{C} button to train. 
After the training process is done, locate the \code{Confusion Matrix} panel and click the \code{C} button (\Figref{fig:12}c) to see how well the models are performing in terms of making correct and incorrect classification.

It is also possible to check the model performance with the Receiver Operating Characteristic (ROC) curve.
In multiclass classification, the concept of ROC (Receiver Operating Characteristic) curve can be extended to evaluate the model's ability to discriminate between multiple classes by treating each class as the positive class and the rest as the negative class, resulting in a set of ROC curves, one for each class.
To see the results of area under ROC curves for each class, locate the \code{Average of Area Under ROC Curve} and \code{Average of Macro Area Under ROC Curve} panel and click the \code{C} button (\Figref{fig:12}d).
To plot the ROC curves, click the \code{ROC Plot} button. This action will open a panel figure plotting all ROC curves from all folds (\Figref{fig:12}e).
...
Finally, To see the feature importance, locate the \code{Average of Feature Importance} panel and click the \code{C} button (\Figref{fig:12}g).
...
In (\Figref{fig:12}e), you can see the score of the feature importance. The values greater than 1 typically indicate that the feature contributes more to the model's predictive performance than an average feature, while values less than 1 suggest it has less impact.
