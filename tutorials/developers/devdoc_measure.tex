...
for li = 1:1:L        
	...
	if directionality_type == Graph.UNDIRECTED  % undirected graphs
		...
		directed_triangles_rule = m.get('RULE'); ¥\circled{1}\circlednote{1}{get the algorithm rule to calculte measure Triangle. Please observe in \circled{2} 		how we define the property rule.}¥
...
¤%% ¡props!
%%% ¡prop! 
RULE (parameter, option) is the rule to determine what is a triangle in directed networks. ¥\circled{2}\circlednote{2}{these are the available triangle rule options. Notice that the property name is canse sensitive. It is a parameter, which means it will be locked once the value is calculated. Finally, it is of format option, typically used to represent cell arrays that are case sensitive.}¥
%%%% ¡settings!
