...
Finally, BRAPH~2.0 is provided with a set of unit tests (executable by the command \fn{test\_braph2}) that ensure the formal correctness of the code, including that of any newly added elements.
...
To speed up the compilation of BRAPH~2.0 when trying these examples, it is possible to perform a light version of the compilation using the script \code{braph2genesis\_with\_exclusions}, which permits one to exclude entire folders or individual elements, as shown in \Coderef{cd:exclusions}.
%
\begin{lstlisting}[
	label=cd:exclusions,
	caption={
		{\bf BRAPH~2.0 genesis with exclusions.}
		Using \code{braph2genesis\_with\_exclusions}, it is possible to exclude some folders and elements, which are defined in the variable \code{excluded}.
		You can place your elements in the folder \fn{util}.
	}
]
¤...¤
excluded = { ...
...
This same approach (appropriately altering the excluded folders and elements) can also be used to reduce the compilation time when developing new functionalities.
...
The compiled BRAPH~2.0 is saved in \fn{brap2\_with\_exclusions}, which is ignored by GIT.
\end{tcolorbox}
\end{fullwidth}
