...
To speed up the compilation of BRAPH~2.0 when trying these examples, it is possible to perform a light version of the compilation using the script \code{braph2genesis\_with\_rollcall}, which permits one to exclude/include specific folders or elements, as shown in \Coderef{cd:rollcall}.
%
\begin{lstlisting}[
	label=cd:rollcall,
	caption={
...
The compiled BRAPH~2.0 is saved in \fn{brap2\_with\_rollcall}, which is ignored by GIT.
\end{tcolorbox}
\end{fullwidth}
