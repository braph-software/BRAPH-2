classdef Richness < Degree
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the richness.
	%
	...
	%  getDescription - returns the description of the richness.
	%  getName - returns the name of the richness.
	...
	%  tree - displays the element of Richness
	%
	% Richness method (element list):
	%  getElementList - returns a list with all subelements of Richness
	%
	...
	%
	% Richness methods (copy):
	%  copy - copies the Richness
	%  deepclone - deep-clones the Richness
	%  clone - clones the Richness
	%
	% Richness methods (inspection, Static):
	%  getClass - returns Richness
	%  getName - returns the name of Richness
	%  getDescription - returns the description of Richness
	%  getProps - returns the property list of Richness
	%  getPropNumber - returns the property number of Richness
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the richness.
			%
			% NAME = Richness.GETNAME() returns the name of the 'richness'.
			%  Richness.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the richness M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('Richness') returns the name of 'Richness'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the richness.
			%
			% STR = Richness.GETDESCRIPTION() returns the description of the 'richness'.
			%  which is:
			%
			%  The richness of a node is the sum of the edges that connect nodes
			%  of higher degree within a layer.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the richness M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('Richness') returns the description of 'Richness'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the richness M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				richness_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, richness_tag_list));
			else
				assert( ...
					Richness.existsTag(tag), ...
					[BRAPH2.STR ':Richness:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Richness:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, richness_tag_list));
			else % numeric
				prop = pointer;
				Richness.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':Richness:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Richness:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' Richness.getPropTag(prop) ' (' Richness.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			%GETMEASURESHAPE returns 2 (Nodal).
			%
			% 2 = Richness.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			%GETMEASURESCOPE returns 2 (Unilayer).
			%
			% 2 = Richness.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			%GETMEASUREPARAMETRICITY returns 2 (Non-parametric).
			%
			% 2 = Richness.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			%GETCOMPATIBLEGRAPHLIST returns the list of Richness compatible graphs.
			%
			% LIST = Richness.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			%GETCOMPATIBLEGRAPHNUMBER returns the number (12) of Richness compatible graphs.
			%
			% 12 = Richness.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = Richness(varargin)
			...
			%
			% Richness(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
