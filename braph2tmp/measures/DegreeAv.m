classdef DegreeAv < Degree
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the average degree.
	%
	...
	%  getDescription - returns the description of the average degree.
	%  getName - returns the name of the average degree.
	...
	%
	% DegreeAv methods (operators):
	%  isequal - determines whether two DegreeAv are equal (values, locked)
	%
	% DegreeAv methods (display):
	%  tostring - string with information about the DegreeAv
	%  disp - displays information about the DegreeAv
	%  tree - displays the element of DegreeAv
	%
	% DegreeAv method (element list):
	%  getElementList - returns a list with all subelements of DegreeAv
	%
	% DegreeAv method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the DegreeAv
	%
	% DegreeAv method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the DegreeAv
	%
	% DegreeAv methods (copy):
	%  copy - copies the DegreeAv
	%  deepclone - deep-clones the DegreeAv
	%  clone - clones the DegreeAv
	%
	% DegreeAv methods (inspection, Static):
	%  getClass - returns DegreeAv
	%  getName - returns the name of DegreeAv
	%  getDescription - returns the description of DegreeAv
	%  getProps - returns the property list of DegreeAv
	%  getPropNumber - returns the property number of DegreeAv
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the average degree.
			%
			% NAME = DegreeAv.GETNAME() returns the name of the 'average degree'.
			%  Average Degree.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the average degree M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('DegreeAv') returns the name of 'DegreeAv'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			m_name = 'Average Degree';
		end
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the average degree.
			%
			% STR = DegreeAv.GETDESCRIPTION() returns the description of the 'average degree'.
			%  which is:
			%
			%  The average degree of a graph is the average of all number of edges 
			%  connected to a node within a layer. 
			...
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the average degree M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('DegreeAv') returns the description of 'DegreeAv'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the average degree M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				degreeav_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, degreeav_tag_list));
			else
				assert( ...
					DegreeAv.existsTag(tag), ...
					[BRAPH2.STR ':DegreeAv:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':DegreeAv:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, degreeav_tag_list));
			else % numeric
				prop = pointer;
				DegreeAv.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':DegreeAv:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':DegreeAv:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' DegreeAv.getPropTag(prop) ' (' DegreeAv.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 1 = DegreeAv.GETMEASURESHAPE() returns the measures graph shape (Global).
			%
		...
		function scope = getMeasureScope()
			%GETMEASURESCOPE returns 2 (Unilayer).
			%
			% 2 = DegreeAv.GETMEASURESCOPE() returns the measures layer scope (Global).
			%
		...
		function parametricity = getMeasureParametricity()
			%GETMEASUREPARAMETRICITY returns 2 (Non-parametric).
			%
			% 2 = DegreeAv.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			%GETCOMPATIBLEGRAPHLIST returns the list of DegreeAv compatible graphs.
			%
			% LIST = DegreeAv.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			%GETCOMPATIBLEGRAPHNUMBER returns the number (8) of DegreeAv compatible graphs.
			%
			% 8 = DegreeAv.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = DegreeAv(varargin)
			...
			%
			% DegreeAv(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
