classdef Eccentricity < Measure
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the eccentricity.
	...
	%  getDescription - returns the description of the eccentricity.
	%  getName - returns the name of the eccentricity.
	...
	%
	% Eccentricity methods (operators):
	...
	%  tree - displays the element of Eccentricity
	%
	% Eccentricity method (element list):
	%  getElementList - returns a list with all subelements of Eccentricity
	%
	...
	%
	% Eccentricity methods (copy):
	%  copy - copies the Eccentricity
	%  deepclone - deep-clones the Eccentricity
	%  clone - clones the Eccentricity
	%
	% Eccentricity methods (inspection, Static):
	%  getClass - returns Eccentricity
	%  getName - returns the name of Eccentricity
	%  getDescription - returns the description of Eccentricity
	%  getProps - returns the property list of Eccentricity
	%  getPropNumber - returns the property number of Eccentricity
	...
	%
	% Eccentricity properties (Constant).
	%  RULE - 6
	%  RULE_TAG - 'rule'
	%  RULE_CATEGORY - 'p'
	%  RULE_FORMAT - 'op'
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the eccentricity.
			%
			% NAME = Eccentricity.GETNAME() returns the name of the 'eccentricity'.
			%  Eccentricity.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the eccentricity M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('Eccentricity') returns the name of 'Eccentricity'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the eccentricity.
			%
			% STR = Eccentricity.GETDESCRIPTION() returns the description of the 'eccentricity'.
			%  which is:
			%
			%  The eccentricity of a node is the maximal shortest path length between a node and any other node within a layer.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the eccentricity M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('Eccentricity') returns the description of 'Eccentricity'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the eccentricity M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				eccentricity_tag_list = { 'id'  'label'  'notes'  'g'  'm'  'rule' };
				
				check = any(strcmpi(tag, eccentricity_tag_list));
			else
				assert( ...
					Eccentricity.existsTag(tag), ...
					[BRAPH2.STR ':Eccentricity:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Eccentricity:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, eccentricity_tag_list));
			else % numeric
				prop = pointer;
				Eccentricity.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':Eccentricity:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Eccentricity:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' Eccentricity.getPropTag(prop) ' (' Eccentricity.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 2 = Eccentricity.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = Eccentricity.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = Eccentricity.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			...
			%
			% LIST = Eccentricity.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			...
			%
			% 8 = Eccentricity.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = Eccentricity(varargin)
			...
			%
			% Eccentricity(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
