classdef Distance < Measure
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the distance.
	%
	...
	%  getDescription - returns the description of the distance.
	%  getName - returns the name of the distance.
	...
	%  tree - displays the element of Distance
	%
	% Distance method (element list):
	%  getElementList - returns a list with all subelements of Distance
	%
	...
	%
	% Distance methods (copy):
	%  copy - copies the Distance
	%  deepclone - deep-clones the Distance
	%  clone - clones the Distance
	%
	% Distance methods (inspection, Static):
	%  getClass - returns Distance
	%  getName - returns the name of Distance
	%  getDescription - returns the description of Distance
	%  getProps - returns the property list of Distance
	%  getPropNumber - returns the property number of Distance
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the distance.
			%
			% NAME = Distance.GETNAME() returns the name of the 'distance'.
			%  Distance.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the distance M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('Distance') returns the name of 'Distance'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the distance.
			%
			% STR = Distance.GETDESCRIPTION() returns the description of the 'distance'.
			%  which is:
			%
			%  The distance of a graph is the shortest path 
			%  between all pairs of nodes within a layer of the graph.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the distance M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('Distance') returns the description of 'Distance'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the distance M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				distance_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, distance_tag_list));
			else
				assert( ...
					Distance.existsTag(tag), ...
					[BRAPH2.STR ':Distance:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Distance:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, distance_tag_list));
			else % numeric
				prop = pointer;
				Distance.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':Distance:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Distance:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' Distance.getPropTag(prop) ' (' Distance.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 3 = Distance.GETMEASURESHAPE() returns the measures graph shape (Binodal).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = Distance.GETMEASURESCOPE() returns the measures layer scope (Binodal).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = Distance.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			...
			%
			% LIST = Distance.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			...
			%
			% 12 = Distance.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = Distance(varargin)
			...
			%
			% Distance(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
