classdef CorePeriphery < Richness
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the core-periphery.
	%
	% CorePeriphery methods (constructor):
	% CorePeriphery - constructor
	%
	% CorePeriphery methods (Static):
	...
	%  getDescription - returns the description of the core-periphery.
	%  getName - returns the name of the core-periphery.
	...
	%
	% CorePeriphery methods (operators):
	%  isequal - determines whether two CorePeriphery are equal (values, locked)
	%
	% CorePeriphery methods (display):
	%  tostring - string with information about the CorePeriphery
	%  disp - displays information about the CorePeriphery
	%  tree - displays the element of CorePeriphery
	%
	% CorePeriphery method (element list):
	%  getElementList - returns a list with all subelements of CorePeriphery
	%
	% CorePeriphery method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the CorePeriphery
	%
	% CorePeriphery method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the CorePeriphery
	%
	% CorePeriphery methods (copy):
	%  copy - copies the CorePeriphery
	%  deepclone - deep-clones the CorePeriphery
	%  clone - clones the CorePeriphery
	%
	% CorePeriphery methods (inspection, Static):
	%  getClass - returns CorePeriphery
	%  getName - returns the name of CorePeriphery
	%  getDescription - returns the description of CorePeriphery
	%  getProps - returns the property list of CorePeriphery
	%  getPropNumber - returns the property number of CorePeriphery
	...
	%
	% CorePeriphery properties (Constant).
	%  GLOBAL - 1
	%  GLOBAL_NAME - 'Global'
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the core-periphery.
			%
			% NAME = CorePeriphery.GETNAME() returns the name of the 'core-periphery'.
			%  Core-Periphery.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the core-periphery M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('CorePeriphery') returns the name of 'CorePeriphery'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			m_name = 'Core-Periphery';
		end
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the core-periphery.
			%
			% STR = CorePeriphery.GETDESCRIPTION() returns the description of the 'core-periphery'.
			%  which is:
			%
			%  The core-periphery of a node is the value of the rank corresponding 
			%  to the maximum richness nodes. It returns 1 for a node belonging to the 
			...
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the core-periphery M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('CorePeriphery') returns the description of 'CorePeriphery'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the core-periphery M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				coreperiphery_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, coreperiphery_tag_list));
			else
				assert( ...
					CorePeriphery.existsTag(tag), ...
					[BRAPH2.STR ':CorePeriphery:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':CorePeriphery:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, coreperiphery_tag_list));
			else % numeric
				prop = pointer;
				CorePeriphery.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':CorePeriphery:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':CorePeriphery:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' CorePeriphery.getPropTag(prop) ' (' CorePeriphery.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			%GETMEASURESHAPE returns 2 (Nodal).
			%
			% 2 = CorePeriphery.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			%GETMEASURESCOPE returns 2 (Unilayer).
			%
			% 2 = CorePeriphery.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			%GETMEASUREPARAMETRICITY returns 2 (Non-parametric).
			%
			% 2 = CorePeriphery.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			%GETCOMPATIBLEGRAPHLIST returns the list of CorePeriphery compatible graphs.
			%
			% LIST = CorePeriphery.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			%GETCOMPATIBLEGRAPHNUMBER returns the number (12) of CorePeriphery compatible graphs.
			%
			% 12 = CorePeriphery.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
			% See also getCompatibleGraphList.
			
			n = Measure.getCompatibleGraphNumber('CorePeriphery');
		end
	end
	methods % constructor
		function m = CorePeriphery(varargin)
			...
			%
			% CorePeriphery(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
