classdef EigenVectorCentrality < Measure
	% EigenVectorCentrality is the eigen vector centrality.
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the eigen vector centrality.
	%
	% EigenVectorCentrality methods (constructor):
	% EigenVectorCentrality - constructor
	%
	% EigenVectorCentrality methods (Static):
	...
	%  existsTag - checks whether tag exists in eigen vector centrality/error.
	%  existsProp - checks whether property exists in eigen vector centrality/error.
	%  getPropNumber - returns the property number of eigen vector centrality.
	%  getProps - returns the property list of eigen vector centrality.
	%  getDescription - returns the description of the eigen vector centrality.
	%  getName - returns the name of the eigen vector centrality.
	%  getClass - returns the class of the eigen vector centrality.
	...
	%
	% EigenVectorCentrality methods (operators):
	...
	%  tree - displays the element of EigenVectorCentrality
	%
	% EigenVectorCentrality method (element list):
	%  getElementList - returns a list with all subelements of EigenVectorCentrality
	%
	...
	%
	% EigenVectorCentrality methods (copy):
	%  copy - copies the EigenVectorCentrality
	%  deepclone - deep-clones the EigenVectorCentrality
	%  clone - clones the EigenVectorCentrality
	%
	% EigenVectorCentrality methods (inspection, Static):
	%  getClass - returns EigenVectorCentrality
	%  getName - returns the name of EigenVectorCentrality
	%  getDescription - returns the description of EigenVectorCentrality
	...
	%
	% EigenVectorCentrality properties (Constant).
	%  GLOBAL - 1
	%  GLOBAL_NAME - 'Global'
	...
	methods (Static) % inspection methods
		function m_class = getClass()
			...
			%  CLASS = M.GETCLASS() returns the class of the eigen vector centrality M.
		...
		function m_name = getName()
			%GETNAME returns the name of the eigen vector centrality.
			%
			% NAME = EigenVectorCentrality.GETNAME() returns the name of the 'eigen vector centrality'.
			%  Eigen Vector Centrality.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the eigen vector centrality M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('EigenVectorCentrality') returns the name of 'EigenVectorCentrality'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			m_name = 'Eigen Vector Centrality';
		end
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the eigen vector centrality.
			%
			% STR = EigenVectorCentrality.GETDESCRIPTION() returns the description of the 'eigen vector centrality'.
			%  which is:
			%
			%  The eigen vector centrality of a node is the ith element in the eigenvector corresponding 
			%  to the largest eigenvalue of the largest eigenvalue of the graphs adjacency matrix.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the eigen vector centrality M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('EigenVectorCentrality') returns the description of 'EigenVectorCentrality'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of eigen vector centrality.
			%
			% PROPS = EigenVectorCentrality.GETPROPS() returns the property list of eigen vector centrality.
			%
			% PROPS = EigenVectorCentrality.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = M.GETPROPS([CATEGORY]) returns the property list of the eigen vector centrality M.
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of eigen vector centrality.
			%
			% N = EigenVectorCentrality.GETPROPNUMBER() returns the property number of eigen vector centrality.
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the eigen vector centrality M.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					EigenVectorCentrality.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				eigenvectorcentrality_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, eigenvectorcentrality_tag_list));
			else
				assert( ...
					EigenVectorCentrality.existsTag(tag), ...
					[BRAPH2.STR ':EigenVectorCentrality:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':EigenVectorCentrality:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, eigenvectorcentrality_tag_list));
			else % numeric
				prop = pointer;
				EigenVectorCentrality.existsProp(prop);
			end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case EigenVectorCentrality.M
					prop_description = 'M (result, cell) is the eigen vector centrality.';
				otherwise
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':EigenVectorCentrality:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':EigenVectorCentrality:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' EigenVectorCentrality.getPropTag(prop) ' (' EigenVectorCentrality.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 2 = EigenVectorCentrality.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = EigenVectorCentrality.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = EigenVectorCentrality.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			%GETCOMPATIBLEGRAPHLIST returns the list of EigenVectorCentrality compatible graphs.
			%
			% LIST = EigenVectorCentrality.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			%GETCOMPATIBLEGRAPHNUMBER returns the number (8) of EigenVectorCentrality compatible graphs.
			%
			% 8 = EigenVectorCentrality.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = EigenVectorCentrality(varargin)
			% EigenVectorCentrality() creates a eigen vector centrality.
			%
			% EigenVectorCentrality(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% EigenVectorCentrality(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
