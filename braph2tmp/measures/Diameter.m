classdef Diameter < Measure
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the diameter.
	...
	%  getDescription - returns the description of the diameter.
	%  getName - returns the name of the diameter.
	...
	%  tree - displays the element of Diameter
	%
	% Diameter method (element list):
	%  getElementList - returns a list with all subelements of Diameter
	%
	...
	%
	% Diameter methods (copy):
	%  copy - copies the Diameter
	%  deepclone - deep-clones the Diameter
	%  clone - clones the Diameter
	%
	% Diameter methods (inspection, Static):
	%  getClass - returns Diameter
	%  getName - returns the name of Diameter
	%  getDescription - returns the description of Diameter
	%  getProps - returns the property list of Diameter
	%  getPropNumber - returns the property number of Diameter
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the diameter.
			%
			% NAME = Diameter.GETNAME() returns the name of the 'diameter'.
			%  Diameter.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the diameter M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('Diameter') returns the name of 'Diameter'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the diameter.
			%
			% STR = Diameter.GETDESCRIPTION() returns the description of the 'diameter'.
			%  which is:
			%
			%  The diameter is the maximum eccentricity among the vertice within a layer.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the diameter M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('Diameter') returns the description of 'Diameter'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the diameter M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				diameter_tag_list = { 'id'  'label'  'notes'  'g'  'm'  'rule' };
				
				check = any(strcmpi(tag, diameter_tag_list));
			else
				assert( ...
					Diameter.existsTag(tag), ...
					[BRAPH2.STR ':Diameter:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Diameter:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, diameter_tag_list));
			else % numeric
				prop = pointer;
				Diameter.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':Diameter:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Diameter:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' Diameter.getPropTag(prop) ' (' Diameter.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 1 = Diameter.GETMEASURESHAPE() returns the measures graph shape (Global).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = Diameter.GETMEASURESCOPE() returns the measures layer scope (Global).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = Diameter.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			%GETCOMPATIBLEGRAPHLIST returns the list of Diameter compatible graphs.
			%
			% LIST = Diameter.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			%GETCOMPATIBLEGRAPHNUMBER returns the number (8) of Diameter compatible graphs.
			%
			% 8 = Diameter.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = Diameter(varargin)
			...
			%
			% Diameter(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
