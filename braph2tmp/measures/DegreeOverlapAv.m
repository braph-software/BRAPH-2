classdef DegreeOverlapAv < DegreeOverlap
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the average degree overlap.
	%
	...
	%  existsTag - checks whether tag exists in average degree overlap/error.
	%  existsProp - checks whether property exists in average degree overlap/error.
	%  getPropNumber - returns the property number of average degree overlap.
	%  getProps - returns the property list of average degree overlap.
	%  getDescription - returns the description of the average degree overlap.
	%  getName - returns the name of the average degree overlap.
	%  getClass - returns the class of the average degree overlap.
	...
	%
	% DegreeOverlapAv methods (operators):
	%  isequal - determines whether two DegreeOverlapAv are equal (values, locked)
	%
	% DegreeOverlapAv methods (display):
	%  tostring - string with information about the DegreeOverlapAv
	%  disp - displays information about the DegreeOverlapAv
	%  tree - displays the element of DegreeOverlapAv
	%
	% DegreeOverlapAv method (element list):
	%  getElementList - returns a list with all subelements of DegreeOverlapAv
	%
	% DegreeOverlapAv method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the DegreeOverlapAv
	%
	% DegreeOverlapAv method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the DegreeOverlapAv
	%
	% DegreeOverlapAv methods (copy):
	%  copy - copies the DegreeOverlapAv
	%  deepclone - deep-clones the DegreeOverlapAv
	%  clone - clones the DegreeOverlapAv
	%
	% DegreeOverlapAv methods (inspection, Static):
	%  getClass - returns DegreeOverlapAv
	%  getName - returns the name of DegreeOverlapAv
	%  getDescription - returns the description of DegreeOverlapAv
	%  getProps - returns the property list of DegreeOverlapAv
	%  getPropNumber - returns the property number of DegreeOverlapAv
	...
	%
	% DegreeOverlapAv properties (Constant).
	%  GLOBAL - 1
	%  GLOBAL_NAME - 'Global'
	...
	methods (Static) % inspection methods
		function m_class = getClass()
			...
			%  CLASS = M.GETCLASS() returns the class of the average degree overlap M.
		...
		function m_name = getName()
			%GETNAME returns the name of the average degree overlap.
			%
			% NAME = DegreeOverlapAv.GETNAME() returns the name of the 'average degree overlap'.
			%  Average Degree Overlap.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the average degree overlap M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('DegreeOverlapAv') returns the name of 'DegreeOverlapAv'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			m_name = 'Average Degree Overlap';
		end
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the average degree overlap.
			%
			% STR = DegreeOverlapAv.GETDESCRIPTION() returns the description of the 'average degree overlap'.
			%  which is:
			%
			%  The average degree overlap is the average of the number of edges connected to the 
			%  node in all layers. Connection weights are ignored in calculations.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the average degree overlap M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('DegreeOverlapAv') returns the description of 'DegreeOverlapAv'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of average degree overlap.
			%
			% PROPS = DegreeOverlapAv.GETPROPS() returns the property list of average degree overlap.
			%
			% PROPS = DegreeOverlapAv.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = M.GETPROPS([CATEGORY]) returns the property list of the average degree overlap M.
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of average degree overlap.
			%
			% N = DegreeOverlapAv.GETPROPNUMBER() returns the property number of average degree overlap.
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the average degree overlap M.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					DegreeOverlapAv.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				degreeoverlapav_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, degreeoverlapav_tag_list));
			else
				assert( ...
					DegreeOverlapAv.existsTag(tag), ...
					[BRAPH2.STR ':DegreeOverlapAv:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':DegreeOverlapAv:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, degreeoverlapav_tag_list));
			else % numeric
				prop = pointer;
				DegreeOverlapAv.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':DegreeOverlapAv:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':DegreeOverlapAv:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' DegreeOverlapAv.getPropTag(prop) ' (' DegreeOverlapAv.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 1 = DegreeOverlapAv.GETMEASURESHAPE() returns the measures graph shape (Global).
			%
		...
		function scope = getMeasureScope()
			%GETMEASURESCOPE returns 1 (Superglobal).
			%
			% 1 = DegreeOverlapAv.GETMEASURESCOPE() returns the measures layer scope (Global).
			%
		...
		function parametricity = getMeasureParametricity()
			%GETMEASUREPARAMETRICITY returns 2 (Non-parametric).
			%
			% 2 = DegreeOverlapAv.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			%GETCOMPATIBLEGRAPHLIST returns the list of DegreeOverlapAv compatible graphs.
			%
			% LIST = DegreeOverlapAv.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			%GETCOMPATIBLEGRAPHNUMBER returns the number (4) of DegreeOverlapAv compatible graphs.
			%
			% 4 = DegreeOverlapAv.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = DegreeOverlapAv(varargin)
			% DegreeOverlapAv() creates a average degree overlap.
			%
			% DegreeOverlapAv(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% DegreeOverlapAv(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
			...
			switch prop
				otherwise
					if prop <= DegreeOverlap.getPropNumber()
						value = conditioning@DegreeOverlap(pointer, value);
					end
			end
		end
	end
	methods (Access=protected) % calculate value
		function value = calculateValue(m, prop)
			...
			switch prop
				...
				otherwise
					value = calculateValue@DegreeOverlap(m, prop);
