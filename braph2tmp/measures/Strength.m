classdef Strength < Measure
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the strength.
	%
	...
	%  getDescription - returns the description of the strength.
	%  getName - returns the name of the strength.
	...
	%  tree - displays the element of Strength
	%
	% Strength method (element list):
	%  getElementList - returns a list with all subelements of Strength
	%
	...
	%
	% Strength methods (copy):
	%  copy - copies the Strength
	%  deepclone - deep-clones the Strength
	%  clone - clones the Strength
	%
	% Strength methods (inspection, Static):
	%  getClass - returns Strength
	%  getName - returns the name of Strength
	%  getDescription - returns the description of Strength
	%  getProps - returns the property list of Strength
	%  getPropNumber - returns the property number of Strength
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the strength.
			%
			% NAME = Strength.GETNAME() returns the name of the 'strength'.
			%  Strength.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the strength M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('Strength') returns the name of 'Strength'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the strength.
			%
			% STR = Strength.GETDESCRIPTION() returns the description of the 'strength'.
			%  which is:
			%
			%  The strength of a graph is the sum of all weights of the edges connected to
			%  a node within a layer.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the strength M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('Strength') returns the description of 'Strength'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the strength M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				strength_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, strength_tag_list));
			else
				assert( ...
					Strength.existsTag(tag), ...
					[BRAPH2.STR ':Strength:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Strength:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, strength_tag_list));
			else % numeric
				prop = pointer;
				Strength.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':Strength:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Strength:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' Strength.getPropTag(prop) ' (' Strength.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 2 = Strength.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = Strength.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = Strength.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			...
			%
			% LIST = Strength.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			...
			%
			% 2 = Strength.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = Strength(varargin)
			...
			%
			% Strength(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
