classdef PathLength < Measure
	...
	%  existsTag - checks whether tag exists in path length/error.
	%  existsProp - checks whether property exists in path length/error.
	%  getPropNumber - returns the property number of path length.
	%  getProps - returns the property list of path length.
	%  getDescription - returns the description of the path length.
	%  getName - returns the name of the path length.
	%  getClass - returns the class of the path length.
	...
	%  tree - displays the element of PathLength
	%
	% PathLength method (element list):
	%  getElementList - returns a list with all subelements of PathLength
	%
	...
	%
	% PathLength methods (copy):
	%  copy - copies the PathLength
	%  deepclone - deep-clones the PathLength
	%  clone - clones the PathLength
	%
	% PathLength methods (inspection, Static):
	%  getClass - returns PathLength
	%  getName - returns the name of PathLength
	%  getDescription - returns the description of PathLength
	...
	%
	% PathLength properties (Constant).
	%  RULE - 6
	%  RULE_TAG - 'rule'
	%  RULE_CATEGORY - 'p'
	%  RULE_FORMAT - 'op'
	...
	methods (Static) % inspection methods
		function m_class = getClass()
			...
			%  CLASS = M.GETCLASS() returns the class of the path length M.
		...
		function m_name = getName()
			%GETNAME returns the name of the path length.
			%
			% NAME = PathLength.GETNAME() returns the name of the 'path length'.
			%  Path Length.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the path length M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('PathLength') returns the name of 'PathLength'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the path length.
			%
			% STR = PathLength.GETDESCRIPTION() returns the description of the 'path length'.
			%  which is:
			%
			%  The path length is the average shortest path length of one node to all
			%  other nodes within a layer.
			%
			% Alternative forms to call this method are:
			%  STR = M.GETDESCRIPTION() returns the description of the path length M.
			%  STR = Element.GETDESCRIPTION(M) returns the description of 'M'.
			%  STR = Element.GETDESCRIPTION('PathLength') returns the description of 'PathLength'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of path length.
			%
			% PROPS = PathLength.GETPROPS() returns the property list of path length.
			%
			% PROPS = PathLength.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = M.GETPROPS([CATEGORY]) returns the property list of the path length M.
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of path length.
			%
			% N = PathLength.GETPROPNUMBER() returns the property number of path length.
			%
			% Alternative forms to call this method are:
			%  N = M.GETPROPNUMBER() returns the property number of the path length M.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				pathlength_tag_list = { 'id'  'label'  'notes'  'g'  'm'  'rule' };
				
				check = any(strcmpi(tag, pathlength_tag_list));
			else
				assert( ...
					PathLength.existsTag(tag), ...
					[BRAPH2.STR ':PathLength:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PathLength:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, pathlength_tag_list));
			else % numeric
				prop = pointer;
				PathLength.existsProp(prop);
			end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case PathLength.RULE
					prop_description = 'rule (parameter, OPTION) is the path length algorithm';
				case PathLength.M
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':PathLength:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PathLength:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' PathLength.getPropTag(prop) ' (' PathLength.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 2 = PathLength.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = PathLength.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = PathLength.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			...
			%
			% LIST = PathLength.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			...
			%
			% 8 = PathLength.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = PathLength(varargin)
			% PathLength() creates a path length.
			%
			% PathLength(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% PathLength(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
