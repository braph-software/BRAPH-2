classdef Degree < Measure
	...
	%  <strong>5</strong> <strong>m</strong> 	M (result, cell) is the degree.
	%
	...
	%  tree - displays the element of Degree
	%
	% Degree method (element list):
	%  getElementList - returns a list with all subelements of Degree
	%
	...
	%  deepclone - deep-clones the Degree
	%  clone - clones the Degree
	%
	% Degree methods (inspection, Static):
	%  getClass - returns Degree
	%  getName - returns the name of Degree
	%  getDescription - returns the description of Degree
	%  getProps - returns the property list of Degree
	%  getPropNumber - returns the property number of Degree
	...
	methods (Static) % inspection methods
		...
		function m_name = getName()
			%GETNAME returns the name of the degree.
			%
			% NAME = Degree.GETNAME() returns the name of the 'degree'.
			%  Degree.
			%
			% Alternative forms to call this method are:
			%  NAME = M.GETNAME() returns the name of the degree M.
			%  NAME = Element.GETNAME(M) returns the name of 'M'.
			%  NAME = Element.GETNAME('Degree') returns the name of 'Degree'.
			%
		...
		function m_description = getDescription()
			%GETDESCRIPTION returns the description of the degree.
			%
			% STR = Degree.GETDESCRIPTION() returns the description of the 'degree'.
			%  which is:
			%
			%  The degree of a node is the number of edges connected to the node within a layer. 
			%  Connection weights are ignored in calculations.
			...
			%  STR = Element.GETDESCRIPTION('Degree') returns the description of 'Degree'.
			%
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				degree_tag_list = { 'id'  'label'  'notes'  'g'  'm' };
				
				check = any(strcmpi(tag, degree_tag_list));
			else
				assert( ...
					Degree.existsTag(tag), ...
					[BRAPH2.STR ':Degree:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Degree:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, degree_tag_list));
			else % numeric
				prop = pointer;
				Degree.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':Degree:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':Degree:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' Degree.getPropTag(prop) ' (' Degree.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function shape = getMeasureShape()
			...
			%
			% 2 = Degree.GETMEASURESHAPE() returns the measures graph shape (Nodal).
			%
		...
		function scope = getMeasureScope()
			...
			%
			% 2 = Degree.GETMEASURESCOPE() returns the measures layer scope (Nodal).
			%
		...
		function parametricity = getMeasureParametricity()
			...
			%
			% 2 = Degree.GETMEASUREPARAMETRICITY() returns the measures parametricity (Non-parametric).
			%
		...
		function list = getCompatibleGraphList()
			...
			%
			% LIST = Degree.GETCOMPATIBLEGRAPHLIST() returns the list of compatible graphs.
			%
		...
		function n = getCompatibleGraphNumber()
			...
			%
			% 8 = Degree.GETCOMPATIBLEGRAPHNUMBER() returns the measures number of compatible graphs.
			%
	...
	methods % constructor
		function m = Degree(varargin)
			...
			%
			% Degree(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
