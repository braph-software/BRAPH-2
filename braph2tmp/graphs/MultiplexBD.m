classdef MultiplexBD < Graph
	...
	%  <strong>5</strong> <strong>a</strong> 	A (result, cell) is the cell containing the multiplex binary adjacency matrices of the multiplex binary directed graph.
	%  <strong>6</strong> <strong>m_dict</strong> 	M_DICT (result, idict) contains the calculated measures of the graph.
	%  <strong>7</strong> <strong>b</strong> 	B (data, cell) is the input cell containing the multiplex adjacency matrices.
	...
	%  getDescription - returns the description of the multiplex binary directed graph.
	%  getName - returns the name of the multiplex binary directed graph.
	...
	%
	% MultiplexBD methods (operators):
	%  isequal - determines whether two MultiplexBD are equal (values, locked)
	%
	% MultiplexBD methods (display):
	%  tostring - string with information about the MultiplexBD
	%  disp - displays information about the MultiplexBD
	%  tree - displays the element of MultiplexBD
	%
	% MultiplexBD method (element list):
	%  getElementList - returns a list with all subelements of MultiplexBD
	%
	% MultiplexBD method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the MultiplexBD
	%
	% MultiplexBD method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the MultiplexBD
	%
	% MultiplexBD methods (copy):
	%  copy - copies the MultiplexBD
	%  deepclone - deep-clones the MultiplexBD
	%  clone - clones the MultiplexBD
	%
	% MultiplexBD methods (inspection, Static):
	%  getClass - returns MultiplexBD
	%  getName - returns the name of MultiplexBD
	%  getDescription - returns the description of MultiplexBD
	%  getProps - returns the property list of MultiplexBD
	%  getPropNumber - returns the property number of MultiplexBD
	...
	%
	% MultiplexBD properties (Constant).
	%  B - 7
	%  B_TAG - 'b'
	%  B_CATEGORY - 'd'
	%  B_FORMAT - 'll'
	...
	methods (Static) % inspection methods
		...
		function g_name = getName()
			%GETNAME returns the name of the multiplex binary directed graph.
			%
			% NAME = MultiplexBD.GETNAME() returns the name of the 'multiplex binary directed graph'.
			%  Multiplex Binary Directed Graph.
			%
			% Alternative forms to call this method are:
			%  NAME = G.GETNAME() returns the name of the multiplex binary directed graph G.
			%  NAME = Element.GETNAME(G) returns the name of 'G'.
			%  NAME = Element.GETNAME('MultiplexBD') returns the name of 'MultiplexBD'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			g_name = 'Multiplex Binary Directed Graph';
		end
		function g_description = getDescription()
			%GETDESCRIPTION returns the description of the multiplex binary directed graph.
			%
			% STR = MultiplexBD.GETDESCRIPTION() returns the description of the 'multiplex binary directed graph'.
			%  which is:
			%
			%  In a multiplex binary directed (BD) graph, 
			%  the edges can be either 0 (absence of connection) 
			...
			%
			% Alternative forms to call this method are:
			%  STR = G.GETDESCRIPTION() returns the description of the multiplex binary directed graph G.
			%  STR = Element.GETDESCRIPTION(G) returns the description of 'G'.
			%  STR = Element.GETDESCRIPTION('MultiplexBD') returns the description of 'MultiplexBD'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = G.GETPROPNUMBER() returns the property number of the multiplex binary directed graph G.
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				multiplexbd_tag_list = { 'id'  'label'  'notes'  'brainatlas'  'a'  'm_dict'  'b'  'attemptsperedge' };
				
				check = any(strcmpi(tag, multiplexbd_tag_list));
			else
				assert( ...
					MultiplexBD.existsTag(tag), ...
					[BRAPH2.STR ':MultiplexBD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':MultiplexBD:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, multiplexbd_tag_list));
			else % numeric
				prop = pointer;
				MultiplexBD.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':MultiplexBD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':MultiplexBD:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' MultiplexBD.getPropTag(prop) ' (' MultiplexBD.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods (Static) %% graph methods
		function graph = getGraphType()
			...
			%
			% 4 = MultiplexBD.GETGRAPHTYPE() returns the type of graph (Multiplex).
			%
		...
		function connectivity = getConnectivityType(varargin)
			...
			%
			% 2 = MultiplexBD.GETCONNECTIVITYTYPE() returns the type of graph connectivity (Binary).
			%
			% 2 = MultiplexBD.GETCONNECTIVITYTYPE(LAYERNUMBER) returns a matrix with the 
		...
		function directionality = getDirectionalityType(varargin)
			...
			%
			% 2 = MultiplexBD.GETDIRECTIONALITYTYPE() returns the type of graph directionality (Undirected).
			% the directionality type of each layer.
			%
			% 2 = MultiplexBD.GETDIRECTIONALITYTYPE(LAYERNUMBER) returns a matrix with 
			%  the directionality type of each layer.
			%
		...
		function selfconnectivity = getSelfConnectivityType(varargin)
			...
			%
			% 2 = MultiplexBD.GETSELFCONNECTIVITYTYPE() returns the type of graph self connnectivity (Self-connected).
			%
			% 2 = MultiplexBD.GETSELFCONNECTIVITYTYPE(LAYERNUMBER) returns a matrix 
		...
		function negativity = getNegativityType(varargin)
			...
			%
			% 2 = MultiplexBD.GETNEGATIVITYTYPE() returns the type of graph negativity (Negative).
			%
			% 2 = MultiplexBD.GETNEGATIVITYTYPE(LAYERNUMBER) returns a matrix 
	...
	methods % constructor
		function g = MultiplexBD(varargin)
			...
			%
			% MultiplexBD(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
