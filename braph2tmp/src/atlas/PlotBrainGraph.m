classdef PlotBrainGraph < PlotBrainAtlas
	% PlotBrainGraph is a plot of a brain graph.
	...
	%
	% The list of PlotBrainGraph properties is:
	...
	%
	% PlotBrainGraph methods (constructor):
	% PlotBrainGraph - constructor
	%
	% PlotBrainGraph methods (Static):
	...
	%  set_settings - h/set_settings is a function.
	%  cb_close - closes the figure containing the panel and the settings figure.
	%  cb_close_fs - ph/cb_close_fs is a function.
	...
	%
	% PlotBrainGraph methods (operators):
	%  isequal - determines whether two PlotBrainGraph are equal (values, locked)
	%
	% PlotBrainGraph methods (display):
	%  tostring - string with information about the PlotBrainGraph
	%  disp - displays information about the PlotBrainGraph
	%  tree - displays the element of PlotBrainGraph
	%
	% PlotBrainGraph method (element list):
	%  getElementList - returns a list with all subelements of PlotBrainGraph
	%
	% PlotBrainGraph method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the PlotBrainGraph
	%
	% PlotBrainGraph method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the PlotBrainGraph
	%
	% PlotBrainGraph methods (copy):
	%  copy - copies the PlotBrainGraph
	%  deepclone - deep-clones the PlotBrainGraph
	%  clone - clones the PlotBrainGraph
	%
	% PlotBrainGraph methods (inspection, Static):
	%  getClass - returns PlotBrainGraph
	%  getName - returns the name of PlotBrainGraph
	%  getDescription - returns the description of PlotBrainGraph
	%  getProps - returns the property list of PlotBrainGraph
	%  getPropNumber - returns the property number of PlotBrainGraph
	...
	%  checkProp - checks whether a value has the correct format/error
	%
	% PlotBrainGraph methods (category, Static):
	...
	%
	% PlotBrainGraph methods (format, Static):
	...
	%
	% PlotBrainGraph methods (GUI):
	%  getGUI - returns the element GUI
	%  getPlotElement - returns the element plot
	%  getPlotProp - returns a prop plot
	%
	% PlotBrainGraph methods (GUI, Static):
	%  getGUIMenuImport - returns an importer menu
	%  getGUIMenuExport - returns an exporter menu
	%
	% PlotBrainGraph properties (Constant).
	...
	methods (Static) % inspection methods
		function pl_class = getClass()
			%GETCLASS returns the class of the plot brain graph.
			%
			% CLASS = PlotBrainGraph.GETCLASS() returns the class 'PlotBrainGraph'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PL.GETCLASS() returns the class of the plot brain graph PL.
			%  CLASS = Element.GETCLASS(PL) returns the class of 'PL'.
			%  CLASS = Element.GETCLASS('PlotBrainGraph') returns 'PlotBrainGraph'.
			%
			% See also getName, getDescription.
			
			pl_class = 'PlotBrainGraph';
		end
		function pl_name = getName()
			%GETNAME returns the name of the plot brain graph.
			%
			% NAME = PlotBrainGraph.GETNAME() returns the name of the 'plot brain graph'.
			%  Plot Brain Graph.
			%
			% Alternative forms to call this method are:
			%  NAME = PL.GETNAME() returns the name of the plot brain graph PL.
			%  NAME = Element.GETNAME(PL) returns the name of 'PL'.
			%  NAME = Element.GETNAME('PlotBrainGraph') returns the name of 'PlotBrainGraph'.
			%
		...
		function pl_description = getDescription()
			%GETDESCRIPTION returns the description of the plot brain graph.
			%
			% STR = PlotBrainGraph.GETDESCRIPTION() returns the description of the 'plot brain graph'.
			%  which is:
			%
			%  PlotBrainGraph manages the plot of the graph edges, arrows and cylinders.
			%  PlotBrainGraph utilizes the surface created from PlotBrainAtlas to
			...
			%  STR = Element.GETDESCRIPTION('PlotBrainGraph') returns the description of 'PlotBrainGraph'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of plot brain graph.
			%
			% PROPS = PlotBrainGraph.GETPROPS() returns the property list of plot brain graph.
			%
			% PROPS = PlotBrainGraph.GETPROPS(CATEGORY) returns the property list 
			...
			%  PROPS = Element.GETPROPS('PlotBrainGraph'[, CATEGORY]) returns the property list of 'PlotBrainGraph'.
			%
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of plot brain graph.
			%
			% N = PlotBrainGraph.GETPROPNUMBER() returns the property number of plot brain graph.
			%
			% Alternative forms to call this method are:
			...
			%  N = Element.GETPROPNUMBER('PlotBrainGraph') returns the property number of 'PlotBrainGraph'.
			%
			% See also getProps.
		...
		function check = existsProp(prop)
			%EXISTSPROP checks whether property exists in plot brain graph/error.
			%
			% CHECK = PlotBrainGraph.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			...
			%  CHECK = Element.EXISTSPROP(PlotBrainGraph, PROP) checks whether PROP exists for PlotBrainGraph.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%  Element.EXISTSPROP(PlotBrainGraph, PROP) throws error if PROP does NOT exist for PlotBrainGraph.
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%
			...
			if nargout == 1
				...
				assert( ...
					PlotBrainGraph.existsProp(prop), ...
					[BRAPH2.STR ':PlotBrainGraph:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PlotBrainGraph:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for PlotBrainGraph.'] ...
					)
			end
		end
		function check = existsTag(tag)
			%EXISTSTAG checks whether tag exists in plot brain graph/error.
			%
			% CHECK = PlotBrainGraph.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PL.EXISTSTAG(TAG) checks whether TAG exists for PL.
			%  CHECK = Element.EXISTSTAG(PL, TAG) checks whether TAG exists for PL.
			%  CHECK = Element.EXISTSTAG(PlotBrainGraph, TAG) checks whether TAG exists for PlotBrainGraph.
			%
			...
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%  Element.EXISTSTAG(PlotBrainGraph, TAG) throws error if TAG does NOT exist for PlotBrainGraph.
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(PlotBrainGraph, POINTER) returns property number of POINTER of PlotBrainGraph.
			%  PROPERTY = PL.GETPROPPROP(PlotBrainGraph, POINTER) returns property number of POINTER of PlotBrainGraph.
			%
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, plotbraingraph_tag_list));
			else % numeric
				prop = pointer;
				PlotBrainGraph.existsProp(prop);
			end
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(PlotBrainGraph, POINTER) returns tag of POINTER of PlotBrainGraph.
			%  TAG = PL.GETPROPTAG(PlotBrainGraph, POINTER) returns tag of POINTER of PlotBrainGraph.
			%
			...
			if ischar(pointer)
				...
				switch prop
					case PlotBrainGraph.ME
						tag = PlotBrainGraph.ME_TAG;
					case PlotBrainGraph.TYPE
						tag = PlotBrainGraph.TYPE_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(PlotBrainGraph, POINTER) returns category of POINTER of PlotBrainGraph.
			%  CATEGORY = PL.GETPROPCATEGORY(PlotBrainGraph, POINTER) returns category of POINTER of PlotBrainGraph.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
			
			prop = PlotBrainGraph.getPropProp(pointer);
			
			switch prop
				case PlotBrainGraph.ME
					prop_category = PlotBrainGraph.ME_CATEGORY;
				case PlotBrainGraph.TYPE
					prop_category = PlotBrainGraph.TYPE_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(PlotBrainGraph, POINTER) returns format of POINTER of PlotBrainGraph.
			%  FORMAT = PL.GETPROPFORMAT(PlotBrainGraph, POINTER) returns format of POINTER of PlotBrainGraph.
			%
			...
			switch prop
				case PlotBrainGraph.ME
					prop_format = PlotBrainGraph.ME_FORMAT;
				case PlotBrainGraph.TYPE
					prop_format = PlotBrainGraph.TYPE_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(PlotBrainGraph, POINTER) returns description of POINTER of PlotBrainGraph.
			%  DESCRIPTION = PL.GETPROPDESCRIPTION(PlotBrainGraph, POINTER) returns description of POINTER of PlotBrainGraph.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(PlotBrainGraph, POINTER) returns settings of POINTER of PlotBrainGraph.
			%  SETTINGS = PL.GETPROPSETTINGS(PlotBrainGraph, POINTER) returns settings of POINTER of PlotBrainGraph.
			%
			% See also getPropProp, getPropTag, getPropCategory,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = PlotBrainGraph.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = PlotBrainGraph.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PL.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PL.
			%  DEFAULT = Element.GETPROPDEFAULT(PlotBrainGraph, POINTER) returns the default value of POINTER of PlotBrainGraph.
			%  DEFAULT = PL.GETPROPDEFAULT(PlotBrainGraph, POINTER) returns the default value of POINTER of PlotBrainGraph.
			%
			...
			switch prop
				case PlotBrainGraph.ME
					prop_default = Format.getFormatDefault(Format.ITEM, PlotBrainGraph.getPropSettings(prop));
				case PlotBrainGraph.TYPE
					prop_default = Format.getFormatDefault(Format.STRING, PlotBrainGraph.getPropSettings(prop));
				otherwise
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = PlotBrainGraph.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = PlotBrainGraph.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PL.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PL.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(PlotBrainGraph, POINTER) returns the conditioned default value of POINTER of PlotBrainGraph.
			%  DEFAULT = PL.GETPROPDEFAULTCONDITIONED(PlotBrainGraph, POINTER) returns the conditioned default value of POINTER of PlotBrainGraph.
			%
			...
			prop_default = PlotBrainGraph.conditioning(prop, PlotBrainGraph.getPropDefault(prop));
		end
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(PlotBrainGraph, PROP, VALUE) checks VALUE format for PROP of PlotBrainGraph.
			%  CHECK = PL.CHECKPROP(PlotBrainGraph, PROP, VALUE) checks VALUE format for PROP of PlotBrainGraph.
			% 
			...
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%  Element.CHECKPROP(PlotBrainGraph, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PlotBrainGraph.
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			%  PL.CHECKPROP(PlotBrainGraph, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PlotBrainGraph.
			%   Error id: [BRAPH2:PlotBrainGraph:WrongInput]
			% 
			...
			switch prop
				case PlotBrainGraph.ME
					check = Format.checkFormat(Format.ITEM, value, PlotBrainGraph.getPropSettings(prop));
				case PlotBrainGraph.TYPE
					check = Format.checkFormat(Format.STRING, value, PlotBrainGraph.getPropSettings(prop));
				otherwise
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':PlotBrainGraph:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PlotBrainGraph:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' PlotBrainGraph.getPropTag(prop) ' (' PlotBrainGraph.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function pl = PlotBrainGraph(varargin)
			% PlotBrainGraph() creates a plot brain graph.
			%
			% PlotBrainGraph(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% PlotBrainGraph(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
