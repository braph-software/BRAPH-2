classdef PPAnalyzeGroup_G < PlotProp
	% PPAnalyzeGroup_G is a plot of a graph property.
	...
	%
	% The list of PPAnalyzeGroup_G properties is:
	...
	%
	% PPAnalyzeGroup_G methods (constructor):
	% PPAnalyzeGroup_G - constructor
	%
	% PPAnalyzeGroup_G methods (Static):
	...
	%  cb_graph_adj_figure - graph_adj_figure is a function.
	%  cb_graph_ui_figure - E draws a new figure to manage a plot graph.
	%  cb_measure_calc - /cb_measure_calc is a function.
	...
	%  cb_button_del - _G/cb_button_del is a function.
	%  cb_button_calc - G/cb_button_calc is a function.
	%  cb_button_cb - p_G/cb_button_cb is a function.
	%  set_settings - p_G/set_settings is a function.
	%  cb_close_fs - up_G/cb_close_fs is a function.
	...
	%
	% PPAnalyzeGroup_G methods (operators):
	%  isequal - determines whether two PPAnalyzeGroup_G are equal (values, locked)
	%
	% PPAnalyzeGroup_G methods (display):
	%  tostring - string with information about the PPAnalyzeGroup_G
	%  disp - displays information about the PPAnalyzeGroup_G
	%  tree - displays the element of PPAnalyzeGroup_G
	%
	% PPAnalyzeGroup_G method (element list):
	%  getElementList - returns a list with all subelements of PPAnalyzeGroup_G
	%
	% PPAnalyzeGroup_G method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the PPAnalyzeGroup_G
	%
	% PPAnalyzeGroup_G method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the PPAnalyzeGroup_G
	%
	% PPAnalyzeGroup_G methods (copy):
	%  copy - copies the PPAnalyzeGroup_G
	%  deepclone - deep-clones the PPAnalyzeGroup_G
	%  clone - clones the PPAnalyzeGroup_G
	%
	% PPAnalyzeGroup_G methods (inspection, Static):
	%  getClass - returns PPAnalyzeGroup_G
	%  getName - returns the name of PPAnalyzeGroup_G
	%  getDescription - returns the description of PPAnalyzeGroup_G
	%  getProps - returns the property list of PPAnalyzeGroup_G
	%  getPropNumber - returns the property number of PPAnalyzeGroup_G
	...
	%  checkProp - checks whether a value has the correct format/error
	%
	% PPAnalyzeGroup_G methods (category, Static):
	...
	%
	% PPAnalyzeGroup_G methods (format, Static):
	...
	%
	% PPAnalyzeGroup_G methods (GUI):
	%  getGUI - returns the element GUI
	%  getPlotElement - returns the element plot
	%  getPlotProp - returns a prop plot
	%
	% PPAnalyzeGroup_G methods (GUI, Static):
	%  getGUIMenuImport - returns an importer menu
	%  getGUIMenuExport - returns an exporter menu
	%
	% PPAnalyzeGroup_G properties (Constant).
	%  EL - 3
	%  EL_TAG - 'el'
	%  EL_CATEGORY - 'm'
	%  EL_FORMAT - 'it'
	...
	methods (Static) % inspection methods
		function pr_class = getClass()
			%GETCLASS returns the class of the plot property graph.
			%
			% CLASS = PPAnalyzeGroup_G.GETCLASS() returns the class 'PPAnalyzeGroup_G'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PR.GETCLASS() returns the class of the plot property graph PR.
			%  CLASS = Element.GETCLASS(PR) returns the class of 'PR'.
			%  CLASS = Element.GETCLASS('PPAnalyzeGroup_G') returns 'PPAnalyzeGroup_G'.
			%
			% See also getName, getDescription.
			
			pr_class = 'PPAnalyzeGroup_G';
		end
		function pr_name = getName()
			%GETNAME returns the name of the plot property graph.
			%
			% NAME = PPAnalyzeGroup_G.GETNAME() returns the name of the 'plot property graph'.
			%  Plot Property Graph.
			%
			% Alternative forms to call this method are:
			%  NAME = PR.GETNAME() returns the name of the plot property graph PR.
			%  NAME = Element.GETNAME(PR) returns the name of 'PR'.
			%  NAME = Element.GETNAME('PPAnalyzeGroup_G') returns the name of 'PPAnalyzeGroup_G'.
			%
		...
		function pr_description = getDescription()
			%GETDESCRIPTION returns the description of the plot property graph.
			%
			% STR = PPAnalyzeGroup_G.GETDESCRIPTION() returns the description of the 'plot property graph'.
			%  which is:
			%
			%  PPAnalyzeGroup_G plots the measure table associated with a graph of the analysis.
			%  It also provides the buttons to navigate the graphical interface of both
			...
			%  STR = Element.GETDESCRIPTION('PPAnalyzeGroup_G') returns the description of 'PPAnalyzeGroup_G'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of plot property graph.
			%
			% PROPS = PPAnalyzeGroup_G.GETPROPS() returns the property list of plot property graph.
			%
			% PROPS = PPAnalyzeGroup_G.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the plot property graph PR.
			%  PROPS = Element.GETPROPS(PR[, CATEGORY]) returns the property list of 'PR'.
			%  PROPS = Element.GETPROPS('PPAnalyzeGroup_G'[, CATEGORY]) returns the property list of 'PPAnalyzeGroup_G'.
			%
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of plot property graph.
			%
			% N = PPAnalyzeGroup_G.GETPROPNUMBER() returns the property number of plot property graph.
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER() returns the property number of the plot property graph PR.
			%  N = Element.GETPROPNUMBER(PR) returns the property number of 'PR'.
			%  N = Element.GETPROPNUMBER('PPAnalyzeGroup_G') returns the property number of 'PPAnalyzeGroup_G'.
			%
			% See also getProps.
		...
		function check = existsProp(prop)
			%EXISTSPROP checks whether property exists in plot property graph/error.
			%
			% CHECK = PPAnalyzeGroup_G.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PR.EXISTSPROP(PROP) checks whether PROP exists for PR.
			%  CHECK = Element.EXISTSPROP(PR, PROP) checks whether PROP exists for PR.
			%  CHECK = Element.EXISTSPROP(PPAnalyzeGroup_G, PROP) checks whether PROP exists for PPAnalyzeGroup_G.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%  Element.EXISTSPROP(PPAnalyzeGroup_G, PROP) throws error if PROP does NOT exist for PPAnalyzeGroup_G.
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%
			...
			if nargout == 1
				...
				assert( ...
					PPAnalyzeGroup_G.existsProp(prop), ...
					[BRAPH2.STR ':PPAnalyzeGroup_G:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPAnalyzeGroup_G:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for PPAnalyzeGroup_G.'] ...
					)
			end
		end
		function check = existsTag(tag)
			%EXISTSTAG checks whether tag exists in plot property graph/error.
			%
			% CHECK = PPAnalyzeGroup_G.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PR.EXISTSTAG(TAG) checks whether TAG exists for PR.
			%  CHECK = Element.EXISTSTAG(PR, TAG) checks whether TAG exists for PR.
			%  CHECK = Element.EXISTSTAG(PPAnalyzeGroup_G, TAG) checks whether TAG exists for PPAnalyzeGroup_G.
			%
			...
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%  Element.EXISTSTAG(PPAnalyzeGroup_G, TAG) throws error if TAG does NOT exist for PPAnalyzeGroup_G.
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%
			% See also getProps, existsTag.
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				ppanalyzegroup_g_tag_list = { 'id'  'bkgcolor'  'el'  'prop'  'title'  'enable'  'waitbar' };
				
				check = any(strcmpi(tag, ppanalyzegroup_g_tag_list));
			else
				assert( ...
					PPAnalyzeGroup_G.existsTag(tag), ...
					[BRAPH2.STR ':PPAnalyzeGroup_G:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPAnalyzeGroup_G:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tag ' is not a valid tag for PPAnalyzeGroup_G'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(PPAnalyzeGroup_G, POINTER) returns property number of POINTER of PPAnalyzeGroup_G.
			%  PROPERTY = PR.GETPROPPROP(PPAnalyzeGroup_G, POINTER) returns property number of POINTER of PPAnalyzeGroup_G.
			%
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, ppanalyzegroup_g_tag_list));
			else % numeric
				prop = pointer;
				PPAnalyzeGroup_G.existsProp(prop);
			end
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(PPAnalyzeGroup_G, POINTER) returns tag of POINTER of PPAnalyzeGroup_G.
			%  TAG = PR.GETPROPTAG(PPAnalyzeGroup_G, POINTER) returns tag of POINTER of PPAnalyzeGroup_G.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(PPAnalyzeGroup_G, POINTER) returns category of POINTER of PPAnalyzeGroup_G.
			%  CATEGORY = PR.GETPROPCATEGORY(PPAnalyzeGroup_G, POINTER) returns category of POINTER of PPAnalyzeGroup_G.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
			
			prop = PPAnalyzeGroup_G.getPropProp(pointer);
		...
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(PPAnalyzeGroup_G, POINTER) returns format of POINTER of PPAnalyzeGroup_G.
			%  FORMAT = PR.GETPROPFORMAT(PPAnalyzeGroup_G, POINTER) returns format of POINTER of PPAnalyzeGroup_G.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(PPAnalyzeGroup_G, POINTER) returns description of POINTER of PPAnalyzeGroup_G.
			%  DESCRIPTION = PR.GETPROPDESCRIPTION(PPAnalyzeGroup_G, POINTER) returns description of POINTER of PPAnalyzeGroup_G.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(PPAnalyzeGroup_G, POINTER) returns settings of POINTER of PPAnalyzeGroup_G.
			%  SETTINGS = PR.GETPROPSETTINGS(PPAnalyzeGroup_G, POINTER) returns settings of POINTER of PPAnalyzeGroup_G.
			%
			% See also getPropProp, getPropTag, getPropCategory,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = PPAnalyzeGroup_G.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = PPAnalyzeGroup_G.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PR.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PR.
			%  DEFAULT = Element.GETPROPDEFAULT(PPAnalyzeGroup_G, POINTER) returns the default value of POINTER of PPAnalyzeGroup_G.
			%  DEFAULT = PR.GETPROPDEFAULT(PPAnalyzeGroup_G, POINTER) returns the default value of POINTER of PPAnalyzeGroup_G.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = PPAnalyzeGroup_G.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = PPAnalyzeGroup_G.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PR.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PR.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(PPAnalyzeGroup_G, POINTER) returns the conditioned default value of POINTER of PPAnalyzeGroup_G.
			%  DEFAULT = PR.GETPROPDEFAULTCONDITIONED(PPAnalyzeGroup_G, POINTER) returns the conditioned default value of POINTER of PPAnalyzeGroup_G.
			%
			...
			prop_default = PPAnalyzeGroup_G.conditioning(prop, PPAnalyzeGroup_G.getPropDefault(prop));
		end
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(PPAnalyzeGroup_G, PROP, VALUE) checks VALUE format for PROP of PPAnalyzeGroup_G.
			%  CHECK = PR.CHECKPROP(PPAnalyzeGroup_G, PROP, VALUE) checks VALUE format for PROP of PPAnalyzeGroup_G.
			% 
			...
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%  Element.CHECKPROP(PPAnalyzeGroup_G, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PPAnalyzeGroup_G.
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			%  PR.CHECKPROP(PPAnalyzeGroup_G, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PPAnalyzeGroup_G.
			%   Error id: [BRAPH2:PPAnalyzeGroup_G:WrongInput]
			% 
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':PPAnalyzeGroup_G:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPAnalyzeGroup_G:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' PPAnalyzeGroup_G.getPropTag(prop) ' (' PPAnalyzeGroup_G.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function pr = PPAnalyzeGroup_G(varargin)
			% PPAnalyzeGroup_G() creates a plot property graph.
			%
			% PPAnalyzeGroup_G(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% PPAnalyzeGroup_G(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
