classdef PPAnalyzeEnsemble_ME_DICT < PlotProp
	% PPAnalyzeEnsemble_ME_DICT is a plot of a measure ensemble property.
	...
	%
	% The list of PPAnalyzeEnsemble_ME_DICT properties is:
	...
	%
	% PPAnalyzeEnsemble_ME_DICT methods (constructor):
	% PPAnalyzeEnsemble_ME_DICT - constructor
	%
	% PPAnalyzeEnsemble_ME_DICT methods (Static):
	...
	%
	% PPAnalyzeEnsemble_ME_DICT methods:
	...
	%  cb_graph_adj_figure - _DICT/cb_graph_adj_figure is a function.
	%  cb_graph_ui_figure - E draws a new figure to manage a plot graph.
	%  cb_measure_calc - e_ME_DICT/cb_measure_calc is a function.
	...
	%  cb_button_del - ble_ME_DICT/cb_button_del is a function.
	%  cb_button_calc - le_ME_DICT/cb_button_calc is a function.
	%  cb_button_cb - mble_ME_DICT/cb_button_cb is a function.
	%  set_settings - mble_ME_DICT/set_settings is a function.
	%  cb_close_fs - emble_ME_DICT/cb_close_fs is a function.
	...
	%
	% PPAnalyzeEnsemble_ME_DICT methods (operators):
	%  isequal - determines whether two PPAnalyzeEnsemble_ME_DICT are equal (values, locked)
	%
	% PPAnalyzeEnsemble_ME_DICT methods (display):
	%  tostring - string with information about the PPAnalyzeEnsemble_ME_DICT
	%  disp - displays information about the PPAnalyzeEnsemble_ME_DICT
	%  tree - displays the element of PPAnalyzeEnsemble_ME_DICT
	%
	% PPAnalyzeEnsemble_ME_DICT method (element list):
	%  getElementList - returns a list with all subelements of PPAnalyzeEnsemble_ME_DICT
	%
	% PPAnalyzeEnsemble_ME_DICT method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the PPAnalyzeEnsemble_ME_DICT
	%
	% PPAnalyzeEnsemble_ME_DICT method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the PPAnalyzeEnsemble_ME_DICT
	%
	% PPAnalyzeEnsemble_ME_DICT methods (copy):
	%  copy - copies the PPAnalyzeEnsemble_ME_DICT
	%  deepclone - deep-clones the PPAnalyzeEnsemble_ME_DICT
	%  clone - clones the PPAnalyzeEnsemble_ME_DICT
	%
	% PPAnalyzeEnsemble_ME_DICT methods (inspection, Static):
	%  getClass - returns PPAnalyzeEnsemble_ME_DICT
	%  getName - returns the name of PPAnalyzeEnsemble_ME_DICT
	%  getDescription - returns the description of PPAnalyzeEnsemble_ME_DICT
	%  getProps - returns the property list of PPAnalyzeEnsemble_ME_DICT
	%  getPropNumber - returns the property number of PPAnalyzeEnsemble_ME_DICT
	...
	%  checkProp - checks whether a value has the correct format/error
	%
	% PPAnalyzeEnsemble_ME_DICT methods (category, Static):
	...
	%
	% PPAnalyzeEnsemble_ME_DICT methods (format, Static):
	...
	%
	% PPAnalyzeEnsemble_ME_DICT methods (GUI):
	%  getGUI - returns the element GUI
	%  getPlotElement - returns the element plot
	%  getPlotProp - returns a prop plot
	%
	% PPAnalyzeEnsemble_ME_DICT methods (GUI, Static):
	%  getGUIMenuImport - returns an importer menu
	%  getGUIMenuExport - returns an exporter menu
	%
	% PPAnalyzeEnsemble_ME_DICT properties (Constant).
	%  EL - 3
	%  EL_TAG - 'el'
	%  EL_CATEGORY - 'm'
	%  EL_FORMAT - 'it'
	...
	methods (Static) % inspection methods
		function pr_class = getClass()
			%GETCLASS returns the class of the plot property graph.
			%
			% CLASS = PPAnalyzeEnsemble_ME_DICT.GETCLASS() returns the class 'PPAnalyzeEnsemble_ME_DICT'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PR.GETCLASS() returns the class of the plot property graph PR.
			%  CLASS = Element.GETCLASS(PR) returns the class of 'PR'.
			%  CLASS = Element.GETCLASS('PPAnalyzeEnsemble_ME_DICT') returns 'PPAnalyzeEnsemble_ME_DICT'.
			%
			% See also getName, getDescription.
			
			pr_class = 'PPAnalyzeEnsemble_ME_DICT';
		end
		function pr_name = getName()
			%GETNAME returns the name of the plot property graph.
			%
			% NAME = PPAnalyzeEnsemble_ME_DICT.GETNAME() returns the name of the 'plot property graph'.
			%  Plot Property Graph.
			%
			% Alternative forms to call this method are:
			%  NAME = PR.GETNAME() returns the name of the plot property graph PR.
			%  NAME = Element.GETNAME(PR) returns the name of 'PR'.
			%  NAME = Element.GETNAME('PPAnalyzeEnsemble_ME_DICT') returns the name of 'PPAnalyzeEnsemble_ME_DICT'.
			%
		...
		function pr_description = getDescription()
			%GETDESCRIPTION returns the description of the plot property graph.
			%
			% STR = PPAnalyzeEnsemble_ME_DICT.GETDESCRIPTION() returns the description of the 'plot property graph'.
			%  which is:
			%
			%  PPAnalyzeGroup_ME_DICT plots the measure table associated with a measure ensemble of the analysis.
			%  It also provides the buttons to navigate the graphical interface of both
			...
			%  STR = Element.GETDESCRIPTION('PPAnalyzeEnsemble_ME_DICT') returns the description of 'PPAnalyzeEnsemble_ME_DICT'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of plot property graph.
			%
			% PROPS = PPAnalyzeEnsemble_ME_DICT.GETPROPS() returns the property list of plot property graph.
			%
			% PROPS = PPAnalyzeEnsemble_ME_DICT.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the plot property graph PR.
			%  PROPS = Element.GETPROPS(PR[, CATEGORY]) returns the property list of 'PR'.
			%  PROPS = Element.GETPROPS('PPAnalyzeEnsemble_ME_DICT'[, CATEGORY]) returns the property list of 'PPAnalyzeEnsemble_ME_DICT'.
			%
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of plot property graph.
			%
			% N = PPAnalyzeEnsemble_ME_DICT.GETPROPNUMBER() returns the property number of plot property graph.
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER() returns the property number of the plot property graph PR.
			%  N = Element.GETPROPNUMBER(PR) returns the property number of 'PR'.
			%  N = Element.GETPROPNUMBER('PPAnalyzeEnsemble_ME_DICT') returns the property number of 'PPAnalyzeEnsemble_ME_DICT'.
			%
			% See also getProps.
		...
		function check = existsProp(prop)
			%EXISTSPROP checks whether property exists in plot property graph/error.
			%
			% CHECK = PPAnalyzeEnsemble_ME_DICT.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PR.EXISTSPROP(PROP) checks whether PROP exists for PR.
			%  CHECK = Element.EXISTSPROP(PR, PROP) checks whether PROP exists for PR.
			%  CHECK = Element.EXISTSPROP(PPAnalyzeEnsemble_ME_DICT, PROP) checks whether PROP exists for PPAnalyzeEnsemble_ME_DICT.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%  Element.EXISTSPROP(PPAnalyzeEnsemble_ME_DICT, PROP) throws error if PROP does NOT exist for PPAnalyzeEnsemble_ME_DICT.
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%
			...
			if nargout == 1
				...
				assert( ...
					PPAnalyzeEnsemble_ME_DICT.existsProp(prop), ...
					[BRAPH2.STR ':PPAnalyzeEnsemble_ME_DICT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPAnalyzeEnsemble_ME_DICT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for PPAnalyzeEnsemble_ME_DICT.'] ...
					)
			end
		end
		function check = existsTag(tag)
			%EXISTSTAG checks whether tag exists in plot property graph/error.
			%
			% CHECK = PPAnalyzeEnsemble_ME_DICT.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PR.EXISTSTAG(TAG) checks whether TAG exists for PR.
			%  CHECK = Element.EXISTSTAG(PR, TAG) checks whether TAG exists for PR.
			%  CHECK = Element.EXISTSTAG(PPAnalyzeEnsemble_ME_DICT, TAG) checks whether TAG exists for PPAnalyzeEnsemble_ME_DICT.
			%
			...
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%  Element.EXISTSTAG(PPAnalyzeEnsemble_ME_DICT, TAG) throws error if TAG does NOT exist for PPAnalyzeEnsemble_ME_DICT.
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%
			% See also getProps, existsTag.
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				ppanalyzeensemble_me_dict_tag_list = { 'id'  'bkgcolor'  'el'  'prop'  'title'  'enable'  'waitbar' };
				
				check = any(strcmpi(tag, ppanalyzeensemble_me_dict_tag_list));
			else
				assert( ...
					PPAnalyzeEnsemble_ME_DICT.existsTag(tag), ...
					[BRAPH2.STR ':PPAnalyzeEnsemble_ME_DICT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPAnalyzeEnsemble_ME_DICT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tag ' is not a valid tag for PPAnalyzeEnsemble_ME_DICT'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(PPAnalyzeEnsemble_ME_DICT, POINTER) returns property number of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  PROPERTY = PR.GETPROPPROP(PPAnalyzeEnsemble_ME_DICT, POINTER) returns property number of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, ppanalyzeensemble_me_dict_tag_list));
			else % numeric
				prop = pointer;
				PPAnalyzeEnsemble_ME_DICT.existsProp(prop);
			end
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(PPAnalyzeEnsemble_ME_DICT, POINTER) returns tag of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  TAG = PR.GETPROPTAG(PPAnalyzeEnsemble_ME_DICT, POINTER) returns tag of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(PPAnalyzeEnsemble_ME_DICT, POINTER) returns category of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  CATEGORY = PR.GETPROPCATEGORY(PPAnalyzeEnsemble_ME_DICT, POINTER) returns category of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
			
			prop = PPAnalyzeEnsemble_ME_DICT.getPropProp(pointer);
		...
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(PPAnalyzeEnsemble_ME_DICT, POINTER) returns format of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  FORMAT = PR.GETPROPFORMAT(PPAnalyzeEnsemble_ME_DICT, POINTER) returns format of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(PPAnalyzeEnsemble_ME_DICT, POINTER) returns description of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  DESCRIPTION = PR.GETPROPDESCRIPTION(PPAnalyzeEnsemble_ME_DICT, POINTER) returns description of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
			...
			switch prop
				case PPAnalyzeEnsemble_ME_DICT.ENABLE
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(PPAnalyzeEnsemble_ME_DICT, POINTER) returns settings of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  SETTINGS = PR.GETPROPSETTINGS(PPAnalyzeEnsemble_ME_DICT, POINTER) returns settings of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
			% See also getPropProp, getPropTag, getPropCategory,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = PPAnalyzeEnsemble_ME_DICT.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = PPAnalyzeEnsemble_ME_DICT.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PR.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PR.
			%  DEFAULT = Element.GETPROPDEFAULT(PPAnalyzeEnsemble_ME_DICT, POINTER) returns the default value of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  DEFAULT = PR.GETPROPDEFAULT(PPAnalyzeEnsemble_ME_DICT, POINTER) returns the default value of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = PPAnalyzeEnsemble_ME_DICT.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = PPAnalyzeEnsemble_ME_DICT.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PR.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PR.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(PPAnalyzeEnsemble_ME_DICT, POINTER) returns the conditioned default value of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%  DEFAULT = PR.GETPROPDEFAULTCONDITIONED(PPAnalyzeEnsemble_ME_DICT, POINTER) returns the conditioned default value of POINTER of PPAnalyzeEnsemble_ME_DICT.
			%
			...
			prop_default = PPAnalyzeEnsemble_ME_DICT.conditioning(prop, PPAnalyzeEnsemble_ME_DICT.getPropDefault(prop));
		end
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(PPAnalyzeEnsemble_ME_DICT, PROP, VALUE) checks VALUE format for PROP of PPAnalyzeEnsemble_ME_DICT.
			%  CHECK = PR.CHECKPROP(PPAnalyzeEnsemble_ME_DICT, PROP, VALUE) checks VALUE format for PROP of PPAnalyzeEnsemble_ME_DICT.
			% 
			...
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%  Element.CHECKPROP(PPAnalyzeEnsemble_ME_DICT, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PPAnalyzeEnsemble_ME_DICT.
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			%  PR.CHECKPROP(PPAnalyzeEnsemble_ME_DICT, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PPAnalyzeEnsemble_ME_DICT.
			%   Error id: [BRAPH2:PPAnalyzeEnsemble_ME_DICT:WrongInput]
			% 
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':PPAnalyzeEnsemble_ME_DICT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPAnalyzeEnsemble_ME_DICT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' PPAnalyzeEnsemble_ME_DICT.getPropTag(prop) ' (' PPAnalyzeEnsemble_ME_DICT.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function pr = PPAnalyzeEnsemble_ME_DICT(varargin)
			% PPAnalyzeEnsemble_ME_DICT() creates a plot property graph.
			%
			% PPAnalyzeEnsemble_ME_DICT(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% PPAnalyzeEnsemble_ME_DICT(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
