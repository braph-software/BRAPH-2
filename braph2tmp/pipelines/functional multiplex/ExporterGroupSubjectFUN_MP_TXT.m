classdef ExporterGroupSubjectFUN_MP_TXT < Exporter
	...
	%  <strong>7</strong> <strong>save</strong> 	SAVE (result, empty) saves the group of subjects with functional multiplex data in TXT files in the selected directory.
	%
	% ExporterGroupSubjectFUN_MP_TXT methods (constructor):
	% ExporterGroupSubjectFUN_MP_TXT - constructor
	%
	% ExporterGroupSubjectFUN_MP_TXT methods (Static):
	...
	%  getDescription - returns the description of the exporter of FUN MP subject group in TXT.
	%  getName - returns the name of the exporter of FUN MP subject group in TXT.
	%  getClass - returns the class of the exporter of FUN MP subject group in TXT.
	%
	% ExporterGroupSubjectFUN_MP_TXT methods:
	%  uigetdir - R opens a dialog box to set the directory where to save the group of subjects with functional multiplex data.
	...
	%
	% ExporterGroupSubjectFUN_MP_TXT methods (operators):
	%  isequal - determines whether two ExporterGroupSubjectFUN_MP_TXT are equal (values, locked)
	%
	% ExporterGroupSubjectFUN_MP_TXT methods (display):
	%  tostring - string with information about the ExporterGroupSubjectFUN_MP_TXT
	%  disp - displays information about the ExporterGroupSubjectFUN_MP_TXT
	%  tree - displays the element of ExporterGroupSubjectFUN_MP_TXT
	%
	% ExporterGroupSubjectFUN_MP_TXT method (element list):
	%  getElementList - returns a list with all subelements of ExporterGroupSubjectFUN_MP_TXT
	%
	% ExporterGroupSubjectFUN_MP_TXT method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the ExporterGroupSubjectFUN_MP_TXT
	%
	% ExporterGroupSubjectFUN_MP_TXT method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the ExporterGroupSubjectFUN_MP_TXT
	%
	% ExporterGroupSubjectFUN_MP_TXT methods (copy):
	%  copy - copies the ExporterGroupSubjectFUN_MP_TXT
	%  deepclone - deep-clones the ExporterGroupSubjectFUN_MP_TXT
	%  clone - clones the ExporterGroupSubjectFUN_MP_TXT
	%
	% ExporterGroupSubjectFUN_MP_TXT methods (inspection, Static):
	%  getClass - returns ExporterGroupSubjectFUN_MP_TXT
	%  getName - returns the name of ExporterGroupSubjectFUN_MP_TXT
	%  getDescription - returns the description of ExporterGroupSubjectFUN_MP_TXT
	%  getProps - returns the property list of ExporterGroupSubjectFUN_MP_TXT
	%  getPropNumber - returns the property number of ExporterGroupSubjectFUN_MP_TXT
	...
	%
	% ExporterGroupSubjectFUN_MP_TXT properties (Constant).
	%  GR - 5
	%  GR_TAG - 'gr'
	%  GR_CATEGORY - 'd'
	%  GR_FORMAT - 'it'
	...
	%
	% See also Element, Exporter, ImporterGroupSubjectFUN_MP_TXT.
	...
	methods (Static) % inspection methods
		...
		function ex_name = getName()
			%GETNAME returns the name of the exporter of FUN MP subject group in TXT.
			%
			% NAME = ExporterGroupSubjectFUN_MP_TXT.GETNAME() returns the name of the 'exporter of FUN MP subject group in TXT'.
			%  Exporter Of FUN MP Subject Group In TXT.
			%
			% Alternative forms to call this method are:
			%  NAME = EX.GETNAME() returns the name of the exporter of FUN MP subject group in TXT EX.
			%  NAME = Element.GETNAME(EX) returns the name of 'EX'.
			%  NAME = Element.GETNAME('ExporterGroupSubjectFUN_MP_TXT') returns the name of 'ExporterGroupSubjectFUN_MP_TXT'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			ex_name = 'Exporter Of FUN MP Subject Group In TXT';
		end
		function ex_description = getDescription()
			%GETDESCRIPTION returns the description of the exporter of FUN MP subject group in TXT.
			%
			% STR = ExporterGroupSubjectFUN_MP_TXT.GETDESCRIPTION() returns the description of the 'exporter of FUN MP subject group in TXT'.
			%  which is:
			%
			%  ExporterGroupSubjectFUN_MP_TXT exports a group of subjects with functional multiplex data to a series of TXT file and their covariates age and sex (if existing) to another TXT file.
			%  All these files are saved in the same folder.
			...
			%
			% Alternative forms to call this method are:
			%  STR = EX.GETDESCRIPTION() returns the description of the exporter of FUN MP subject group in TXT EX.
			%  STR = Element.GETDESCRIPTION(EX) returns the description of 'EX'.
			%  STR = Element.GETDESCRIPTION('ExporterGroupSubjectFUN_MP_TXT') returns the description of 'ExporterGroupSubjectFUN_MP_TXT'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = EX.GETPROPNUMBER() returns the property number of the exporter of FUN MP subject group in TXT EX.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					ExporterGroupSubjectFUN_MP_TXT.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				exportergroupsubjectfun_mp_txt_tag_list = { 'id'  'label'  'notes'  'waitbar'  'gr'  'directory'  'save' };
				
				check = any(strcmpi(tag, exportergroupsubjectfun_mp_txt_tag_list));
			else
				assert( ...
					ExporterGroupSubjectFUN_MP_TXT.existsTag(tag), ...
					[BRAPH2.STR ':ExporterGroupSubjectFUN_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectFUN_MP_TXT:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, exportergroupsubjectfun_mp_txt_tag_list));
			else % numeric
				prop = pointer;
				ExporterGroupSubjectFUN_MP_TXT.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':ExporterGroupSubjectFUN_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectFUN_MP_TXT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ExporterGroupSubjectFUN_MP_TXT.getPropTag(prop) ' (' ExporterGroupSubjectFUN_MP_TXT.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function ex = ExporterGroupSubjectFUN_MP_TXT(varargin)
			...
			%
			% ExporterGroupSubjectFUN_MP_TXT(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(ex, prop)
			...
			switch prop
				case ExporterGroupSubjectFUN_MP_TXT.SAVE
					rng(ex.getPropSeed(ExporterGroupSubjectFUN_MP_TXT.SAVE), 'twister')
