classdef PPCompareGroup_CPDict_ST_BUD < PlotProp
	% PPCompareGroup_CPDict_ST_BUD is a plot of a binary undirect using densities comparison dictionary.
	...
	%
	% The list of PPCompareGroup_CPDict_ST_BUD properties is:
	...
	%
	% PPCompareGroup_CPDict_ST_BUD methods (constructor):
	% PPCompareGroup_CPDict_ST_BUD - constructor
	%
	% PPCompareGroup_CPDict_ST_BUD methods (Static):
	...
	%
	% PPCompareGroup_CPDict_ST_BUD methods:
	...
	%  cb_measure_calc - PDict_ST_BUD/cb_measure_calc is a function.
	...
	%  cb_button_del - _CPDict_ST_BUD/cb_button_del is a function.
	%  cb_button_calc - CPDict_ST_BUD/cb_button_calc is a function.
	%  cb_button_cb - p_CPDict_ST_BUD/cb_button_cb is a function.
	%  set_settings - p_CPDict_ST_BUD/set_settings is a function.
	%  cb_close_fs - up_CPDict_ST_BUD/cb_close_fs is a function.
	...
	%
	% PPCompareGroup_CPDict_ST_BUD methods (operators):
	%  isequal - determines whether two PPCompareGroup_CPDict_ST_BUD are equal (values, locked)
	%
	% PPCompareGroup_CPDict_ST_BUD methods (display):
	%  tostring - string with information about the PPCompareGroup_CPDict_ST_BUD
	%  disp - displays information about the PPCompareGroup_CPDict_ST_BUD
	%  tree - displays the element of PPCompareGroup_CPDict_ST_BUD
	%
	% PPCompareGroup_CPDict_ST_BUD method (element list):
	%  getElementList - returns a list with all subelements of PPCompareGroup_CPDict_ST_BUD
	%
	% PPCompareGroup_CPDict_ST_BUD method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the PPCompareGroup_CPDict_ST_BUD
	%
	% PPCompareGroup_CPDict_ST_BUD method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the PPCompareGroup_CPDict_ST_BUD
	%
	% PPCompareGroup_CPDict_ST_BUD methods (copy):
	%  copy - copies the PPCompareGroup_CPDict_ST_BUD
	%  deepclone - deep-clones the PPCompareGroup_CPDict_ST_BUD
	%  clone - clones the PPCompareGroup_CPDict_ST_BUD
	%
	% PPCompareGroup_CPDict_ST_BUD methods (inspection, Static):
	%  getClass - returns PPCompareGroup_CPDict_ST_BUD
	%  getName - returns the name of PPCompareGroup_CPDict_ST_BUD
	%  getDescription - returns the description of PPCompareGroup_CPDict_ST_BUD
	%  getProps - returns the property list of PPCompareGroup_CPDict_ST_BUD
	%  getPropNumber - returns the property number of PPCompareGroup_CPDict_ST_BUD
	...
	%  checkProp - checks whether a value has the correct format/error
	%
	% PPCompareGroup_CPDict_ST_BUD methods (category, Static):
	...
	%
	% PPCompareGroup_CPDict_ST_BUD methods (format, Static):
	...
	%
	% PPCompareGroup_CPDict_ST_BUD methods (GUI):
	%  getGUI - returns the element GUI
	%  getPlotElement - returns the element plot
	%  getPlotProp - returns a prop plot
	%
	% PPCompareGroup_CPDict_ST_BUD methods (GUI, Static):
	%  getGUIMenuImport - returns an importer menu
	%  getGUIMenuExport - returns an exporter menu
	%
	% PPCompareGroup_CPDict_ST_BUD properties (Constant).
	%  EL - 3
	%  EL_TAG - 'el'
	%  EL_CATEGORY - 'm'
	%  EL_FORMAT - 'it'
	...
	methods (Static) % inspection methods
		function pr_class = getClass()
			%GETCLASS returns the class of the plot property graph.
			%
			% CLASS = PPCompareGroup_CPDict_ST_BUD.GETCLASS() returns the class 'PPCompareGroup_CPDict_ST_BUD'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PR.GETCLASS() returns the class of the plot property graph PR.
			%  CLASS = Element.GETCLASS(PR) returns the class of 'PR'.
			%  CLASS = Element.GETCLASS('PPCompareGroup_CPDict_ST_BUD') returns 'PPCompareGroup_CPDict_ST_BUD'.
			%
			% See also getName, getDescription.
			
			pr_class = 'PPCompareGroup_CPDict_ST_BUD';
		end
		function pr_name = getName()
			%GETNAME returns the name of the plot property graph.
			%
			% NAME = PPCompareGroup_CPDict_ST_BUD.GETNAME() returns the name of the 'plot property graph'.
			%  Plot Property Graph.
			%
			% Alternative forms to call this method are:
			%  NAME = PR.GETNAME() returns the name of the plot property graph PR.
			%  NAME = Element.GETNAME(PR) returns the name of 'PR'.
			%  NAME = Element.GETNAME('PPCompareGroup_CPDict_ST_BUD') returns the name of 'PPCompareGroup_CPDict_ST_BUD'.
			%
		...
		function pr_description = getDescription()
			%GETDESCRIPTION returns the description of the plot property graph.
			%
			% STR = PPCompareGroup_CPDict_ST_BUD.GETDESCRIPTION() returns the description of the 'plot property graph'.
			%  which is:
			%
			%  PPCompareGroup_CPDict_BUD plots the binary undirect using densities comparison 
			%  dictionary property associated with a graph.
			...
			%  STR = Element.GETDESCRIPTION('PPCompareGroup_CPDict_ST_BUD') returns the description of 'PPCompareGroup_CPDict_ST_BUD'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of plot property graph.
			%
			% PROPS = PPCompareGroup_CPDict_ST_BUD.GETPROPS() returns the property list of plot property graph.
			%
			% PROPS = PPCompareGroup_CPDict_ST_BUD.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the plot property graph PR.
			%  PROPS = Element.GETPROPS(PR[, CATEGORY]) returns the property list of 'PR'.
			%  PROPS = Element.GETPROPS('PPCompareGroup_CPDict_ST_BUD'[, CATEGORY]) returns the property list of 'PPCompareGroup_CPDict_ST_BUD'.
			%
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of plot property graph.
			%
			% N = PPCompareGroup_CPDict_ST_BUD.GETPROPNUMBER() returns the property number of plot property graph.
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER() returns the property number of the plot property graph PR.
			%  N = Element.GETPROPNUMBER(PR) returns the property number of 'PR'.
			%  N = Element.GETPROPNUMBER('PPCompareGroup_CPDict_ST_BUD') returns the property number of 'PPCompareGroup_CPDict_ST_BUD'.
			%
			% See also getProps.
		...
		function check = existsProp(prop)
			%EXISTSPROP checks whether property exists in plot property graph/error.
			%
			% CHECK = PPCompareGroup_CPDict_ST_BUD.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PR.EXISTSPROP(PROP) checks whether PROP exists for PR.
			%  CHECK = Element.EXISTSPROP(PR, PROP) checks whether PROP exists for PR.
			%  CHECK = Element.EXISTSPROP(PPCompareGroup_CPDict_ST_BUD, PROP) checks whether PROP exists for PPCompareGroup_CPDict_ST_BUD.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%  Element.EXISTSPROP(PPCompareGroup_CPDict_ST_BUD, PROP) throws error if PROP does NOT exist for PPCompareGroup_CPDict_ST_BUD.
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%
			...
			if nargout == 1
				...
				assert( ...
					PPCompareGroup_CPDict_ST_BUD.existsProp(prop), ...
					[BRAPH2.STR ':PPCompareGroup_CPDict_ST_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPCompareGroup_CPDict_ST_BUD:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for PPCompareGroup_CPDict_ST_BUD.'] ...
					)
			end
		end
		function check = existsTag(tag)
			%EXISTSTAG checks whether tag exists in plot property graph/error.
			%
			% CHECK = PPCompareGroup_CPDict_ST_BUD.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PR.EXISTSTAG(TAG) checks whether TAG exists for PR.
			%  CHECK = Element.EXISTSTAG(PR, TAG) checks whether TAG exists for PR.
			%  CHECK = Element.EXISTSTAG(PPCompareGroup_CPDict_ST_BUD, TAG) checks whether TAG exists for PPCompareGroup_CPDict_ST_BUD.
			%
			...
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%  Element.EXISTSTAG(PPCompareGroup_CPDict_ST_BUD, TAG) throws error if TAG does NOT exist for PPCompareGroup_CPDict_ST_BUD.
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%
			% See also getProps, existsTag.
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				ppcomparegroup_cpdict_st_bud_tag_list = { 'id'  'bkgcolor'  'el'  'prop'  'title'  'enable'  'waitbar' };
				
				check = any(strcmpi(tag, ppcomparegroup_cpdict_st_bud_tag_list));
			else
				assert( ...
					PPCompareGroup_CPDict_ST_BUD.existsTag(tag), ...
					[BRAPH2.STR ':PPCompareGroup_CPDict_ST_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPCompareGroup_CPDict_ST_BUD:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tag ' is not a valid tag for PPCompareGroup_CPDict_ST_BUD'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(PPCompareGroup_CPDict_ST_BUD, POINTER) returns property number of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  PROPERTY = PR.GETPROPPROP(PPCompareGroup_CPDict_ST_BUD, POINTER) returns property number of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, ppcomparegroup_cpdict_st_bud_tag_list));
			else % numeric
				prop = pointer;
				PPCompareGroup_CPDict_ST_BUD.existsProp(prop);
			end
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(PPCompareGroup_CPDict_ST_BUD, POINTER) returns tag of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  TAG = PR.GETPROPTAG(PPCompareGroup_CPDict_ST_BUD, POINTER) returns tag of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(PPCompareGroup_CPDict_ST_BUD, POINTER) returns category of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  CATEGORY = PR.GETPROPCATEGORY(PPCompareGroup_CPDict_ST_BUD, POINTER) returns category of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
			
			prop = PPCompareGroup_CPDict_ST_BUD.getPropProp(pointer);
		...
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(PPCompareGroup_CPDict_ST_BUD, POINTER) returns format of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  FORMAT = PR.GETPROPFORMAT(PPCompareGroup_CPDict_ST_BUD, POINTER) returns format of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(PPCompareGroup_CPDict_ST_BUD, POINTER) returns description of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  DESCRIPTION = PR.GETPROPDESCRIPTION(PPCompareGroup_CPDict_ST_BUD, POINTER) returns description of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(PPCompareGroup_CPDict_ST_BUD, POINTER) returns settings of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  SETTINGS = PR.GETPROPSETTINGS(PPCompareGroup_CPDict_ST_BUD, POINTER) returns settings of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
			% See also getPropProp, getPropTag, getPropCategory,
			% getPropFormat, getPropDescription, getPropDefault,
			% checkProp.
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = PPCompareGroup_CPDict_ST_BUD.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = PPCompareGroup_CPDict_ST_BUD.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PR.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PR.
			%  DEFAULT = Element.GETPROPDEFAULT(PPCompareGroup_CPDict_ST_BUD, POINTER) returns the default value of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  DEFAULT = PR.GETPROPDEFAULT(PPCompareGroup_CPDict_ST_BUD, POINTER) returns the default value of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = PPCompareGroup_CPDict_ST_BUD.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = PPCompareGroup_CPDict_ST_BUD.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PR.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PR.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(PPCompareGroup_CPDict_ST_BUD, POINTER) returns the conditioned default value of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%  DEFAULT = PR.GETPROPDEFAULTCONDITIONED(PPCompareGroup_CPDict_ST_BUD, POINTER) returns the conditioned default value of POINTER of PPCompareGroup_CPDict_ST_BUD.
			%
			...
			prop_default = PPCompareGroup_CPDict_ST_BUD.conditioning(prop, PPCompareGroup_CPDict_ST_BUD.getPropDefault(prop));
		end
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(PPCompareGroup_CPDict_ST_BUD, PROP, VALUE) checks VALUE format for PROP of PPCompareGroup_CPDict_ST_BUD.
			%  CHECK = PR.CHECKPROP(PPCompareGroup_CPDict_ST_BUD, PROP, VALUE) checks VALUE format for PROP of PPCompareGroup_CPDict_ST_BUD.
			% 
			...
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%  Element.CHECKPROP(PPCompareGroup_CPDict_ST_BUD, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PPCompareGroup_CPDict_ST_BUD.
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			%  PR.CHECKPROP(PPCompareGroup_CPDict_ST_BUD, PROP, VALUE) throws error if VALUE has not a valid format for PROP of PPCompareGroup_CPDict_ST_BUD.
			%   Error id: [BRAPH2:PPCompareGroup_CPDict_ST_BUD:WrongInput]
			% 
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':PPCompareGroup_CPDict_ST_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':PPCompareGroup_CPDict_ST_BUD:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' PPCompareGroup_CPDict_ST_BUD.getPropTag(prop) ' (' PPCompareGroup_CPDict_ST_BUD.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function pr = PPCompareGroup_CPDict_ST_BUD(varargin)
			% PPCompareGroup_CPDict_ST_BUD() creates a plot property graph.
			%
			% PPCompareGroup_CPDict_ST_BUD(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% PPCompareGroup_CPDict_ST_BUD(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
		...
		function update(pr)
		    ...
		    if ~button_state
		        ...
		        function plot_type_rules()
		            if ~isempty(pr.graph) && isa(el.get('A1'), 'AnalyzeGroup_ST_BUD') && ~isempty(pr.already_calculated) && any([pr.already_calculated{:}])
