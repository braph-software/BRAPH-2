classdef AnalyzeGroup_ST_BUT < AnalyzeGroup
	% AnalyzeGroup_ST_BUT is a graph analysis using structural data of fixed threshold.
	...
	%
	% AnalyzeGroup_ST_BUT methods (Static):
	...
	%  existsTag - checks whether tag exists in graph analysis with structural data of fixed threshold/error.
	%  existsProp - checks whether property exists in graph analysis with structural data of fixed threshold/error.
	%  getPropNumber - returns the property number of graph analysis with structural data of fixed threshold.
	%  getProps - returns the property list of graph analysis with structural data of fixed threshold.
	%  getDescription - returns the description of the graph analysis with structural data of fixed threshold.
	%  getName - returns the name of the graph analysis with structural data of fixed threshold.
	%  getClass - returns the class of the graph analysis with structural data of fixed threshold.
	%
	...
	%
	% AnalyzeGroup_ST_BUT methods (operators):
	%  isequal - determines whether two AnalyzeGroup_ST_BUT are equal (values, locked)
	%
	% AnalyzeGroup_ST_BUT methods (display):
	%  tostring - string with information about the AnalyzeGroup_ST_BUT
	%  disp - displays information about the AnalyzeGroup_ST_BUT
	%  tree - displays the element of AnalyzeGroup_ST_BUT
	%
	% AnalyzeGroup_ST_BUT method (element list):
	%  getElementList - returns a list with all subelements of AnalyzeGroup_ST_BUT
	%
	% AnalyzeGroup_ST_BUT method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the AnalyzeGroup_ST_BUT
	%
	% AnalyzeGroup_ST_BUT method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the AnalyzeGroup_ST_BUT
	%
	% AnalyzeGroup_ST_BUT methods (copy):
	%  copy - copies the AnalyzeGroup_ST_BUT
	%  deepclone - deep-clones the AnalyzeGroup_ST_BUT
	%  clone - clones the AnalyzeGroup_ST_BUT
	%
	% AnalyzeGroup_ST_BUT methods (inspection, Static):
	%  getClass - returns AnalyzeGroup_ST_BUT
	%  getName - returns the name of AnalyzeGroup_ST_BUT
	%  getDescription - returns the description of AnalyzeGroup_ST_BUT
	%  getProps - returns the property list of AnalyzeGroup_ST_BUT
	%  getPropNumber - returns the property number of AnalyzeGroup_ST_BUT
	...
	%
	% AnalyzeGroup_ST_BUT properties (Constant).
	%  CORRELATION_RULE - 6
	...
	methods (Static) % inspection methods
		function a_class = getClass()
			...
			%  CLASS = A.GETCLASS() returns the class of the graph analysis with structural data of fixed threshold A.
		...
		function a_name = getName()
			%GETNAME returns the name of the graph analysis with structural data of fixed threshold.
			%
			% NAME = AnalyzeGroup_ST_BUT.GETNAME() returns the name of the 'graph analysis with structural data of fixed threshold'.
			%  Graph Analysis With Structural Data Of Fixed Threshold.
			%
			% Alternative forms to call this method are:
			%  NAME = A.GETNAME() returns the name of the graph analysis with structural data of fixed threshold A.
			%  NAME = Element.GETNAME(A) returns the name of 'A'.
			%  NAME = Element.GETNAME('AnalyzeGroup_ST_BUT') returns the name of 'AnalyzeGroup_ST_BUT'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			a_name = 'Graph Analysis With Structural Data Of Fixed Threshold';
		end
		function a_description = getDescription()
			%GETDESCRIPTION returns the description of the graph analysis with structural data of fixed threshold.
			%
			% STR = AnalyzeGroup_ST_BUT.GETDESCRIPTION() returns the description of the 'graph analysis with structural data of fixed threshold'.
			%  which is:
			%
			%  
			%
			% Alternative forms to call this method are:
			%  STR = A.GETDESCRIPTION() returns the description of the graph analysis with structural data of fixed threshold A.
			%  STR = Element.GETDESCRIPTION(A) returns the description of 'A'.
			%  STR = Element.GETDESCRIPTION('AnalyzeGroup_ST_BUT') returns the description of 'AnalyzeGroup_ST_BUT'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of graph analysis with structural data of fixed threshold.
			%
			% PROPS = AnalyzeGroup_ST_BUT.GETPROPS() returns the property list of graph analysis with structural data of fixed threshold.
			%
			% PROPS = AnalyzeGroup_ST_BUT.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = A.GETPROPS([CATEGORY]) returns the property list of the graph analysis with structural data of fixed threshold A.
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of graph analysis with structural data of fixed threshold.
			%
			% N = AnalyzeGroup_ST_BUT.GETPROPNUMBER() returns the property number of graph analysis with structural data of fixed threshold.
			%
			% Alternative forms to call this method are:
			%  N = A.GETPROPNUMBER() returns the property number of the graph analysis with structural data of fixed threshold A.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					AnalyzeGroup_ST_BUT.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				analyzegroup_st_but_tag_list = { 'id'  'label'  'notes'  'gr'  'g'  'correlation_rule'  'negative_weight_rule'  'thresholds' };
				
				check = any(strcmpi(tag, analyzegroup_st_but_tag_list));
			else
				assert( ...
					AnalyzeGroup_ST_BUT.existsTag(tag), ...
					[BRAPH2.STR ':AnalyzeGroup_ST_BUT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeGroup_ST_BUT:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, analyzegroup_st_but_tag_list));
			else % numeric
				prop = pointer;
				AnalyzeGroup_ST_BUT.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':AnalyzeGroup_ST_BUT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeGroup_ST_BUT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeGroup_ST_BUT.getPropTag(prop) ' (' AnalyzeGroup_ST_BUT.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function a = AnalyzeGroup_ST_BUT(varargin)
			% AnalyzeGroup_ST_BUT() creates a graph analysis with structural data of fixed threshold.
			%
			% AnalyzeGroup_ST_BUT(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% AnalyzeGroup_ST_BUT(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
