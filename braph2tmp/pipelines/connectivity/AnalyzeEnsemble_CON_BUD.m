classdef AnalyzeEnsemble_CON_BUD < AnalyzeEnsemble
	% AnalyzeEnsemble_CON_BUD is a graph analysis using connectivity data of fixed density.
	...
	%  <strong>7</strong> <strong>densities</strong> 	DENSITIES (parameter, rvector) is the vector of densities.
	%
	% AnalyzeEnsemble_CON_BUD methods (constructor):
	% AnalyzeEnsemble_CON_BUD - constructor
	%
	% AnalyzeEnsemble_CON_BUD methods (Static):
	...
	%  getDescription - returns the description of the graph analysis with connectivity data of fixed density.
	%  getName - returns the name of the graph analysis with connectivity data of fixed density.
	...
	%
	% AnalyzeEnsemble_CON_BUD methods (operators):
	%  isequal - determines whether two AnalyzeEnsemble_CON_BUD are equal (values, locked)
	%
	% AnalyzeEnsemble_CON_BUD methods (display):
	%  tostring - string with information about the AnalyzeEnsemble_CON_BUD
	%  disp - displays information about the AnalyzeEnsemble_CON_BUD
	%  tree - displays the element of AnalyzeEnsemble_CON_BUD
	%
	% AnalyzeEnsemble_CON_BUD method (element list):
	%  getElementList - returns a list with all subelements of AnalyzeEnsemble_CON_BUD
	%
	% AnalyzeEnsemble_CON_BUD method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the AnalyzeEnsemble_CON_BUD
	%
	% AnalyzeEnsemble_CON_BUD method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the AnalyzeEnsemble_CON_BUD
	%
	% AnalyzeEnsemble_CON_BUD methods (copy):
	%  copy - copies the AnalyzeEnsemble_CON_BUD
	%  deepclone - deep-clones the AnalyzeEnsemble_CON_BUD
	%  clone - clones the AnalyzeEnsemble_CON_BUD
	%
	% AnalyzeEnsemble_CON_BUD methods (inspection, Static):
	%  getClass - returns AnalyzeEnsemble_CON_BUD
	%  getName - returns the name of AnalyzeEnsemble_CON_BUD
	%  getDescription - returns the description of AnalyzeEnsemble_CON_BUD
	%  getProps - returns the property list of AnalyzeEnsemble_CON_BUD
	%  getPropNumber - returns the property number of AnalyzeEnsemble_CON_BUD
	...
	%
	% AnalyzeEnsemble_CON_BUD properties (Constant).
	%  DENSITIES - 7
	...
	methods (Static) % inspection methods
		...
		function a_name = getName()
			%GETNAME returns the name of the graph analysis with connectivity data of fixed density.
			%
			% NAME = AnalyzeEnsemble_CON_BUD.GETNAME() returns the name of the 'graph analysis with connectivity data of fixed density'.
			%  Graph Analysis With Connectivity Data Of Fixed Density.
			%
			% Alternative forms to call this method are:
			%  NAME = A.GETNAME() returns the name of the graph analysis with connectivity data of fixed density A.
			%  NAME = Element.GETNAME(A) returns the name of 'A'.
			%  NAME = Element.GETNAME('AnalyzeEnsemble_CON_BUD') returns the name of 'AnalyzeEnsemble_CON_BUD'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			a_name = 'Graph Analysis With Connectivity Data Of Fixed Density';
		end
		function a_description = getDescription()
			%GETDESCRIPTION returns the description of the graph analysis with connectivity data of fixed density.
			%
			% STR = AnalyzeEnsemble_CON_BUD.GETDESCRIPTION() returns the description of the 'graph analysis with connectivity data of fixed density'.
			%  which is:
			%
			%  
			%
			% Alternative forms to call this method are:
			%  STR = A.GETDESCRIPTION() returns the description of the graph analysis with connectivity data of fixed density A.
			%  STR = Element.GETDESCRIPTION(A) returns the description of 'A'.
			%  STR = Element.GETDESCRIPTION('AnalyzeEnsemble_CON_BUD') returns the description of 'AnalyzeEnsemble_CON_BUD'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = A.GETPROPNUMBER() returns the property number of the graph analysis with connectivity data of fixed density A.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					AnalyzeEnsemble_CON_BUD.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				analyzeensemble_con_bud_tag_list = { 'id'  'label'  'notes'  'gr'  'g_dict'  'me_dict'  'densities' };
				
				check = any(strcmpi(tag, analyzeensemble_con_bud_tag_list));
			else
				assert( ...
					AnalyzeEnsemble_CON_BUD.existsTag(tag), ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_BUD:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, analyzeensemble_con_bud_tag_list));
			else % numeric
				prop = pointer;
				AnalyzeEnsemble_CON_BUD.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_BUD:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeEnsemble_CON_BUD.getPropTag(prop) ' (' AnalyzeEnsemble_CON_BUD.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function a = AnalyzeEnsemble_CON_BUD(varargin)
			...
			%
			% AnalyzeEnsemble_CON_BUD(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
