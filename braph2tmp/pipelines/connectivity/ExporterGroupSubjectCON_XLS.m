classdef ExporterGroupSubjectCON_XLS < Exporter
	...
	%  <strong>7</strong> <strong>save</strong> 	SAVE (result, empty) saves the group of subjects with connectivity data in XLS/XLSX files in the selected directory.
	%
	% ExporterGroupSubjectCON_XLS methods (constructor):
	% ExporterGroupSubjectCON_XLS - constructor
	%
	% ExporterGroupSubjectCON_XLS methods (Static):
	...
	%  existsTag - checks whether tag exists in exporter of CON subject group in XLS/XLSX/error.
	%  existsProp - checks whether property exists in exporter of CON subject group in XLS/XLSX/error.
	%  getPropNumber - returns the property number of exporter of CON subject group in XLS/XLSX.
	%  getProps - returns the property list of exporter of CON subject group in XLS/XLSX.
	%  getDescription - returns the description of the exporter of CON subject group in XLS/XLSX.
	%  getName - returns the name of the exporter of CON subject group in XLS/XLSX.
	%  getClass - returns the class of the exporter of CON subject group in XLS/XLSX.
	%
	...
	%
	% ExporterGroupSubjectCON_XLS methods (operators):
	%  isequal - determines whether two ExporterGroupSubjectCON_XLS are equal (values, locked)
	%
	% ExporterGroupSubjectCON_XLS methods (display):
	%  tostring - string with information about the ExporterGroupSubjectCON_XLS
	%  disp - displays information about the ExporterGroupSubjectCON_XLS
	%  tree - displays the element of ExporterGroupSubjectCON_XLS
	%
	% ExporterGroupSubjectCON_XLS method (element list):
	%  getElementList - returns a list with all subelements of ExporterGroupSubjectCON_XLS
	%
	% ExporterGroupSubjectCON_XLS method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the ExporterGroupSubjectCON_XLS
	%
	% ExporterGroupSubjectCON_XLS method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the ExporterGroupSubjectCON_XLS
	%
	% ExporterGroupSubjectCON_XLS methods (copy):
	%  copy - copies the ExporterGroupSubjectCON_XLS
	%  deepclone - deep-clones the ExporterGroupSubjectCON_XLS
	%  clone - clones the ExporterGroupSubjectCON_XLS
	%
	% ExporterGroupSubjectCON_XLS methods (inspection, Static):
	%  getClass - returns ExporterGroupSubjectCON_XLS
	%  getName - returns the name of ExporterGroupSubjectCON_XLS
	%  getDescription - returns the description of ExporterGroupSubjectCON_XLS
	%  getProps - returns the property list of ExporterGroupSubjectCON_XLS
	%  getPropNumber - returns the property number of ExporterGroupSubjectCON_XLS
	...
	%
	% ExporterGroupSubjectCON_XLS properties (Constant).
	%  GR - 5
	%  GR_TAG - 'gr'
	%  GR_CATEGORY - 'd'
	%  GR_FORMAT - 'it'
	...
	methods (Static) % inspection methods
		function ex_class = getClass()
			...
			%  CLASS = EX.GETCLASS() returns the class of the exporter of CON subject group in XLS/XLSX EX.
		...
		function ex_name = getName()
			%GETNAME returns the name of the exporter of CON subject group in XLS/XLSX.
			%
			% NAME = ExporterGroupSubjectCON_XLS.GETNAME() returns the name of the 'exporter of CON subject group in XLS/XLSX'.
			%  Exporter Of CON Subject Group In XLS/XLSX.
			%
			% Alternative forms to call this method are:
			%  NAME = EX.GETNAME() returns the name of the exporter of CON subject group in XLS/XLSX EX.
			%  NAME = Element.GETNAME(EX) returns the name of 'EX'.
			%  NAME = Element.GETNAME('ExporterGroupSubjectCON_XLS') returns the name of 'ExporterGroupSubjectCON_XLS'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			ex_name = 'Exporter Of CON Subject Group In XLS/XLSX';
		end
		function ex_description = getDescription()
			%GETDESCRIPTION returns the description of the exporter of CON subject group in XLS/XLSX.
			%
			% STR = ExporterGroupSubjectCON_XLS.GETDESCRIPTION() returns the description of the 'exporter of CON subject group in XLS/XLSX'.
			%  which is:
			%
			%  ExporterGroupSubjectCON_XLS exports a group of subjects with connectivity data to a series of XLSX file and their covariates (if existing).
			%  All these files are saved in the same folder.
			...
			%
			% Alternative forms to call this method are:
			%  STR = EX.GETDESCRIPTION() returns the description of the exporter of CON subject group in XLS/XLSX EX.
			%  STR = Element.GETDESCRIPTION(EX) returns the description of 'EX'.
			%  STR = Element.GETDESCRIPTION('ExporterGroupSubjectCON_XLS') returns the description of 'ExporterGroupSubjectCON_XLS'.
			%
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of exporter of CON subject group in XLS/XLSX.
			%
			% PROPS = ExporterGroupSubjectCON_XLS.GETPROPS() returns the property list of exporter of CON subject group in XLS/XLSX.
			%
			% PROPS = ExporterGroupSubjectCON_XLS.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = EX.GETPROPS([CATEGORY]) returns the property list of the exporter of CON subject group in XLS/XLSX EX.
		...
		function prop_number = getPropNumber()
			%GETPROPNUMBER returns the property number of exporter of CON subject group in XLS/XLSX.
			%
			% N = ExporterGroupSubjectCON_XLS.GETPROPNUMBER() returns the property number of exporter of CON subject group in XLS/XLSX.
			%
			% Alternative forms to call this method are:
			%  N = EX.GETPROPNUMBER() returns the property number of the exporter of CON subject group in XLS/XLSX EX.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					ExporterGroupSubjectCON_XLS.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				exportergroupsubjectcon_xls_tag_list = { 'id'  'label'  'notes'  'waitbar'  'gr'  'directory'  'save' };
				
				check = any(strcmpi(tag, exportergroupsubjectcon_xls_tag_list));
			else
				assert( ...
					ExporterGroupSubjectCON_XLS.existsTag(tag), ...
					[BRAPH2.STR ':ExporterGroupSubjectCON_XLS:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectCON_XLS:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, exportergroupsubjectcon_xls_tag_list));
			else % numeric
				prop = pointer;
				ExporterGroupSubjectCON_XLS.existsProp(prop);
			end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case ExporterGroupSubjectCON_XLS.SAVE
					prop_description = 'SAVE (result, empty) saves the group of subjects with connectivity data in XLS/XLSX files in the selected directory.';
				otherwise
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':ExporterGroupSubjectCON_XLS:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectCON_XLS:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ExporterGroupSubjectCON_XLS.getPropTag(prop) ' (' ExporterGroupSubjectCON_XLS.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function ex = ExporterGroupSubjectCON_XLS(varargin)
			% ExporterGroupSubjectCON_XLS() creates a exporter of CON subject group in XLS/XLSX.
			%
			% ExporterGroupSubjectCON_XLS(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% ExporterGroupSubjectCON_XLS(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(ex, prop)
			...
			switch prop
				case ExporterGroupSubjectCON_XLS.SAVE
					rng(ex.getPropSeed(ExporterGroupSubjectCON_XLS.SAVE), 'twister')
