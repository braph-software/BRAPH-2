classdef ImporterGroupSubjectST_MP_TXT < Importer
	...
	%
	% ImporterGroupSubjectST_MP_TXT methods (Static):
	...
	%  getDescription - returns the description of the importer of ST MP subject group from TXT.
	%  getName - returns the name of the importer of ST MP subject group from TXT.
	%  getClass - returns the class of the importer of ST MP subject group from TXT.
	%
	% ImporterGroupSubjectST_MP_TXT methods:
	%  uigetfile - E opens a dialog box to set the TXT file from where to load the ST MP subject group.
	%  uigetdir - R opens a dialog box to set the directory from where to load the TXT files of the ST MP subject group with L layers.
	...
	%
	% ImporterGroupSubjectST_MP_TXT methods (operators):
	%  isequal - determines whether two ImporterGroupSubjectST_MP_TXT are equal (values, locked)
	%
	% ImporterGroupSubjectST_MP_TXT methods (display):
	%  tostring - string with information about the ImporterGroupSubjectST_MP_TXT
	%  disp - displays information about the ImporterGroupSubjectST_MP_TXT
	%  tree - displays the element of ImporterGroupSubjectST_MP_TXT
	%
	% ImporterGroupSubjectST_MP_TXT method (element list):
	%  getElementList - returns a list with all subelements of ImporterGroupSubjectST_MP_TXT
	%
	% ImporterGroupSubjectST_MP_TXT method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the ImporterGroupSubjectST_MP_TXT
	%
	% ImporterGroupSubjectST_MP_TXT method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the ImporterGroupSubjectST_MP_TXT
	%
	% ImporterGroupSubjectST_MP_TXT methods (copy):
	%  copy - copies the ImporterGroupSubjectST_MP_TXT
	%  deepclone - deep-clones the ImporterGroupSubjectST_MP_TXT
	%  clone - clones the ImporterGroupSubjectST_MP_TXT
	%
	% ImporterGroupSubjectST_MP_TXT methods (inspection, Static):
	%  getClass - returns ImporterGroupSubjectST_MP_TXT
	%  getName - returns the name of ImporterGroupSubjectST_MP_TXT
	%  getDescription - returns the description of ImporterGroupSubjectST_MP_TXT
	%  getProps - returns the property list of ImporterGroupSubjectST_MP_TXT
	%  getPropNumber - returns the property number of ImporterGroupSubjectST_MP_TXT
	...
	%
	% ImporterGroupSubjectST_MP_TXT properties (Constant).
	%  DIRECTORY - 5
	...
	%
	% See also Element, Importer, ExporterGroupSubjectST_MP_TXT.
	...
	methods (Static) % inspection methods
		...
		function im_name = getName()
			%GETNAME returns the name of the importer of ST MP subject group from TXT.
			%
			% NAME = ImporterGroupSubjectST_MP_TXT.GETNAME() returns the name of the 'importer of ST MP subject group from TXT'.
			%  Importer Of ST MP Subject Group From TXT.
			%
			% Alternative forms to call this method are:
			%  NAME = IM.GETNAME() returns the name of the importer of ST MP subject group from TXT IM.
			%  NAME = Element.GETNAME(IM) returns the name of 'IM'.
			%  NAME = Element.GETNAME('ImporterGroupSubjectST_MP_TXT') returns the name of 'ImporterGroupSubjectST_MP_TXT'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			im_name = 'Importer Of ST MP Subject Group From TXT';
		end
		function im_description = getDescription()
			%GETDESCRIPTION returns the description of the importer of ST MP subject group from TXT.
			%
			% STR = ImporterGroupSubjectST_MP_TXT.GETDESCRIPTION() returns the description of the 'importer of ST MP subject group from TXT'.
			%  which is:
			%
			%  ImporterGroupSubjectST_MP_TXT imports a group of subjects with structural multiplex data from an TXT file and their covariates from another TXT file.
			%  The files from the same group containing the data from L layers must be in the same folder.
			...
			%
			% Alternative forms to call this method are:
			%  STR = IM.GETDESCRIPTION() returns the description of the importer of ST MP subject group from TXT IM.
			%  STR = Element.GETDESCRIPTION(IM) returns the description of 'IM'.
			%  STR = Element.GETDESCRIPTION('ImporterGroupSubjectST_MP_TXT') returns the description of 'ImporterGroupSubjectST_MP_TXT'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = IM.GETPROPNUMBER() returns the property number of the importer of ST MP subject group from TXT IM.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					ImporterGroupSubjectST_MP_TXT.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				importergroupsubjectst_mp_txt_tag_list = { 'id'  'label'  'notes'  'waitbar'  'directory'  'ba'  'gr' };
				
				check = any(strcmpi(tag, importergroupsubjectst_mp_txt_tag_list));
			else
				assert( ...
					ImporterGroupSubjectST_MP_TXT.existsTag(tag), ...
					[BRAPH2.STR ':ImporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ImporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, importergroupsubjectst_mp_txt_tag_list));
			else % numeric
				prop = pointer;
				ImporterGroupSubjectST_MP_TXT.existsProp(prop);
			end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				case ImporterGroupSubjectST_MP_TXT.DIRECTORY
					prop_default = Format.getFormatDefault(Format.STRING, ImporterGroupSubjectST_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectST_MP_TXT.BA
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':ImporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ImporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ImporterGroupSubjectST_MP_TXT.getPropTag(prop) ' (' ImporterGroupSubjectST_MP_TXT.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function im = ImporterGroupSubjectST_MP_TXT(varargin)
			...
			%
			% ImporterGroupSubjectST_MP_TXT(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(im, prop)
			...
			switch prop
				case ImporterGroupSubjectST_MP_TXT.GR
					rng(im.getPropSeed(ImporterGroupSubjectST_MP_TXT.GR), 'twister')
