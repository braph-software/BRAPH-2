classdef AnalyzeGroup_ST_MP_BUD < AnalyzeGroup
	...
	%
	% AnalyzeGroup_ST_MP_BUD methods (Static):
	...
	%  getDescription - returns the description of the graph analysis with structural multiplex data of fixed density.
	%  getName - returns the name of the graph analysis with structural multiplex data of fixed density.
	...
	%
	% AnalyzeGroup_ST_MP_BUD methods (operators):
	%  isequal - determines whether two AnalyzeGroup_ST_MP_BUD are equal (values, locked)
	%
	% AnalyzeGroup_ST_MP_BUD methods (display):
	%  tostring - string with information about the AnalyzeGroup_ST_MP_BUD
	%  disp - displays information about the AnalyzeGroup_ST_MP_BUD
	%  tree - displays the element of AnalyzeGroup_ST_MP_BUD
	%
	% AnalyzeGroup_ST_MP_BUD method (element list):
	%  getElementList - returns a list with all subelements of AnalyzeGroup_ST_MP_BUD
	%
	% AnalyzeGroup_ST_MP_BUD method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the AnalyzeGroup_ST_MP_BUD
	%
	% AnalyzeGroup_ST_MP_BUD method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the AnalyzeGroup_ST_MP_BUD
	%
	% AnalyzeGroup_ST_MP_BUD methods (copy):
	%  copy - copies the AnalyzeGroup_ST_MP_BUD
	%  deepclone - deep-clones the AnalyzeGroup_ST_MP_BUD
	%  clone - clones the AnalyzeGroup_ST_MP_BUD
	%
	% AnalyzeGroup_ST_MP_BUD methods (inspection, Static):
	%  getClass - returns AnalyzeGroup_ST_MP_BUD
	%  getName - returns the name of AnalyzeGroup_ST_MP_BUD
	%  getDescription - returns the description of AnalyzeGroup_ST_MP_BUD
	%  getProps - returns the property list of AnalyzeGroup_ST_MP_BUD
	%  getPropNumber - returns the property number of AnalyzeGroup_ST_MP_BUD
	...
	%
	% AnalyzeGroup_ST_MP_BUD properties (Constant).
	%  CORRELATION_RULE - 6
	...
	methods (Static) % inspection methods
		...
		function a_name = getName()
			%GETNAME returns the name of the graph analysis with structural multiplex data of fixed density.
			%
			% NAME = AnalyzeGroup_ST_MP_BUD.GETNAME() returns the name of the 'graph analysis with structural multiplex data of fixed density'.
			%  Graph Analysis With Structural Multiplex Data Of Fixed Density.
			%
			% Alternative forms to call this method are:
			%  NAME = A.GETNAME() returns the name of the graph analysis with structural multiplex data of fixed density A.
			%  NAME = Element.GETNAME(A) returns the name of 'A'.
			%  NAME = Element.GETNAME('AnalyzeGroup_ST_MP_BUD') returns the name of 'AnalyzeGroup_ST_MP_BUD'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			a_name = 'Graph Analysis With Structural Multiplex Data Of Fixed Density';
		end
		function a_description = getDescription()
			%GETDESCRIPTION returns the description of the graph analysis with structural multiplex data of fixed density.
			%
			% STR = AnalyzeGroup_ST_MP_BUD.GETDESCRIPTION() returns the description of the 'graph analysis with structural multiplex data of fixed density'.
			%  which is:
			%
			%  
			%
			% Alternative forms to call this method are:
			%  STR = A.GETDESCRIPTION() returns the description of the graph analysis with structural multiplex data of fixed density A.
			%  STR = Element.GETDESCRIPTION(A) returns the description of 'A'.
			%  STR = Element.GETDESCRIPTION('AnalyzeGroup_ST_MP_BUD') returns the description of 'AnalyzeGroup_ST_MP_BUD'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = A.GETPROPNUMBER() returns the property number of the graph analysis with structural multiplex data of fixed density A.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					AnalyzeGroup_ST_MP_BUD.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				analyzegroup_st_mp_bud_tag_list = { 'id'  'label'  'notes'  'gr'  'g'  'correlation_rule'  'negative_weight_rule'  'densities' };
				
				check = any(strcmpi(tag, analyzegroup_st_mp_bud_tag_list));
			else
				assert( ...
					AnalyzeGroup_ST_MP_BUD.existsTag(tag), ...
					[BRAPH2.STR ':AnalyzeGroup_ST_MP_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeGroup_ST_MP_BUD:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, analyzegroup_st_mp_bud_tag_list));
			else % numeric
				prop = pointer;
				AnalyzeGroup_ST_MP_BUD.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':AnalyzeGroup_ST_MP_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeGroup_ST_MP_BUD:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeGroup_ST_MP_BUD.getPropTag(prop) ' (' AnalyzeGroup_ST_MP_BUD.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function a = AnalyzeGroup_ST_MP_BUD(varargin)
			...
			%
			% AnalyzeGroup_ST_MP_BUD(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
