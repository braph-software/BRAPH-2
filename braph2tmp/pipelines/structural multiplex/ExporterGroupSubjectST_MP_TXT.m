classdef ExporterGroupSubjectST_MP_TXT < Exporter
	...
	%  <strong>7</strong> <strong>save</strong> 	SAVE (result, empty) saves the group of subjects with structural multiplex data in TXT files in the selected directory.
	%
	% ExporterGroupSubjectST_MP_TXT methods (constructor):
	% ExporterGroupSubjectST_MP_TXT - constructor
	%
	% ExporterGroupSubjectST_MP_TXT methods (Static):
	...
	%  getDescription - returns the description of the exporter of ST MP subject group in TXT.
	%  getName - returns the name of the exporter of ST MP subject group in TXT.
	...
	%
	% ExporterGroupSubjectST_MP_TXT methods (operators):
	%  isequal - determines whether two ExporterGroupSubjectST_MP_TXT are equal (values, locked)
	%
	% ExporterGroupSubjectST_MP_TXT methods (display):
	%  tostring - string with information about the ExporterGroupSubjectST_MP_TXT
	%  disp - displays information about the ExporterGroupSubjectST_MP_TXT
	%  tree - displays the element of ExporterGroupSubjectST_MP_TXT
	%
	% ExporterGroupSubjectST_MP_TXT method (element list):
	%  getElementList - returns a list with all subelements of ExporterGroupSubjectST_MP_TXT
	%
	% ExporterGroupSubjectST_MP_TXT method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the ExporterGroupSubjectST_MP_TXT
	%
	% ExporterGroupSubjectST_MP_TXT method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the ExporterGroupSubjectST_MP_TXT
	%
	% ExporterGroupSubjectST_MP_TXT methods (copy):
	%  copy - copies the ExporterGroupSubjectST_MP_TXT
	%  deepclone - deep-clones the ExporterGroupSubjectST_MP_TXT
	%  clone - clones the ExporterGroupSubjectST_MP_TXT
	%
	% ExporterGroupSubjectST_MP_TXT methods (inspection, Static):
	%  getClass - returns ExporterGroupSubjectST_MP_TXT
	%  getName - returns the name of ExporterGroupSubjectST_MP_TXT
	%  getDescription - returns the description of ExporterGroupSubjectST_MP_TXT
	%  getProps - returns the property list of ExporterGroupSubjectST_MP_TXT
	%  getPropNumber - returns the property number of ExporterGroupSubjectST_MP_TXT
	...
	%
	% ExporterGroupSubjectST_MP_TXT properties (Constant).
	%  GR - 5
	%  GR_TAG - 'gr'
	%  GR_CATEGORY - 'd'
	%  GR_FORMAT - 'it'
	...
	%
	% See also Element, Exporter, ImporterGroupSubjectST_MP_TXT.
	...
	methods (Static) % inspection methods
		...
		function ex_name = getName()
			%GETNAME returns the name of the exporter of ST MP subject group in TXT.
			%
			% NAME = ExporterGroupSubjectST_MP_TXT.GETNAME() returns the name of the 'exporter of ST MP subject group in TXT'.
			%  Exporter Of ST MP Subject Group In TXT.
			%
			% Alternative forms to call this method are:
			%  NAME = EX.GETNAME() returns the name of the exporter of ST MP subject group in TXT EX.
			%  NAME = Element.GETNAME(EX) returns the name of 'EX'.
			%  NAME = Element.GETNAME('ExporterGroupSubjectST_MP_TXT') returns the name of 'ExporterGroupSubjectST_MP_TXT'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			ex_name = 'Exporter Of ST MP Subject Group In TXT';
		end
		function ex_description = getDescription()
			%GETDESCRIPTION returns the description of the exporter of ST MP subject group in TXT.
			%
			% STR = ExporterGroupSubjectST_MP_TXT.GETDESCRIPTION() returns the description of the 'exporter of ST MP subject group in TXT'.
			%  which is:
			%
			%  ExporterGroupSubjectST_MP_TXT exports a group of subjects with structural multiplex data to an TXT file and their covariates age and sex (if existing) to another TXT file.
			%  The files from the same group containing the data from L layers are saved in the same folder.
			...
			%
			% Alternative forms to call this method are:
			%  STR = EX.GETDESCRIPTION() returns the description of the exporter of ST MP subject group in TXT EX.
			%  STR = Element.GETDESCRIPTION(EX) returns the description of 'EX'.
			%  STR = Element.GETDESCRIPTION('ExporterGroupSubjectST_MP_TXT') returns the description of 'ExporterGroupSubjectST_MP_TXT'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = EX.GETPROPNUMBER() returns the property number of the exporter of ST MP subject group in TXT EX.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					ExporterGroupSubjectST_MP_TXT.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				exportergroupsubjectst_mp_txt_tag_list = { 'id'  'label'  'notes'  'waitbar'  'gr'  'directory'  'save' };
				
				check = any(strcmpi(tag, exportergroupsubjectst_mp_txt_tag_list));
			else
				assert( ...
					ExporterGroupSubjectST_MP_TXT.existsTag(tag), ...
					[BRAPH2.STR ':ExporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, exportergroupsubjectst_mp_txt_tag_list));
			else % numeric
				prop = pointer;
				ExporterGroupSubjectST_MP_TXT.existsProp(prop);
			end
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':ExporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectST_MP_TXT:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ExporterGroupSubjectST_MP_TXT.getPropTag(prop) ' (' ExporterGroupSubjectST_MP_TXT.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function ex = ExporterGroupSubjectST_MP_TXT(varargin)
			...
			%
			% ExporterGroupSubjectST_MP_TXT(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(ex, prop)
			...
			switch prop
				case ExporterGroupSubjectST_MP_TXT.SAVE
					rng(ex.getPropSeed(ExporterGroupSubjectST_MP_TXT.SAVE), 'twister')
