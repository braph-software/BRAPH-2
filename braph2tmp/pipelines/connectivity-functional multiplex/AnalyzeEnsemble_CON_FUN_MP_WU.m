classdef AnalyzeEnsemble_CON_FUN_MP_WU < AnalyzeEnsemble
	% AnalyzeEnsemble_CON_FUN_MP_WU is a graph analysis using connectivity and functional multiplex data.
	...
	%  <strong>5</strong> <strong>g_dict</strong> 	G_DICT (result, idict) is the multiplex (MultiplexWU) ensemble obtained from this analysis.
	...
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU methods (Static):
	...
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU methods (operators):
	%  isequal - determines whether two AnalyzeEnsemble_CON_FUN_MP_WU are equal (values, locked)
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU methods (display):
	%  tostring - string with information about the AnalyzeEnsemble_CON_FUN_MP_WU
	%  disp - displays information about the AnalyzeEnsemble_CON_FUN_MP_WU
	%  tree - displays the element of AnalyzeEnsemble_CON_FUN_MP_WU
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU method (element list):
	%  getElementList - returns a list with all subelements of AnalyzeEnsemble_CON_FUN_MP_WU
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the AnalyzeEnsemble_CON_FUN_MP_WU
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the AnalyzeEnsemble_CON_FUN_MP_WU
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU methods (copy):
	%  copy - copies the AnalyzeEnsemble_CON_FUN_MP_WU
	%  deepclone - deep-clones the AnalyzeEnsemble_CON_FUN_MP_WU
	%  clone - clones the AnalyzeEnsemble_CON_FUN_MP_WU
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU methods (inspection, Static):
	%  getClass - returns AnalyzeEnsemble_CON_FUN_MP_WU
	%  getName - returns the name of AnalyzeEnsemble_CON_FUN_MP_WU
	%  getDescription - returns the description of AnalyzeEnsemble_CON_FUN_MP_WU
	%  getProps - returns the property list of AnalyzeEnsemble_CON_FUN_MP_WU
	%  getPropNumber - returns the property number of AnalyzeEnsemble_CON_FUN_MP_WU
	...
	%
	% AnalyzeEnsemble_CON_FUN_MP_WU properties (Constant).
	%  REPETITION - 7
	...
	methods (Static) % inspection methods
		...
		function a_name = getName()
			%GETNAME returns the name of the graph analysis with connectivity and functional multiplex data.
			%
			% NAME = AnalyzeEnsemble_CON_FUN_MP_WU.GETNAME() returns the name of the 'graph analysis with connectivity and functional multiplex data'.
			...
			%  NAME = Element.GETNAME('AnalyzeEnsemble_CON_FUN_MP_WU') returns the name of 'AnalyzeEnsemble_CON_FUN_MP_WU'.
			%
		...
		function a_description = getDescription()
			%GETDESCRIPTION returns the description of the graph analysis with connectivity and functional multiplex data.
			%
			% STR = AnalyzeEnsemble_CON_FUN_MP_WU.GETDESCRIPTION() returns the description of the 'graph analysis with connectivity and functional multiplex data'.
			%  which is:
			%
			%  
			%
			...
			%  STR = Element.GETDESCRIPTION('AnalyzeEnsemble_CON_FUN_MP_WU') returns the description of 'AnalyzeEnsemble_CON_FUN_MP_WU'.
			%
			...
			switch category
				...
				case Category.PARAMETER
					prop_list = [
						...
						AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN
						AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					AnalyzeEnsemble_CON_FUN_MP_WU.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				analyzeensemble_con_fun_mp_wu_tag_list = { 'id'  'label'  'notes'  'gr'  'g_dict'  'me_dict'  'repetition'  'frequencyrulemin'  'frequencyrulemax'  'correlation_rule'  'negative_weight_rule' };
				
				check = any(strcmpi(tag, analyzeensemble_con_fun_mp_wu_tag_list));
			else
				assert( ...
					AnalyzeEnsemble_CON_FUN_MP_WU.existsTag(tag), ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_FUN_MP_WU:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_FUN_MP_WU:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, analyzeensemble_con_fun_mp_wu_tag_list));
			else % numeric
				prop = pointer;
				AnalyzeEnsemble_CON_FUN_MP_WU.existsProp(prop);
			end
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					...
					case AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN
						tag = AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN_TAG;
					case AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX
						tag = AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX_TAG;
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN
					prop_category = AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN_CATEGORY;
				case AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX
					prop_category = AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX_CATEGORY;
				case AnalyzeEnsemble_CON_FUN_MP_WU.CORRELATION_RULE
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN
					prop_format = AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMIN_FORMAT;
				case AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX
					prop_format = AnalyzeEnsemble_CON_FUN_MP_WU.FREQUENCYRULEMAX_FORMAT;
				case AnalyzeEnsemble_CON_FUN_MP_WU.CORRELATION_RULE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_FUN_MP_WU.G_DICT
					prop_default = IndexedDictionary('IT_CLASS', 'MultiplexWU');
				otherwise
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_FUN_MP_WU:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_FUN_MP_WU:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeEnsemble_CON_FUN_MP_WU.getPropTag(prop) ' (' AnalyzeEnsemble_CON_FUN_MP_WU.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function a = AnalyzeEnsemble_CON_FUN_MP_WU(varargin)
			...
			%
			% AnalyzeEnsemble_CON_FUN_MP_WU(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
