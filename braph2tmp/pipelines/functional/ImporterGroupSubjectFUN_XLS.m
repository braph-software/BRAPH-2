classdef ImporterGroupSubjectFUN_XLS < Importer
	...
	%
	% ImporterGroupSubjectFUN_XLS methods (Static):
	...
	%  getDescription - returns the description of the importer of FUN subject group from XLS/XLSX.
	%  getName - returns the name of the importer of FUN subject group from XLS/XLSX.
	%  getClass - returns the class of the importer of FUN subject group from XLS/XLSX.
	%
	% ImporterGroupSubjectFUN_XLS methods:
	%  uigetdir - R opens a dialog box to set the directory from where to load the XLS/XLSX files of the FUN subject group.
	...
	%
	% ImporterGroupSubjectFUN_XLS methods (operators):
	%  isequal - determines whether two ImporterGroupSubjectFUN_XLS are equal (values, locked)
	%
	% ImporterGroupSubjectFUN_XLS methods (display):
	%  tostring - string with information about the ImporterGroupSubjectFUN_XLS
	%  disp - displays information about the ImporterGroupSubjectFUN_XLS
	%  tree - displays the element of ImporterGroupSubjectFUN_XLS
	%
	% ImporterGroupSubjectFUN_XLS method (element list):
	%  getElementList - returns a list with all subelements of ImporterGroupSubjectFUN_XLS
	%
	% ImporterGroupSubjectFUN_XLS method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the ImporterGroupSubjectFUN_XLS
	%
	% ImporterGroupSubjectFUN_XLS method (JSON decode, Static):
	%  decodeJSON - returns a JSON string encoding the ImporterGroupSubjectFUN_XLS
	%
	% ImporterGroupSubjectFUN_XLS methods (copy):
	%  copy - copies the ImporterGroupSubjectFUN_XLS
	%  deepclone - deep-clones the ImporterGroupSubjectFUN_XLS
	%  clone - clones the ImporterGroupSubjectFUN_XLS
	%
	% ImporterGroupSubjectFUN_XLS methods (inspection, Static):
	%  getClass - returns ImporterGroupSubjectFUN_XLS
	%  getName - returns the name of ImporterGroupSubjectFUN_XLS
	%  getDescription - returns the description of ImporterGroupSubjectFUN_XLS
	%  getProps - returns the property list of ImporterGroupSubjectFUN_XLS
	%  getPropNumber - returns the property number of ImporterGroupSubjectFUN_XLS
	...
	%
	% ImporterGroupSubjectFUN_XLS properties (Constant).
	%  DIRECTORY - 5
	...
	%
	% See also Element, Importer, ExporterGroupSubjectFUN_XLS.
	...
	methods (Static) % inspection methods
		...
		function im_name = getName()
			%GETNAME returns the name of the importer of FUN subject group from XLS/XLSX.
			%
			% NAME = ImporterGroupSubjectFUN_XLS.GETNAME() returns the name of the 'importer of FUN subject group from XLS/XLSX'.
			%  Importer Of FUN Subject Group From XLS/XLSX.
			%
			% Alternative forms to call this method are:
			%  NAME = IM.GETNAME() returns the name of the importer of FUN subject group from XLS/XLSX IM.
			%  NAME = Element.GETNAME(IM) returns the name of 'IM'.
			%  NAME = Element.GETNAME('ImporterGroupSubjectFUN_XLS') returns the name of 'ImporterGroupSubjectFUN_XLS'.
			%
			% See also getClass, getDescription.
			
			% COMPUTATIONAL EFFICIENCY TRICK
			im_name = 'Importer Of FUN Subject Group From XLS/XLSX';
		end
		function im_description = getDescription()
			%GETDESCRIPTION returns the description of the importer of FUN subject group from XLS/XLSX.
			%
			% STR = ImporterGroupSubjectFUN_XLS.GETDESCRIPTION() returns the description of the 'importer of FUN subject group from XLS/XLSX'.
			%  which is:
			%
			%  ImporterGroupSubjectFUN_XLS imports a group of subjects with functional data from a series of XLS/XLSX file and their covariates (optional) from another XLS/XLSX file.
			%  All these files must be in the same folder; also, no other files should be in the folder.
			...
			%
			% Alternative forms to call this method are:
			%  STR = IM.GETDESCRIPTION() returns the description of the importer of FUN subject group from XLS/XLSX IM.
			%  STR = Element.GETDESCRIPTION(IM) returns the description of 'IM'.
			%  STR = Element.GETDESCRIPTION('ImporterGroupSubjectFUN_XLS') returns the description of 'ImporterGroupSubjectFUN_XLS'.
			%
		...
		function prop_number = getPropNumber()
			...
			%
			% Alternative forms to call this method are:
			%  N = IM.GETPROPNUMBER() returns the property number of the importer of FUN subject group from XLS/XLSX IM.
		...
		function check = existsProp(prop)
			...
			if nargout == 1
				...
				assert( ...
					ImporterGroupSubjectFUN_XLS.existsProp(prop), ...
		...
		function check = existsTag(tag)
			...
			if nargout == 1
				% COMPUTATIONAL EFFICIENCY TRICK
				importergroupsubjectfun_xls_tag_list = { 'id'  'label'  'notes'  'waitbar'  'directory'  'ba'  'gr' };
				
				check = any(strcmpi(tag, importergroupsubjectfun_xls_tag_list));
			else
				assert( ...
					ImporterGroupSubjectFUN_XLS.existsTag(tag), ...
					[BRAPH2.STR ':ImporterGroupSubjectFUN_XLS:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ImporterGroupSubjectFUN_XLS:' BRAPH2.WRONG_INPUT ' ' ...
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				...
				prop = find(strcmpi(tag, importergroupsubjectfun_xls_tag_list));
			else % numeric
				prop = pointer;
				ImporterGroupSubjectFUN_XLS.existsProp(prop);
			end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				case ImporterGroupSubjectFUN_XLS.DIRECTORY
					prop_default = Format.getFormatDefault(Format.STRING, ImporterGroupSubjectFUN_XLS.getPropSettings(prop));
				case ImporterGroupSubjectFUN_XLS.BA
		...
		function prop_check = checkProp(pointer, value)
			...
			if nargout == 1
				...
				assert( ...
					...
					[BRAPH2.STR ':ImporterGroupSubjectFUN_XLS:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ImporterGroupSubjectFUN_XLS:' BRAPH2.WRONG_INPUT ' ' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ImporterGroupSubjectFUN_XLS.getPropTag(prop) ' (' ImporterGroupSubjectFUN_XLS.getPropFormat(prop) ').'] ...
					)
			end
		end
	end
	methods % constructor
		function im = ImporterGroupSubjectFUN_XLS(varargin)
			...
			%
			% ImporterGroupSubjectFUN_XLS(TAG, VALUE, ...) with property with tag TAG set to VALUE.
			%
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(im, prop)
			...
			switch prop
				case ImporterGroupSubjectFUN_XLS.GR
					rng(im.getPropSeed(ImporterGroupSubjectFUN_XLS.GR), 'twister')
