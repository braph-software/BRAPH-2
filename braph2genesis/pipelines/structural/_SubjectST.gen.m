...
function cb_exporter_TXT(~, ~)
    ...
    if ~isa(ex.getr('FILE'), 'NoValue') % % % ~strcmp(ex.getr('FILE'), ExporterGroupSubjectST_TXT.getPropDefault('FILE')) %TBE
        ex.get('SAVE');
    end
end
...
function cb_exporter_XLS(~, ~)
    ...
    if ~isa(ex.getr('FILE'), 'NoValue') % % % ~strcmp(ex.get('FILE'), ExporterGroupSubjectST_XLS.getPropDefault('FILE')) %TBE
        ex.get('SAVE');
    end
end
