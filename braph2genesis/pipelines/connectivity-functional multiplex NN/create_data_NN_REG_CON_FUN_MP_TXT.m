function create_data_NN_REG_CON_FUN_MP_TXT(data_dir, random_seed)
%CREATE_DATA_CON_FUN_MP_NN_TXT creates connectivity-functional data for neural network regression analysis
%
% CREATE_DATA_CON_FUN_MP_NN_TXT() creates connectivity-functional data in default folder 'Example
%  data NN CLA CON FUN MP TXT'.
%
...
if nargin < 1
    data_dir = [fileparts(which('NNDataPoint_CON_FUN_MP_REG')) filesep 'Example data NN REG CON_FUN_MP TXT'];
end
...
if ~isdir(data_dir)
    ...
    for i_tab = 1:1:N_subjects
        ...
        file_name = [data_dir filesep() 'Functional' filesep() gr_name filesep() sub_id{i_tab} '.txt'];
    ...
    for i_tab = 1:1:N_subjects
        ...
        file_name = [data_dir filesep() 'Connectivity' filesep() gr_name filesep() sub_id{i_tab} '.txt'];
        writetable(T_gr12, file_name, 'Delimiter', '\t', 'WriteRowNames', false, 'WriteVariableNames', false)
    end
    
    writetable(table(vois), [data_dir filesep() 'Connectivity' filesep() gr_name '.vois.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
    writetable(table(vois), [data_dir filesep() 'Functional' filesep() gr_name '.vois.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
    % reset RNG
    rng(rng_settings_)
end
end
