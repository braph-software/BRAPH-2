...
if ~braph2_testing
    if isa(nnd.getr('GRAPH_TEMPLATE'), 'NoValue')
        if nnd.get('GR').get('SUB_DICT').length() > 0
            nnd.set('GRAPH_TEMPLATE', MultiplexBUD('BAS', nnd.get('GR').get('SUB_DICT').getItem(1).get('BA')));
        else
            nnd.set('GRAPH_TEMPLATE', MultiplexBUD());
        end
    end
end
...
%%%% ¡settings!
'NNData_CON_FUN_MP_BUD'
%%%% ¡postprocessing!
if nnd.prop_set(NNData_CON_FUN_MP_BUD.TEMPLATE, varargin{:})
    varargin = {};
    ...
    for i = 1:1:length(parameters)
        ...
        if parameter ~= NNData_CON_FUN_MP_BUD.TEMPLATE
