function create_data_NN_REG_ST_MM_XLS(data_dir, random_seed)
%CREATE_DATA_NN_REG_ST_MM_XLS creates multi-modality structural data for neural network regression analysis
%
% CREATE_DATA_NN_REG_ST_MM_XLS() creates multi-modality structural data in default folder 'Example
%  data NN REG ST XLS'.
%
% CREATE_DATA_NN_REG_ST_MM_XLS(DATA_DIR) creates multi-modality structural data in DATA_DIR folder.
%
% CREATE_DATA_NN_REG_ST_MM_XLS(DATA_DIR, RANDOM_SEED) cretes multi-modality structural data in
%  DATA_DIR folder with a specified RANDOM_SEED.
%
% See also create_data_nn_reg_st_mm_txt.

if nargin < 1
    data_dir = [fileparts(which('NNDataPoint_ST_MM_REG')) filesep 'Example data NN REG ST MM XLS'];
end
