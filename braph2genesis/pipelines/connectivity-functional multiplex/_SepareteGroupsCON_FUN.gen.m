...
ba = gr.getItem(1).get('BA');

% declare variables
mixed_data{i} = cell(gr.get('SUB_DICT').length(), 1);
gr_con = Group( ...
...
for i = 1:1:gr.get('SUB_DICT').length()
    if se.get('WAITBAR')
        waitbar(.30 + .70 * i / gr.get('SUB_DICT').length(), wb, ['Separating subject ' num2str(i) ' of ' num2str(gr.get('SUB_DICT').length()) ' ...'])
    end
    sub = gr.getItem(i);
    mixed_data{i} = sub.get('CON_FUN_MP');
    sub_con = SubjectCON( ...
        'ID', sub.get('ID'), ...
        'BA', ba, ...
        'age', sub.get('AGE'), ...
