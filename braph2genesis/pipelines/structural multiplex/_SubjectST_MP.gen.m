...
function cb_importer_TXT(~, ~)
    im = ImporterGroupSubjectSTMP_TXT( ...
    ...
    try
        if isfolder(im.get('DIRECTORY'))
            ...
            assert( ...
                ...
                [BRAPH2.STR ':SubjectSTMP:' BRAPH2.BUG_FUNC], ...
...
function cb_importer_XLS(~, ~)
    im = ImporterGroupSubjectSTMP_XLS( ...
...
function cb_exporter_TXT(~, ~)
    ex = ExporterGroupSubjectSTMP_TXT( ...
        'ID', 'Export Brain Group of SubjectSTMPs to TXT', ...
        'GR', el.copy(), ...
        'WAITBAR', true ...
        );
    ex.uigetdir()
    if ~strcmp(ex.get('DIRECTORY'), ExporterGroupSubjectSTMP_TXT.getPropDefault('DIRECTORY'))
        ex.get('SAVE');
    end
end
...
function cb_exporter_XLS(~, ~)
    ex = ExporterGroupSubjectSTMP_XLS( ...
        'ID', 'Export Brain Group of SubjectSTMPs to XLS', ...
        'GR', el.copy(), ...
        'WAITBAR', true ...
        );
    ex.uigetdir()
    if ~strcmp(ex.get('DIRECTORY'), ExporterGroupSubjectSTMP_XLS.getPropDefault('DIRECTORY'))
        ex.get('SAVE');
    end
end
...
%%%% ¡gui!
pr = PPMultiplexSTMP_STMP('EL', sub, 'PROP', SubjectST_MP.ST_MP, varargin{:});
