...
cv = AnalyzeKFoldCrossValidation( ...
    'A_1', a_WU1, ...
    'A_2', a_WU2, ...
    'KFOLD', 5, ...
    'DENSITY_OF_FEATURE_SELECTION', 0.05);

average_auc = cv.getCrossValidationMetric('AUC').get('VALUE');
average_auc_cil = cv.getCrossValidationMetric('AUC').get('CIL');
average_auc_cil = cv.getCrossValidationMetric('AUC').get('CIU');
...
classifier = ClassifierNN_CON_WU( ...
    ...
    'A_2_test', a_WU2_test, ...
    'DENSITY_OF_FEATURE_SELECTION', 0.05);
auc_train = classifier.getMetrics('ACCURACY').get('VALUE_TRAIN');
auc_test = classifier.getMetrics('AUC').get('VALUE_TEST');
