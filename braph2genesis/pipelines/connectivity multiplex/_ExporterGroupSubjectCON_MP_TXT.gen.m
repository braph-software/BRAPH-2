...
%%% ¡seealso!
Element, Exporter, ImporterGroupSubjectCON_MP_TXT
...
function uigetdir(ex)
    % UIGETDIR opens a dialog box to set the directory where to save the group of subjects with connectivity multiplex data.
...
sub1 = SubjectCON_MP( ...
    ...
    'sex', 'female', ...
    'L', 2, ...
    'CON_MP', {rand(ba.get('BR_DICT').length(), ba.get('BR_DICT').length()), rand(ba.get('BR_DICT').length(), ba.get('BR_DICT').length())} ...
    );
sub2 = SubjectCON_MP( ...
    'ID', 'SUB CON_MP 2', ...
...
assert(gr.get('SUB_DICT').length() == gr_loaded1.get('SUB_DICT').length(), ...
	[BRAPH2.STR ':ExporterGroupSubjectCON_MP_TXT:' BRAPH2.BUG_IO], ...
