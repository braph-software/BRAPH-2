function create_data_NN_REG_ST_TXT(data_dir, random_seed)
%CREATE_DATA_NN_REG_ST_TXT creates strucutural data for neural network regression analysis
%
% CREATE_DATA_NN_REG_ST_TXT() creates strucutural data in default folder 'Example
%  data NN REG ST TXT'.
%
% CREATE_DATA_NN_REG_ST_TXT(DATA_DIR) creates strucutural data in DATA_DIR folder.
%
% CREATE_DATA_NN_REG_ST_TXT(DATA_DIR, RANDOM_SEED) cretes strucutural data in
%  DATA_DIR folder with a specified RANDOM_SEED.
%
% See also create_data_nn_reg_st_txt.

if nargin < 1
    data_dir = [fileparts(which('NNDataPoint_ST_REG')) filesep 'Example data NN REG ST TXT'];
end
