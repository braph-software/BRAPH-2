function create_data_NN_CLA_ST_TXT(data_dir, random_seed)
%CREATE_DATA_NN_CLA_ST_TXT creates structural data for neural network classification analysis
%
% CREATE_DATA_NN_CLA_ST_TXT() creates structural data in default folder 'Example
%  data NN CLA FUN TXT'.
%
% CREATE_DATA_NN_CLA_ST_TXT(DATA_DIR) creates structural data in DATA_DIR folder.
%
% CREATE_DATA_NN_CLA_ST_TXT(DATA_DIR, RANDOM_SEED) cretes structural data in
%  DATA_DIR folder with a specified RANDOM_SEED.
%
% See also create_data_nn_cla_st_xls.

if nargin < 1
    data_dir = [fileparts(which('NNDataPoint_ST_CLA')) filesep 'Example data NN CLA ST TXT'];
end
...
if ~isdir(data_dir)
    ...
    writetable(array2table([cellstr(sub_Tags3) cellstr(label_Tags3) cellstr(note_Tags3) num2cell(R3)], 'VariableNames', reg_Tags), [data_dir filesep() 'ST_Group_3.txt'], 'Delimiter', '	', 'WriteRowNames', true)
    ...
    writetable(table(vois3), [data_dir filesep() 'ST_Group_3.vois.txt'], 'Delimiter', '	', 'WriteVariableNames', false)
