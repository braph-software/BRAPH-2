...
function cb_exporter_TXT(~, ~)
    ...
    if ~is(ex.get('DIRECTORY'), 'NoValue') % % % ~strcmp(ex.get('DIRECTORY'), ExporterGroupSubjectFUN_MP_TXT.getPropDefault('DIRECTORY'))
        ex.get('SAVE');
    end
end
...
function cb_exporter_XLS(~, ~)
    ...
    if ~is(ex.get('DIRECTORY'), 'NoValue') % % % ~strcmp(ex.get('DIRECTORY'), ExporterGroupSubjectFUN_MP_XLS.getPropDefault('DIRECTORY'))
        ex.get('SAVE');
    end
end
...
false
%%%% ¡code!
im_ba = ImporterBrainAtlasXLS('FILE', [fileparts(which('SubjectFUN_MP')) filesep 'example data FUN_MP' filesep 'aal90_atlas.xlsx']);
ba = im_ba.get('BA');
