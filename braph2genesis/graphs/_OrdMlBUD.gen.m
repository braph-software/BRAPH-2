...
%%% ¡description!
In a ordinal binary undirected multilayer with fixed densities (BUD), the layers are 
...
%%%% ¡default!
'In a ordinal binary undirected multilayer with fixed densities (BUD), the layers are those of binary undirected (BU) multilayer graphs derived from the same weighted supra-connectivity matrices binarized at different densities. The supra-connectivity matrix has a number of partitions equal to the number of densities.The layers are connected in an ordinal fashion, where just consecutive layers are connected'
