%% ¡header!
PPNNCrossValidation_Feature_Map < PlotPropCell (pr, plot property feature map) is a plot of the feature map.

%%% ¡description!
PPNNCrossValidation_Feature_Map plots the feature map with in a brain atlas.
...
function plot_brain_surface(pr)
    ...
    el = pr.get('el');
    prop = pr.get('prop');
    sub = el.get('NN_DICT').getItem(1).get('GR').get('SUB_DICT').getItem(1);
