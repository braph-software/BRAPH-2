%% ¡header!
NNFeatureImportanceBrainSurfacePF_Measure_NU < NNFeatureImportanceBrainSurfacePF_Measure (pf, panel nodal superglobal ensemble-based comparison figure on brain surface figure) is the base element to plot a nodal superglobal ensemble-based comparison figure on brain surface figure.

%%% ¡description!
NNFeatureImportanceBrainSurfacePF_Measure_NU manages the basic functionalities to plot of a nodal superglobal ensemble-based comparison figure on brain surface figure.

%%% ¡seealso!
ComparisonEnsemble

%% ¡layout!

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.TOP_SHOWN
%%%% ¡title!
TOP ONLY

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.TOP_PERCENTAGE
%%%% ¡title!
TOP RATIO 

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.LAYER
%%%% ¡title!
Graph LAYER

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SPHS
%%%% ¡title!
Brain Region SPHERES ON/OFF

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SPH_DICT
%%%% ¡title!
Brain Region SPHERES PROPERTIES

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SIZE_DIFF
%%%% ¡title!
Show Difference with SPHERES SIZE

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SIZE_SCALE
%%%% ¡title!
Size SCALE

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.COLOR_DIFF
%%%% ¡title!
Show Difference with SPHERES COLOR

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.ID
%%%% ¡title!
Brain Atlas Figure ID

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.LABEL
%%%% ¡title!
Brain Atlas Figure NAME

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.WAITBAR
%%%% ¡title!
WAITBAR ON/OFF

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.NOTES
%%%% ¡title!
Brain Atlas NOTES

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.BKGCOLOR
%%%% ¡title!
BACKGROUND COLOR

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_POSITION
%%%% ¡title!
PANEL POSITION

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.VIEW
%%%% ¡title!
3D VIEW

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_AXIS
%%%% ¡title!
AXIS

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.BRAIN
%%%% ¡title!
BRAIN ON/OFF

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SURFFILE
%%%% ¡title!
BRAIN SURFACE

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_SURFACE
%%%% ¡title!
BRAIN COLOR

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_AMBIENT
%%%% ¡title!
MATERIAL & LIGHTNING

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SYMS
%%%% ¡title!
Brain Region SYMBOLS ON/OFF

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.SYM_DICT
%%%% ¡title!
Brain Region SYMBOLS PROPERTIES

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.IDS
%%%% ¡title!
Brain Region IDs ON/OFF

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.ID_DICT
%%%% ¡title!
Brain Region IDs PROPERTIES

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.LABS
%%%% ¡title!
Brain Region LABELS ON/OFF

%%% ¡prop!
%%%% ¡id!
NNFeatureImportanceBrainSurfacePF_Measure_NU.LAB_DICT
%%%% ¡title!
Brain Region LABELS PROPERTIES

%% ¡props_update!

%%% ¡prop!
ELCLASS (constant, string) is the class of % % % .
%%%% ¡default!
'NNFeatureImportanceBrainSurfacePF_Measure_NU'

%%% ¡prop!
NAME (constant, string) is the name of the panel figure nodal superglobal ensemble-based comparison figure on brain surface.
%%%% ¡default!
'NNFeatureImportanceBrainSurfacePF_Measure_NU'

%%% ¡prop!
DESCRIPTION (constant, string) is the description of the panel figure nodal superglobal ensemble-based comparison figure on brain surface.
%%%% ¡default!
'NNFeatureImportanceBrainSurfacePF_Measure_NU manages the basic functionalities to plot of a nodal superglobal ensemble-based comparison figure on brain surface.'

%%% ¡prop!
TEMPLATE (parameter, item) is the template of the panel figure nodal superglobal ensemble-based comparison figure on brain surface.
%%%% ¡settings!
'NNFeatureImportanceBrainSurfacePF_Measure_NU'

%%% ¡prop!
ID (data, string) is a few-letter code for the panel figure nodal superglobal ensemble-based comparison figure on brain surface.
%%%% ¡default!
'NNFeatureImportanceBrainSurfacePF_Measure_NU ID'

%%% ¡prop!
LABEL (metadata, string) is an extended label of the panel figure nodal superglobal ensemble-based comparison figure on brain surface.
%%%% ¡default!
'NNFeatureImportanceBrainSurfacePF_Measure_NU label'

%%% ¡prop!
NOTES (metadata, string) are some specific notes about the panel figure nodal superglobal ensemble-based comparison figure on brain surface.
%%%% ¡default!
'NNFeatureImportanceBrainSurfacePF_Measure_NU notes'

%%% ¡prop!
SETUP (query, empty) calculates the diff value and stores it to be implemented in the subelements.
%%%% ¡calculate!
g = pf.get('D').get('DP_DICT').get('IT', 1).get('G');

% get brain region related list
sph_list = pf.get('SPH_DICT').get('IT_LIST');
sym_list = pf.get('SYM_DICT').get('IT_LIST');
id_list = pf.get('ID_DICT').get('IT_LIST');
lab_list = pf.get('LAB_DICT').get('IT_LIST');

% get the value to show on the surface
layer = pf.get('LAYER');
fis = pf.get('FI');
if isempty(fis)
    value = {};
    return
end
fi = fis{layer};
if isempty(fi)
    value = {};
    return
end

% apply top percentage to spheres, symbols, ids, and labels
top_shown = pf.get('TOP_SHOWN');
top_percentage = pf.get('TOP_PERCENTAGE');
switch top_shown
    case 'on'
        mask = zeros(size(fi));
        [~, idx_fi] = sort(fi, 'descend');
        mask(idx_fi(1:floor(top_percentage * length(fi)))) = 1;
        mask = logical(mask);
        for i = 1:1:length(sph_list)
            set(sph_list{i}, 'VISIBLE', mask(i));
        end
        for i = 1:1:length(sym_list)
            set(sym_list{i}, 'VISIBLE', mask(i));
        end
        for i = 1:1:length(id_list)
            set(id_list{i}, 'VISIBLE', mask(i));
        end
        for i = 1:1:length(lab_list)
            set(lab_list{i}, 'VISIBLE', mask(i));
        end
    case 'off'
        if pf.get('SPHS')
            for i = 1:1:length(sph_list)
                set(sph_list{i}, 'VISIBLE', true);
            end
        end
        if pf.get('SYMS')
            for i = 1:1:length(sym_list)
                set(sym_list{i}, 'VISIBLE', true);
            end
        end
        if pf.get('IDS')
            for i = 1:1:length(id_list)
                set(id_list{i}, 'VISIBLE', true);
            end
        end
        if pf.get('LABS')
            for i = 1:1:length(lab_list)
                set(lab_list{i}, 'VISIBLE', true);
            end
        end
    case 'disable'
end

size_diff = pf.get('SIZE_DIFF');
switch size_diff
    case 'on'
        % transfrom diff value to appropriate size
        % value ranching from 0.01 to 1
        fi(isnan(fi)) = 0.1;
        fi(isinf(fi)) = 0.1;
        size_value = abs(fi);
        min_bound = 0.01;
        max_bound = 1.0;
        min_size_value = min(size_value);
        max_size_value = max(size_value);
        if max_size_value == min_size_value
            normalized_size_value = ones(size(size_value)) * max_bound;
        else
            normalized_size_value = min_bound + (max_bound - min_bound) * (size_value - min_size_value) / (max_size_value - min_size_value);
        end
        size_scale = pf.get('SIZE_SCALE');
        scaled_size_value = normalized_size_value * size_scale;

        % set size to sphs
        for i = 1:1:length(sph_list)
            set(sph_list{i}, 'SPHERESIZE', scaled_size_value(i));
        end
    case 'off'
        if pf.get('SPHS')
            for i = 1:1:length(sph_list)
                set(sph_list{i}, 'SPHERESIZE', SettingsSphere.getPropDefault('SPHERESIZE'));
            end
        end
    case 'disable'
end

color_diff = pf.get('COLOR_DIFF');
switch color_diff
    case 'on'
        % transfrom m value to appropriate color
        % RGB code ranching from 0 to 1
        fi(isnan(fi)) = 0;
        size_value = abs(fi);
        min_bound = 0.0;
        max_bound = 1.0;
        min_size_value = min(size_value);
        max_size_value = max(size_value);
        if max_size_value == min_size_value
            normalized_size_value = ones(size(size_value)) * max_bound;
        else
            normalized_size_value = min_bound + (max_bound - min_bound) * (size_value - min_size_value) / (max_size_value - min_size_value);
        end

        % Map the normalized values to colors in the Jet colormap
        cmap = jet(256);
        color_indices = ceil(normalized_size_value * (size(cmap, 1) - 1)) + 1;

        % Clip color_indices to valid range
        color_indices = max(1, min(color_indices, size(cmap, 1)));

        % Get the RGB colors for the indices
        rgb_colors = cmap(color_indices, :);

        % set color to sphs
        for i = 1:1:length(sph_list)
            set(sph_list{i}, 'FACECOLOR', rgb_colors(i, :));
        end
    case 'off'
        if pf.get('SPHS')
            for i = 1:1:length(sph_list)
                set(sph_list{i}, 'FACECOLOR', SettingsSphere.getPropDefault('FACECOLOR'));
            end
        end
    case 'disable'
end

value = {};

%% ¡props!

%%% ¡prop!
LAYER (figure, scalar) is the layer number of the nodal measure.
%%%% ¡default!
1
%%%% ¡postset!
pf.get('SETUP');
%%%% ¡gui!
pr = NNFeatureImportanceBrainSurfacePF_x_xUPP_Layer('EL', pf, 'PROP', NNFeatureImportanceBrainSurfacePF_Measure_NU.LAYER);

%%% ¡prop!
SIZE_DIFF (figure, option) determines whether the difference is shown with size effect.
%%%% ¡settings!
{'on' 'off' 'disable'}
%%%% ¡default!
'on'
%%%% ¡postset!
pf.get('SETUP');

%%% ¡prop!
SIZE_SCALE (figure, scalar) determines the scale of size effect.
%%%% ¡default!
10
%%%% ¡postset!
pf.get('SETUP');

%%% ¡prop!
COLOR_DIFF (figure, option) determines whether the difference is shown with color effect.
%%%% ¡settings!
{'on' 'off' 'disable'}
%%%% ¡default!
'on'
%%%% ¡postset!
pf.get('SETUP');

%%% ¡prop!
TOP_SHOWN (figure, option) determines whether to show the features of which the feature importance are on top.
%%%% ¡settings!
{'on' 'off' 'disable'}
%%%% ¡default!
'off'
%%%% ¡postset!
pf.get('SETUP');

%%% ¡prop!
TOP_PERCENTAGE (figure, scalar) determines the percentage to show for top feature importance values.
%%%% ¡default!
0.05
%%%% ¡postset!
pf.get('SETUP');

%% ¡tests!

%%% ¡excluded_props!
[NNFeatureImportanceBrainSurfacePF_Measure_NU.PARENT NNFeatureImportanceBrainSurfacePF_Measure_NU.H NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_POSITION NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_AXIS NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_SURFACE NNFeatureImportanceBrainSurfacePF_Measure_NU.ST_AMBIENT NNFeatureImportanceBrainSurfacePF_Measure_NU.CP]

%%% ¡warning_off!
true

%%% ¡test!
%%%% ¡name!
Remove Figures
%%%% ¡code!
warning('off', [BRAPH2.STR ':NNFeatureImportanceBrainSurfacePF_Measure_NU'])
assert(length(findall(0, 'type', 'figure')) == 5)
delete(findall(0, 'type', 'figure'))
warning('on', [BRAPH2.STR ':NNFeatureImportanceBrainSurfacePF_Measure_NU'])
