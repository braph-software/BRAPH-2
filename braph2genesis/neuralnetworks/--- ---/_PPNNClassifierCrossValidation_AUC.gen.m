%% ¡header!
PPNNClassifierCrossValidation_AUC < PlotPropCell (pr, plot property auc) plots the ROC for neural network analysis.

%%% ¡description!
PPNNClassifierCrossValidation_AUC plots the ROC for neural network analysis.
...
function h_panel = draw(pr, varargin)
    ...
    function cb_button_mod_calc(~, ~)              
        ...
        mean_curve = 0;
        for i = 1:1:el.get('NNE_DICT').length()
            ...
            mean_curve = mean_curve + (interp1(Xadjusted, Y{i}, intervals))/el.get('NNE_DICT').length();
        end
