%% ¡header!
PPNNClassifierCrossValidation_Confusion_Matrix < PlotPropMatrix (pr, plot property confusion matrix) plots the confusion matrix for cross validation analysis.

%%% ¡description!
PPNNClassifierCrossValidation_Confusion_Matrix plots confusion matrix for neural network analysis.
...
function h_panel = draw(pr, varargin)
    ...
    function cb_button_mod_calc(~, ~)              
        ...
        if ~isa(el.getr('NN_DICT'), 'NoValue') 
            nn = el.get('NN_DICT').getItem(1);
            gr = nn.get('GR');
