%% ¡header!
PPNNClassifierGAT_Model < PlotProp (pr, plot property model) plots the training progress of graph attention networks.

%%% ¡description!
PPNNClassifierGAT_Model plots the training progress of graph attention networks.
...
function h_panel = draw(pr, varargin)
    ...
    function cb_button_mod_calc(~, ~)              
        ...
        if BRAPH2.installed('NN', 'masgbox')
            pr.h = figure('Name', 'Training progress', 'UNITS', 'normalized', 'Position', [x/screen_w y/screen_h w/screen_w h/screen_h]);
        end
        pr.cb_button_calc()
    end
...
function get_buttons(pr)
    ...
    for i = 1:length(plot_prop_children)
        ...
        if check_graphics(pp_c, 'pushbutton') && isequal(pp_c.Tag, 'button_calc')
            pr.c_btn = pp_c;
            if pp_c.Value == 1 % button is on
            end
        end
        if check_graphics(pp_c, 'pushbutton') && isequal(pp_c.Tag, 'button_del')
            if any(isgraphics(pr.h)) && pp_c.Value == 1 % button is on
                close(pr.h);
            end
            pr.d_btn = pp_c;
        end
