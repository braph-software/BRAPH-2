classdef Category < handle
    ...
    %               (can be NoValue when not set, allow incoming and outgoing
    %               callbacks, not cloned).
    %
    %  RESULT       Result calculated by the element using parameters and data
    %               (can be NoValue when not calculated, allows incoming
    %               callbacks).
    %
    ...
    methods (Static)
        ...
        function category_number = getCategoryNumber()
            ...
            category_number = 4; % numel(Category.getCategories());
        end
