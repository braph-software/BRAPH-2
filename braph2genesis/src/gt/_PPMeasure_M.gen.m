...
% FIXME: use instead pr.measure_value_container = get(p, 'UserData')

% FIXME: eliminate sliders from here
% % % measure_text
% % % slider
...
% % %     L = length(value);
    ...
    if Measure.is_global(el)
        pr.p = PlotGlobalMeasure('EL', el, 'PROP', prop, 'ID', 'm', 'TITLE', 'M', 'BKGCOLOR', [0.8 0.5 0.2]).draw();
...
%     else
%         if Measure.is_global(el)
%             pr.measure_value_container.redraw('EDITHEIGHT', 3.3, varargin{:})
%         else
%             pr.measure_value_container.redraw('WIDHT', varargin{:})
%         end
%         
%     end
end