%% ¡header!
PlotPropMeasureNodalMultilayer < PlotPropMatrix (pr, plot nodal measure mp) represents the nodal measure mp.
...
function h_panel = draw(pr, varargin)
    ...
    if el.get('G').getPropNumber() > 9
        pr.layer_slider = uicontrol( ...
            ...
            'SliderStep', [map_multiplier/(n*map_multiplier) map_multiplier/(n*map_multiplier)] , ...
...
function update(pr)
    ...
    if isa(value, 'NoValue')
        ...
        if el.get('G').getPropNumber() > 9
            ...
            set(pr.layer_text, ...
            'String', [label ': ' slider_tags{round(get(pr.layer_slider, 'Value') * map_multiplier)}]);
