...
function cb_importer_TXT(~, ~)
    try
        im = ImporterBrainAtlasTXT('ID', 'Import Brain Atlas from TXT', 'WAITBAR', true).get('GET_FILE');
        ba = im.get('BA');
...
function cb_importer_XLS(~, ~)
    try
        im = ImporterBrainAtlasXLS('ID', 'Import Brain Atlas from XLS', 'WAITBAR', true).get('GET_FILE');
        ba = im.get('BA');
        gui = GUIElement('PE', ba);
        gui.get('DRAW')
        gui.get('SHOW')
...
function cb_exporter_TXT(~, ~)
    ex = ExporterBrainAtlasTXT('ID', 'Export Brain Atlas to TXT', 'BA', el.copy(), 'WAITBAR', true).get('PUT_FILE');
...
function cb_exporter_XLS(~, ~)
    ex = ExporterBrainAtlasXLS('ID', 'Export Brain Atlas to XLS', 'BA', el.copy(), 'WAITBAR', true).get('PUT_FILE');
