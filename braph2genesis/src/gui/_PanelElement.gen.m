...
% % %     pe.update()
    set(pe.p, 'Visible', 'on') % calls also redraw() and slide()
...
% % %     % See also draw, update, slide.
    ...
    % redraw prop panels
    dw = pe.get('DW') * BRAPH2.S;
    w_s = pe.get('WSLIDER') * BRAPH2.S;
    for prop = 1:1:length(pr_list)
        pe.get('PR_DICT').getItem(prop).redraw('Width', w(p, 'pixels') - 2 * dw - w_s)
    end

    % redraw panel and place prop panels (pp)
    dh = pe.get('DH') * BRAPH2.S;
    ...
    if h_p > h(p, 'pixels')
        for prop = 1:1:length(pr_list)
            pr = pr_list{prop};
            set(pr, 'Position', [x0_pp y0_pp(prop) w(pr, 'pixels') h(pr, 'pixels')])
        end
    else
        for prop = 1:1:length(pr_list)
            pr = pr_list{prop};
            set(pr, 'Position', [x0_pp y0_pp(prop)+h(p, 'pixels')-h_p w(pr, 'pixels') h(pr, 'pixels')])
        end
    end    
