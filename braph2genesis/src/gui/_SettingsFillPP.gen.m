...
function redraw(pr, varargin)
    ...
    set(pr.checkbox_visible, 'Position', [s(.3) s(4.3) .2*w(pr.p, 'pixels') s(1.7)])
    set(pr.facecolor_button, 'Position', [s(.3) s(2) .2*w(pr.p, 'pixels') s(1.7)])
    set(pr.facealpha_label, 'Position', [s(.6)+.35*w(pr.p, 'pixels')+s(1.7) s(4.5) .2*w(pr.p, 'pixels') s(1.7)])
    set(pr.facealpha_slider, 'Position', [s(.6)+.35*w(pr.p, 'pixels')+s(1.7) s(3) .45*w(pr.p, 'pixels')   3])
end
...
function cb_fill(pr)
    ...
    el.get(prop).set( ...
        'Visible', get(pr.checkbox_visible, 'Value'), ...
        'FACEALPHA', get(pr.facealpha_slider, 'Value') ...
        )
    pr.update()
end