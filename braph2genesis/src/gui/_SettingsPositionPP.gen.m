...
% % %     % See also update, redraw, uipanel.
    ...
    if ~check_graphics(pr.label_x0, 'uilabel')
        pr.label_x0 =  uilabel( ...
            ...
            'Tooltip', [num2str(prop) ' ' upper(el.getPropTag(prop)) '>' num2str(el.get(prop).getPropProp('X0')) ' ' el.get(prop).getPropDescription('Y0')], ...
...
function cb_position(pr)
    ...
    el.get(prop).set('AUTOPOS', get(pr.checkbox_autopos, 'Value'))
    el.get(prop).set('X0', get(pr.editfield_x0, 'Value'))
    el.get(prop).set('Y0', get(pr.editfield_y0, 'Value'))
    el.get(prop).set('WIDTH', get(pr.editfield_w, 'Value'))
    el.get(prop).set('HEIGHT', get(pr.editfield_h, 'Value'))
