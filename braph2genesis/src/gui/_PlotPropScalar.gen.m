...
function h_panel = draw(pr, varargin)
    ...
    %
    % DRAW(PL) draws the panel of the scalar property.
    %
    % H = DRAW(PL) returns a handle to the the panel of the scalar property.
    %
...
function update(pr)
    ...
    %
    % UPDATE(PL) updates the content and permissions of the edit field.
    %
...
function redraw(pr, varargin)
    ...
    pr.redraw@PlotProp('Height', 2.50, varargin{:})
end
function cb_edit_value(pr)
    %CB_EDIT_VALUE executes callback for the edit value.
    %
    % CB_EDIT_VALUE(PL) executes callback for the edit value.
