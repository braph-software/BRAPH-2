...
function redraw(pr, varargin)
    ...
    h_ps_header = ceil(.5 * BRAPH2.FONTSIZE * BRAPH2.S); % height section header
    ...
    for s = 1:1:ps_number
        ...
        set(pr.ps_panels{s}, ...
            'Units', 'pixels', ...
            'Position', [s5 ps_y w(pr.ps_panels{s}, 'pixels')-s5-s5 ps_h] ...
            )
        
        for c = 1:1:pc_numbers(s)
            set(pr.pc_btns{s}{c}, ...
                'Units', 'pixels', ...
                'Position', [s5 (h_ps_footer+(pc_numbers(s) - c)*h_pc) w(pr.ps_panels{s}, 'pixels')-s5-s5 h_pc] ...
                )
        end
    end
end
