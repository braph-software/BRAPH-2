...
function redraw(pr, varargin)
    ...
    [h_p, varargin] = get_and_remove_from_varargin(s(5), 'Height', varargin);
    
    pr.redraw@PanelProp('Height', h_p, varargin{:})
    
    set(pr.editfield, 'Position', [s(.3) s(.7) .15*w(pr.p, 'pixels') s(1.7)])
    
    set(pr.slider, 'Position', [s(.3)+.15*w(pr.p, 'pixels')+s(1.7) s(2.25) .70*w(pr.p, 'pixels') 3]) % the height of a slider cannot be changed
end
