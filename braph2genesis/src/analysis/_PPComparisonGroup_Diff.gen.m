...
function h_panel = draw(pr, varargin)
    ...
    if L2 > 1
        pr.second_slider = uicontrol( ...
            ...
            'SliderStep', [map_multiplier/(L2*map_multiplier) map_multiplier/(L2*map_multiplier)] , ...
            'Callback', {@cb_slider} ...
            );
...
function update(pr)
    ...
    if  size(value, 2) > 2
        ...
        cil = el.memorize('cil');
        ciu = el.memorize('ciu');
        p1 = p1{round(get(pr.slider, 'Value')), round(get(pr.second_slider, 'Value')* map_multiplier)};
        p2 = p2{round(get(pr.slider, 'Value')), round(get(pr.second_slider, 'Value')* map_multiplier)};
        cil = cil{round(get(pr.slider, 'Value')), round(get(pr.second_slider, 'Value')* map_multiplier)};
        ciu = ciu{round(get(pr.slider, 'Value')), round(get(pr.second_slider, 'Value')* map_multiplier)};
...
function redraw(pr, varargin)
    ...
    if L2 > 1
        ...
        set(pr.slider, ...
            'Units', 'normalized', ...
            'Visible', 'on', ...
            'Position', [.01 (Sh+Th+Dh)/(h+Sh+Sh+Th+Th+Dh)-.2 .97 (Th/(h+Sh+Sh+Th+Th+Dh)-.02)] ...
            );
        
        set(pr.slider_text, ...
            'Units', 'normalized', ...
            'Visible', 'on', ...
            'Position', [.01 (Sh+Th+Th+Dh)/(h+Sh+Sh+Th+Th+Dh)-.2 .97 (Th/(h+Sh+Sh+Th+Th+Dh)-.02)] ...
            );
        
        set(pr.second_slider, ...
            'Units', 'normalized', ...
            'Position', [.01 (Dh)/(h+Sh+Sh+Th+Th+Dh)-.2 .97 (Th/(h+Sh+Sh+Th+Th+Dh)-.02)] ...
            );
        
        set(pr.second_slider_text, ...
            'Units', 'normalized', ...
            'Position', [.01 (Dh+Th)/(h+Sh+Sh+Th+Th+Dh)-.2 .97 (Th/(h+Sh+Sh+Th+Th+Dh)-.02)] ...
            );
        
        set(pr.comparison_tbl, ...
            'Visible', 'on', ...
            'Units', 'normalized', ...
            'Position', [.01 .02 .97 (Dh/(h+Sh+Sh+Th+Th+Dh)-.2)] ...
            )
    elseif L
