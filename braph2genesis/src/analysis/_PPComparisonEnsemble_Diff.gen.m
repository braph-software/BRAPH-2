...
function update(pr, sliders_condition)
    ...
    if isa(value, 'NoValue')
        ...
        if layer_dim_element.getGraphType() == 4 && layer_dim_element.getPropNumber() > 9 % mp bud/but
            ...
            if Measure.is_superglobal(el.get('MEASUREPARAM'))
                L = size(layer_dim_element.get('B'), 2);
            else
                L = L/L2;
            end
        ...
        if L2 > 1 && init_sliders
            ...
            if isempty(pr.second_slider_text)
        ...
        if layer_dim_element.getGraphType() == 4 && L > 1 && L2 > 1 && ~Measure.is_superglobal(el.get('Measure'))% mp bud
            set(pr.slider_text, ...
        ...
        elseif layer_dim_element.getGraphType() == 4 && L > 1 && L2 > 1 && Measure.is_superglobal(el.get('Measure')) % superglobal bud
            set(pr.slider_text, ...
                'String', ['Layer: ' num2str(round(get(pr.slider, 'Value') * map_multiplier))]);
            set(pr.second_slider_text, ...
                'String', [label ' ' num2str(round(get(pr.second_slider, 'Value') * map_multiplier)) ': ' slider_tags{round(get(pr.second_slider, 'Value') * map_multiplier)}]);
            ...
            tmp_value = value{round(get(pr.second_slider, 'Value') * map_multiplier)};
            p1 = el.memorize('P1');
            p2 = el.memorize('P2');
            cil = el.memorize('cil');
            ciu = el.memorize('ciu');
            p1 = p1{round(get(pr.second_slider, 'Value') * map_multiplier)};
            p2 = p2{round(get(pr.second_slider, 'Value') * map_multiplier)};
            cil = cil{round(get(pr.second_slider, 'Value') * map_multiplier)};
            ciu = ciu{round(get(pr.second_slider, 'Value') * map_multiplier)};
        ...
        elseif layer_dim_element.getGraphType() == 4 && L == 1 && L2 > 1 && Measure.is_superglobal(el.get('Measure')) % mp superglobal wu
            set(pr.second_slider_text, ...
