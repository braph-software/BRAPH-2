...
function h_panel = draw(pr, varargin)
    ...
    if ~check_graphics(pr.graph_btn, 'pushbutton') || ~check_graphics(pr.measure_btn, 'pushbutton')
        ...
        pr.graph_btn = uicontrol( ...
            ...
            'Parent', pr.p, ...
            'Units', 'normalized', ...
            'Position', [.01 .11 .4 .09], ... % position defined here because it's always the same
...
function update(pr)
    ...
    if  ~isa(graph, 'NoValue') && isa(graph, 'Graph')
        ...
        pr.already_calculated = is_measure_calculated();
...
function list =  is_measure_calculated(pr)
    ...
    calculated_list = cell(size(measure_list, 1), 1);
