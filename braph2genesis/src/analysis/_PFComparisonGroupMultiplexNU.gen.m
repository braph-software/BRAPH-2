%% ¡header!
PFComparisonGroupMultiplexNU < PFComparisonGroupGU (pf, panel figure group comparison multiplex GU) is a plot of a nodal unilayer group comparison for multiplex graphs.
...
if check_graphics(pf.h_area_ci, 'patch')
    ...
    pf.get('ST_FILL').set( ...
        'X', pf.get('CP').get('MEASURE_TEMPLATE').get('G').get('LAYERTICKS'), ...
        'Y_UPPER', cellfun(@(x) x(br1_id), data_to_plot_u) , ...
        'Y_LOWER', cellfun(@(x) x(br1_id), data_to_plot_l) ...
        )
end
%%%% ¡gui!
pr = SettingsFillPP('EL', pf, 'PROP', PFComparisonGroupMultiplexNU.ST_FILL, ...    
    varargin{:});
...
pr = PP_LayerID('EL', pf, 'PROP', PFComparisonGroupMultiplexNU.LAYER, ...
    'G', g, ...
    varargin{:});
...
pr = PP_BrainRegion('EL', pf, 'PROP', PFComparisonGroupMultiplexNU.BR1_ID, ...
    'BA', ba, ...
    varargin{:});
